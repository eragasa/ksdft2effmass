#!/usr/bin/env python3
"""Validate the bounded production-source-facts implementation slice.

This command reconstructs and checks only declared control projections, structural
software evidence, formatting, typing, documentation, and changed-path scope. It
never repairs source and establishes no runtime semantic completeness, architecture
policy finding, numerical verification, scientific validation, uncertainty
quantification, human acceptance, or successor authority.
"""
...
class PythonArchitectureProductionFactsCompletionValidator:
    """Validate activation, ownership, implementation evidence, and projections."""

    repository_root: Path

    _TASK = "python.architecture-refactor.architecture-conformance.production-facts"
    _TEST = (
    ...
    _OWNERSHIP = (
        ...
        "production-facts-test-ownership.json"
    )
    ...
    _OWNED_PATHS = frozenset(
        {
            ...
            ".pi/task-ownership/validate_python_architecture_conformance_production_facts.py",
            ...
            "python/tests/software_verification/ksdft2effmass/harness/resources/production-facts-test-ownership.json",
            "python/tests/software_verification/ksdft2effmass/harness/resources/production_facts/invalid_syntax.py.txt",
            "python/tests/software_verification/ksdft2effmass/harness/resources/production_facts/invalid_utf8.bin",
            "python/tests/software_verification/ksdft2effmass/harness/resources/production_facts/rich_source.py.txt",
            _TEST,
    ...
    def execute(self) -> int:
        """Return zero only when every bounded deterministic completion check passes."""
        missing = tuple(
            path
        ...
        source = "python/src/ksdft2effmass/harness/pi/conformance/python/production.py"
        completion_validator = (
            ".pi/task-ownership/"
            "validate_python_architecture_conformance_production_facts.py"
        )
        existing_conformance_tests = (
            (
                "python/tests/software_verification/ksdft2effmass/harness/"
        ...
        commands: tuple[tuple[str | Path, ...], ...] = (
            (
                ...
                (
                    ...
                    "production-facts.json"
                ),
            ),
            (interpreter, "-m", "pytest", "-q", self._TEST),
            (interpreter, "-m", "pytest", "-q", *existing_conformance_tests),
            (
                interpreter,
                "-m",
                "ksdft2effmass.harness.cli",
            ...
            (interpreter, "-m", "mypy", source, self._TEST, completion_validator),
        )
        for command in commands:
            if self._run(command) != 0:
                return 1

        with tempfile.TemporaryDirectory(prefix="production-facts-sphinx-") as build:
        ...
        for command in final_commands:
            if self._run(command) != 0:
                return 1
        print(
            "PASS: bounded accepted software-verification closeout state is consistent; "
            "the validator does not independently establish runtime completeness, "
            "policy findings, numerical/scientific/VVUQ claims, release status, or "
            "successor activation."
        )
        return 0

    def _activation_state_is_exact(self) -> bool:
        """Check accepted closeout and bounded parent/sibling phase state."""
        required = {
            "harness/task-selection.json": (
                '"active_task_id": null',
            ...
            f"tasks/software/{self._TASK}.json": (
                ...
                "human acceptance of the bounded production-facts software-verification result",
                "known repository-wide mypy duplicate-conftest limitation",
                "task selection is cleared",
            ),
            "tasks/software/python.architecture-refactor.architecture-conformance.json": (
                '"status": "deferred_between_children"',
                "accepted and authorized",
                "selection is cleared",
                "parent remains deferred between children and is not closed",
            ),
        }
        inactive = (
...
def main() -> int:
    ...
    return PythonArchitectureProductionFactsCompletionValidator(
        repository_root
    ).execute()
