...
- Document private scientific or numerical policy and non-obvious implementation owners with their responsibility, assumptions, invariants, units, failure behavior, and relationship to the public contract as applicable. Private mechanical helpers and obvious stored state need concise documentation only when it improves understanding; do not restate assignments or types.
- Add nearby comments for scientifically, numerically, or architecturally meaningful local variables such as canonicalized values, residuals, norms, singular values, compatibility findings, validation state, unit conversions, shapes, and deterministic ordering state when the comment explains meaning rather than the assignment.
