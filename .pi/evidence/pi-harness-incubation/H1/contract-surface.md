...
| `DeserializeJsonRecord.execute(record_kind: WireRecordKind, payload: bytes)` | `JsonDeserializationResult` | UTF-8/JSON/duplicate-key/field/type/intrinsic-invariant/version failure returns `FAIL` and no record. Structurally valid `ResourceReference`/`ResourceManifest` candidates with relational defects deserialize successfully and require `ValidateResourceManifest`. | In-memory parsing only. | Explicit closed record kind selects one exact constructor; success establishes structural record validity only. | No inferred kind, dynamic import, schema discovery, hidden migration, or manifest-validity claim. |
...
| `ValidateResourceManifest.execute(generic_manifest: ResourceManifest, generic_manifest_identity: ArtifactIdentity, local_manifest: ResourceManifest | None, local_manifest_identity: ArtifactIdentity | None, profile: ProjectProfile)` | `ValidationResult` | Reports manifest/profile identity or base mismatch; duplicate entry ID/path; self-edge; dependency absence/cycle; generic-to-local dependency; forbidden local replacement; or unsupported kind/version. | None. | Owns all cross-entry and cross-manifest validity. Duplicate IDs/paths precede dependency analysis; self-edges use `PIH.RESOURCE.DEPENDENCY_CYCLE`. Profile supplies supported extensions and policy references. | Does not resolve files, authorize resources, or reinterpret successful construction as manifest acceptance. |
...
`ResourceReference` and `ResourceManifest` are structurally valid candidate
records. Their successful construction, deserialization, or serialization does
not imply usable-manifest validity. `ResolveResource` and
`ValidateSkillResources` first propagate `ValidateResourceManifest` failure and
must not select, resolve, or interpret an invalid candidate manifest.
