...
State claim status precisely. Distinguish implemented capability, software verification, numerical verification, scientific validation, uncertainty quantification, and proposed work; do not infer a stronger status from tests, builds, reviews, or plans.

Document exact concrete types, closed unions, protocols, representation-to-domain conversion, and explicit callable owners. Do not introduce or normalize `Any`, `cast(Any, ...)`, generic `object` boundaries, origin-based trusted/untrusted software language, or dangling non-entry-point functions. Keep authored test resources beneath `python/tests/**/resources/`. Refer to large text or binary artifacts by blob marker, attachment, or path plus content identity and inspect only bounded material. Examples must use supported public interfaces and be truthful, reproducible at their stated status, and clearly identified when illustrative or synthetic. Maintain assigned navigation, links, and source integration without duplicating an owning source of truth.
