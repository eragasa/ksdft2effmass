...
Status: nested audit items are named `P2-A00`--`P2-A11`; `P2-A01` is cleared and `P2-A02` is next but not started; P2 remains open and unaccepted after `P2-HC05` Option B
...
The current human instruction records one authoritative audit queue at `.pi/evidence/backend-neutral-cpn-P2-tools-provenance/provenance-audit-queue.json`. It contains exactly P2-A00--P2-A11, permits only one mutable item at a time, distinguishes deterministic structural validation from semantic review, and requires every item to reach `audited_and_cleared` before P2 may be accepted. P2-A00 (`actions.py`) is cleared. P2-A01 (`external_tools.py` and its four class-owned modules) is now `audited_and_cleared`: its field-wise invalidity, requested-version grammar, complete equality, concrete enum-surface, formatting, and prose findings were corrected without production changes; deterministic validation and the sole targeted semantic review passed. P2-A02 is next but was not started. P2-A02--P2-A06 remain pending read-only audits; P2-A07--P2-A11 remain pending artifact audits.

`P2-HC05` Option B authorized only the nomenclature correction; it did not accept P2. Recording or clearing an audit item does not authorize P2 acceptance, activate P3/H5, authorize external or scientific execution, or establish numerical verification, scientific validation, UQ, provenance truth, publication readiness, or release readiness.