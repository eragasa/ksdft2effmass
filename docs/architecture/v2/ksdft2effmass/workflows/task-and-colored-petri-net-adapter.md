...
The implemented activation-selection slice follows selected explicit-mapping Option B. `ColoredPetriNetWorkflowMapping` is immutable Workflow-owned data for one run-scoped Task instance. It identifies independent Workflow directed-selection permission plus the exact direct and `all_of` activation transitions. `WorkflowResultTokenMapping` correlates each already-bound ResultObject identity and Task input name to one generic binding variable and one supplied individually identified generic token at one exact predecessor place. Candidate bindings must equal the mapped token value for every represented variable. The mapping asserts correlation only; it does not derive a generic value from scientific object fields. `ColoredPetriNetWorkflowActivationRequest` closes the mapping, definition, marking, run, Task instance, inputs, result-token correlations, activation, operation, attempt, and direct/automatic intent. The adapter returns one content-identified closed `activated`, `not_enabled`, or `failure` result while retaining exact generic enablement and optional selection results.

For Task-origin work, the adapter maps Workflow-owned gates and supplied `ResultObject` token correlations to generic colored-Petri-net inputs. It applies the `TaskStartGateSet` mode to determine the intended singular `any_of` gate/binding or canonical compatible `all_of` tuple, maps that domain choice to one exact generic transition and binding, and obtains the generic `ColoredPetriNetEnablementResult` and `ColoredPetriNetSelectionResult`. `all_of` combines a complete member tuple only when shared variable assignments agree and the union exactly matches the mapped activation transition's declared input variables. When the intended domain choice is not the generic canonical choice, both the Workflow mapping and generic definition must permit the exact identity-bound `ColoredPetriNetSelectionDirective`; no other override exists. It constructs the corresponding discriminated `TaskActivation`, or a `direct` activation when an enclosing caller invokes a zero-gate Task instance, and that existing activation record references the exact generic selection result. The adapter is effect-free: workflow control and, for dispatched calculator work, `SimulationDispatchAdapter` invoke the selected Task or executor through the accepted authority boundary.
...
For a nested Workflow, the parent first durably records one exact `NestedWorkflowInvocationIntent` (or retains an actual existing combined pending `NestedWorkflowInvocation`) correlation binding the parent run/revision, parent Task instance, activation, operation, attempt, intended child Workflow definition, child `WorkflowRunIdentity`, input ResultObjects, and child-creation idempotency identity. The child Workflow owns its initial/current marking, Task instances, ordered transitions, failures, replay, and revisions in its own `WorkflowRun`. Child creation and parent advancement are separate single-stream commits; no cross-run atomicity is claimed.

A separate first `NestedWorkflowTerminalObservation` requires its exact intent source in the actual predecessor and introduces the terminal attempt/generic outcome group atomically. Intent is never replaced or copied into a fabricated historical record. One first terminal is allowed, including indeterminate; repeated terminal reconciliation is not introduced by this contract.
...
- Public wire fields for the implemented in-memory mapping, request, result, and activation contracts.
- Domain-specific derivation of generic token values from concrete ResultObject fields; the implemented adapter consumes explicit supplied result-token correlations instead.
- Nested-Workflow cancellation and compensation policy.
- Independent implemented-correction review and remaining nested-history verification.
