...
flowchart LR
    ...
    prepare --> reservation_commit["WorkflowRunAtomicRepository<br/>commit reserved candidate"]
    reservation_commit --> claim_authorize["Claim-phase authorization<br/>exact reserved grant view"]
    claim_authorize --> claim_prepare["Prepare claimed successor candidate"]
    claim_prepare --> claim_commit["WorkflowRunAtomicRepository<br/>commit claim + return claim receipt"]
    claim_commit --> dispatch_authorize["Immediate claim-phase reauthorization"]
    dispatch_authorize --> dispatch_entry["WorkflowRunDispatchEntryCommitter<br/>reconcile claim + win entry CAS"]
    dispatch_entry --> executor_check["Local executor correlation recheck"]
...
`SimulationDispatchControlWorkflow` now composes the persisted reservation, claim, immediate authorization, dispatch-entry, and effect sequence without issuing authority, retrying, or performing result ingress. The exact internal preparation sequence may preserve the accepted request-transition and obligation contracts, but the semantic owners remain fixed: the generic layer returns one identity-closed enablement and selection result and fires pure values; on the task-origin path the effect-free adapter applies `any_of`/`all_of` composition or direct activation, maps the intended choice through canonical generic selection or an exact definition-permitted directive, creates TaskActivation referencing that generic selection result, and maps only confirmed invocation ResultObjects plus the same selection identities into `ColoredPetriNetFiringInput`; workflow control invokes ordinary Tasks and owns closed generic invocation outcomes; and each nested Workflow owns a distinct correlated child WorkflowRun. `SimulationDispatchPreparer` constructs the reserved candidate, `SimulationDispatchClaimPreparer` constructs the claimed candidate, `SimulationDispatchAdapter` repeats claim-phase authorization immediately before durable dispatch entry, and the local executor independently rechecks its complete plan/request correlation before process entry. Workflow control constructs complete candidate successor units. `WorkflowRunAtomicRepository` invokes its bound validator on the exact candidate, serializes that same validated candidate, verifies candidate/bytes/content/revision identity binding, and submits the opaque commit only after those checks succeed.
...
- Each nested Workflow has a distinct child WorkflowRun and marking. The parent retains immutable intent and one separate first-terminal observation; actual combined pending records remain valid unchanged intent sources. Terminal introduction requires that source in the actual predecessor. Only explicit exports from a replay-equal terminal child revision are admitted. Indeterminate terminal evidence preserves exact child/idempotency correlations without automatic duplicate creation or a second terminal observation.
- Start-gate policy and Task input contract are distinct; already-bound inputs must satisfy both.
- One grant authorizes one exact dispatch. The request transaction atomically records its reservation to one obligation. A distinct claim-phase authorization precedes the expected-revision claim commit from `reserved` to `claimed`. The resulting claim receipt is historical evidence, not effect-entry permission. Immediately before the effect, the dispatch adapter repeats claim-phase authorization and only a newly acknowledged dispatch-entry compare-and-swap may proceed.
...
- Workflow control runs `SimulationExecutionAuthorizer` before reservation and claim preparation. `SimulationDispatchAdapter` independently repeats claim-phase authorization before dispatch entry. The local executor then checks complete executor-plan correlation without issuing or reinterpreting authority. Denied, erroneous, stale, revoked, consumed, or mismatched input causes no reservation, claim, dispatch entry, or process execution as applicable.
...
Protected execution remains human-owned where policy requires it. Execution authority does not establish scientific acceptance. `WorkflowRun` stores only the externally issued grant/snapshot references, closed authorization results, and append-only reservation/claim/outcome history needed for one dispatch; it never issues or broadens a grant. New simulation manifests may cite a canonical development-decision identity as evidence alongside the existing generic `ScientificExecutionAuthorityReference`, but the decision identity cannot substitute for the grant, snapshot, authorization result, reservation, or claim. A retry requires a new operation, activation, request, attempt, obligation, and grant. This documentation grants no protected execution authority.
