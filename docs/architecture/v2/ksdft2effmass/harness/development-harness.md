...
`HarnessTask` is an immutable requested development work definition. It records scope, preconditions, completion criteria, exclusions, and review requirements but grants no authority. Canonical `tasks/{research,simulation,software}/*.json` records define lifecycle and parent/prerequisite topology; `harness/task-graph.json` is a derived projection. The public `ksdft2effmass.harness` `HarnessTaskRegistry` indexes explicitly supplied Tasks without storing child lists and derives deterministic direct-child, prerequisite, and recursive-descendant queries. `DevelopmentTaskSelection` owns selection only and keeps automatic successor behavior explicit; it is neither authority nor permission. Transitional `harness.pi.local` imports preserve object identity during consumer migration and are not a second implementation.
