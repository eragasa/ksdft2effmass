...
The generation manifest identifies the generation, candidate set and artifact manifest, complete content closure, generating `HarnessStateIdentity`, predecessor generation, lifecycle status, projection/format versions, and exact successful `SynchronizationResult` identity. That immutable result binds the candidate and predecessor, candidate and staged validation results, affirmative synchronization authorization and authority context, publication policy/context, verified target preconditions, operation and attempt, resulting generation, pointer observation, and reconciliation outcome. Lifecycle is closed: `staging` is represented only in synchronization/recovery state and is unreadable; `closed` is immutable and reader-eligible; `quarantined` and `corrupt` are fail-closed recovery dispositions. The pointer carries pointer identity/revision, target identity, exact closed generation and manifest/content identities, predecessor pointer identity, and pointer format version. Projection generations, synchronization results, and publication recovery records remain derived publication state and do not replace or enter `HarnessState` persistence. The generation directory or recovery marker retains the referenced result; no separate audit repository is introduced.

Failure before pointer replacement leaves the old generation current and writes a `HarnessProjectionRecoveryRecord` marker or quarantines the orphan/incomplete/corrupt candidate under explicit policy. That record identifies the observed phase/state, failure, candidate/generation identities when known, marker/quarantine location, and exact failed or indeterminate `SynchronizationResult`, but grants no repair or deletion authority. Retry uses new operation, attempt, and result identities and never overwrites prior results. Successful pointer replacement makes the complete new generation current. An ambiguous commit is reconciled by rereading the pointer and checking the exact generation identity. Rollback is a separately represented pointer switch to a retained validated generation, never multi-file restoration.
