...
| [ABINIT 10.8.3 local installation](abinit-10.8.3-installation.md) | Retained source, dependency, build, executable-identity, and limitation record | Installation completed without a scientific input; the ABINIT test suites and tutorial executions remain separately protected |
| [ABINIT basic tutorials and QE correspondence](abinit-tutorial-correspondence.md) | Bounded mapping of ABINIT basic1--basic4 to existing QE tutorial workflows | ABINIT 10.8.3 is installed, but the campaign remains inactive and every run requires separate protected-execution authorization |
