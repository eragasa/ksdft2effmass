...
| `abinit.tutorials.basic2-h2-convergence` | `h2-convergence/abinit` | H2 cutoff and supercell convergence; LDA/GGA comparison | No same-material QE Task; the separate silicon cutoff and k-point-density Tasks are methodological context only | The material, finite-cell error, pseudopotential, observables, and XC comparison differ. |
| `abinit.tutorials.basic3-silicon` | `silicon-scf/abinit`, `silicon-wavefunction-cutoff-convergence/abinit`, `silicon-k-point-density-convergence/abinit`, `silicon-structure-optimization/abinit`, `silicon-bands/abinit` | Silicon total energy, k-point convergence, lattice parameter, Kohn--Sham bands | `quantumespresso.simulations.pranab_das.scf-silicon`, `quantumespresso.simulations.pranab_das.convergence-silicon-cutoff`, `quantumespresso.simulations.pranab_das.convergence-silicon-kpoint-density`, `quantumespresso.simulations.pranab_das.structure-optimization-silicon`, `quantumespresso.simulations.pranab_das.bands-silicon` | Strong workflow correspondence. It does not imply equal inputs or numerically comparable results. |
| `abinit.tutorials.basic4-aluminum` | `aluminum-metal/abinit`, `aluminum-smearing-convergence/abinit`, `aluminum-surface/abinit` | Metallic aluminum, k-point/smearing convergence, lattice parameter, surface slabs | `quantumespresso.simulations.pranab_das.aluminum-metal`, `quantumespresso.simulations.pranab_das.smearing-convergence-aluminum` | Strong bulk-metal workflow correspondence. The paired QE surface directory begins as `planned` because no campaign Task currently owns it. |
