...
Every executable candidate is represented by paired JSON and Markdown Task records
whose identifier begins with `quantumespresso.simulations.pranab_das.`. The campaign
record is `quantumespresso.simulations.pranab_das`. A page containing materially distinct
