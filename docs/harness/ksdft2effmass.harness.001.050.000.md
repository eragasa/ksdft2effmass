...
project-role slices used only durable harness writers and a durable reviewer
under explicit ownership. The current architecture-agent
slice assigns those same three durable harness capabilities under a new explicit
...
|---|---|---|---|---|---|---|---|
| [ksdft2effmass-architecture](../../.pi/agents/ksdft2effmass-architecture.md) | project | architecture | read-only | Optional architecture decision support for material project boundaries and cross-surface design | Live: generalized optional durable record, available only for an explicitly authorized genuine architecture decision, boundary conflict, or material cross-surface design question. Historical: the same identity retains closed OperatorRecord, backend-neutral CPN, Rust/control-plane, and H0 references. | durable | ksdft2effmass-architecture |
...
| [ksdft2effmass-harness-documentation](../../.pi/agents/ksdft2effmass-harness-documentation.md) | cross-domain | documentation | writer | Durable harness documentation capability | Live: assigned the maintained inventory and project-role pages in the current architecture-agent slice. Historical: used under explicit ownership for the four earlier project-role slices. | durable | ksdft2effmass-harness-documentation |
...
| [ksdft2effmass-harness-implementation](../../.pi/agents/ksdft2effmass-harness-implementation.md) | cross-domain | implementation | writer | Durable generic and project-local harness implementation capability | Live: assigned the project architecture-agent record and any required capability registration in the current slice. Historical: used under explicit ownership for the four earlier project-role slices. | durable | ksdft2effmass-harness-implementation |
| [ksdft2effmass-harness-integration-reviewer](../../.pi/agents/ksdft2effmass-harness-integration-reviewer.md) | cross-domain | integration-review | read-only | Durable final harness integration-review capability | Live: assigned the final read-only cross-surface review for the current architecture-agent slice. Historical: performed the independent reviews of the four earlier project-role slices. | durable | ksdft2effmass-harness-integration-reviewer |
...
interfaces. The architecture role is now an optional read-only specialist used
only for an explicitly authorized genuine architecture decision, boundary
conflict, or material cross-surface design question. Each role retains its broad
responsibilities and authority boundaries. Subject-specific skills are supplied
by an authorized task only when routing supports and requires them; routing does
not expand authority. No record bypasses task selection, path ownership,
checkpoint, review, or human-authority controls.
...
non-overlapping ownership for the current architecture-agent slice; the four
earlier project-role slices used the same capability trio. The harness tests and
architecture roles remain unassigned. These uses do not retroactively replace historical agents; discovery
...
- The five unprefixed records—`ksdft2effmass-implementation`,
  ...
  the stable current project roles. All five have been simplified to durable
  capability and boundary definitions; all remain subject to task-specific
  authorization.
- Five stable harness capability roles exist. The current architecture-agent
  slice assigns three under explicit ownership without altering their durable
  identities; the harness test and architecture roles remain unassigned. The
  other 24 records whose names start with `ksdft2effmass-harness-` remain bound
  to H2, H3, H4,
  H2-HC01,
