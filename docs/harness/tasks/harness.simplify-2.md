...
- R2.7 — Compose evidence, resource, Task, wire, and other validation results, extract one private control-generation compiler shared by the existing public migrator and verifier, compare a complete candidate artifact set with maintained state without publication, retire replay and duplicated cross-domain gates, and return one structured maintained validation result.
...
- At R2.7, establish the prospective boundary `authoritative repository inputs → one private control-generation compiler → complete candidate SQLite and projection artifact set`. The private compiler owns construction mechanics once; `HarnessControlMigrator` consumes it to compile, validate the candidate, and publish, while `HarnessControlVerifier` consumes it to compile and compare the candidate with maintained SQLite, deterministic SQL export, and projections, report without publication, and remove ordinary temporary candidate artifacts.
- The migrator remains the sole publisher, the verifier performs no maintained writes, source authority versus every maintained representation is checked, database-versus-SQL agreement alone is insufficient because both may be stale, and no second construction algorithm or public construction Action is introduced. R2.3 through R2.6 continue using the existing migrator until R2.7 performs this final shared-compiler refactor.
...
- The active parent already authorizes R2.3 through R2.7. After a child completes, the parent agent may explicitly transition to the next prerequisite-satisfied child without a separate human activation decision when no unresolved checkpoint, materially different human-owned architecture or public-contract choice, protected action, or unresolved material review finding exists. This corrected parent rule supersedes the earlier separate-child-activation language in `harness/intake/harness.simplify-2.md` for R2.3 through R2.7. This is controlled serial progression, not background or uncontrolled activation; `automatic_successor_activation` remains false.
...
- The R2.7 private compiler is the only owner of candidate control-generation mechanics; the migrator is the sole publisher, and the nonmutating verifier compares authoritative inputs with maintained SQLite, SQL export, and projections and removes ordinary temporary candidate artifacts.
