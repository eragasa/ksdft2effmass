...
`inactive`: corrected inactive R2.6 final CLI placement and full-control-synchronization command work package; parent-authorized deterministic transition only, currently not activated, and no automatic successor activation

## Objective

Consolidate maintained live harness commands under `python/src/cli/` and expose one explicit synchronization surface that invokes the existing full `HarnessControlMigrator`, while keeping reusable behavior with its owning ActionObjects and preserving historical command evidence.
...
- Expose maintained behavior equivalent to `harness control sync` and `harness control check` without freezing those exact command spellings during planning. The synchronization command accepts explicit inputs and invokes the existing full deterministic `HarnessControlMigrator`; it does not own database mechanics.
- Define synchronization as an explicit post-edit operation: the authorized agent or human invokes it after an authorized repository edit. Filesystem timing, watcher state, current-directory discovery, and ambient events provide no authority.
- Preserve the full migration flow through staged complete SQLite reconstruction, schema, relationship, and projection validation, and failure-safe publication of SQLite and generated projections; do not add incremental SQL updates or another writing path.
...
- Use the existing `HarnessControlMigrator` throughout R2.6 and route the maintained check surface to the existing public `HarnessControlVerifier`; neither Action currently has nor is assumed to have a migrator check mode. Private compiler extraction and full source-aware verifier integration belong only to R2.7.
- Implement R2.6 as a complete vertical replacement of its owned CLI subsystem: accepted end-state contract, isolated implementation, complete affected-command migration, controlled parity, one cutover, and removal of obsolete live wrappers. Do not retain old and new operational CLI authorities after cutover except for temporary compatibility required by an accepted public contract.
- Because the active parent authorizes the full round, R2.6 requires no separate human activation cycle once R2.5 is complete and the parent agent explicitly transitions to it with no unresolved checkpoint, human-owned material choice, protected action, or unresolved material review finding. Keep one active child, prohibit background activation, and keep automatic successor activation false.
...
- One maintained synchronization command delegates complete database construction and publication to `HarnessControlMigrator`, and one maintained check surface reports deterministic control agreement; exact user-facing spellings are selected during implementation without creating another persistence owner.
...
- Focused CLI and command/API agreement tests establish explicit arguments, full-migration delegation, output, exit status, nonmutation or authorized-publication boundaries, documentation and resource agreement, and dependency-lock nonmutation.
- The work package completes without self-activating R2.7 or another successor. A prerequisite-satisfied R2.7 transition is performed explicitly by the parent agent under the parent-authorized deterministic rule, not by an automatic successor mechanism.
...
- Do not introduce row-level SQL mutation, partial projection dependency tracking, another database writer, another persistence framework, a filesystem watcher or daemon, event-log authority, or ambient repository discovery.
