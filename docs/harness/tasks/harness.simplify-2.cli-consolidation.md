...
- Produce a durable R2.7 handoff listing the final CLI inventory, retired paths, retained historical scripts, command/API agreement evidence, and unresolved compatibility risks.
- Make the synchronization command supply the canonical maintained evidence, resource, Task, and graph inputs to the existing `HarnessControlMigrator`; route the maintained check surface to the existing public `HarnessControlVerifier`, and introduce no other control-construction or publication path.
