...
- Propose a bounded ten-slice migration, governed action execution, an immutable SQLite projection lifecycle, extension boundaries, and effectiveness criteria using the completed QE tutorial as the primary acceptance scenario.
...
- Current authority and proposed target state are distinct; current responsibilities and migration slices are mapped; extension, SQLite, governed-execution, and scientific-fast-path boundaries are explicit.
...
- Do not implement Architecture v2, refactor source or tests, change SQLite schemas or lifecycle, migrate control state, change CLIs, add Pi extension code, modify agents or skills, implement telemetry, integrate CPN behavior, or add dependencies.
