...
In the special case of an unconstrained finite Fourier class, direct
least-squares fitting and truncation of the complete localized operator are the
same orthogonal projection only when they use the same complete uniform mesh,
equal weights, aligned frame, Fourier convention, and retained model class.
Nonuniform weights, restricted fitting domains, parameter-sharing constraints,
or different gauges define different reduction problems and need not produce
the same coefficients. Appendix~\ref{app:one-dimensional-reduction} verifies
the equality under matched conditions and measures its loss when the objective
is changed one condition at a time.
...
two paths. Similar \emph{route defects} compare direct and Wannier-mediated bulk
reductions and test gauge equivariance. A route defect is interpretable only
when its comparison domain, weights, model class, gauge, and normalization are
