\chapter{Structured Learning as a Model-Class Diagnostic}
\index{structured learning|textbf}

\label{ch:structured-learning}

\chapterstatus{proposed learning framework. No trained model, learned impurity
...
\citationtodo{low priority}{Check the statement that Kohn--Sham DFT supplies the
parent one-particle operator against the primary Kohn--Sham paper, bibliography
key \texttt{kohn1965} (already present).}
...
\index{learning!parameter prediction}
...
\index{learning!continuum crossover}
...
\citationtodo{high priority}{Check the screened-Coulomb donor asymptote and its
silicon multivalley context against Kohn--Luttinger donor theory (1955),
provisional key \texttt{kohnLuttinger1955donor}; the candidate record is present
...
\index{learning!joint identification and reduction}
...
\citationtodo{medium priority}{Check the named Slater--Koster parameterization
against the primary source, bibliography key \texttt{slater1954} (already
present).}
