\chapter{Two Finite Harmonic-Oscillator Constructions: A Frobenius Comparison}
\index{harmonic oscillator!finite constructions|textbf}

\label{app:harmonic-oscillator}

\chapterstatus{calculated illustrative numerical experiment. The retained
sweep, comparison maps, verifier, report, and graphics are under
\texttt{calculations/research-monograph/harmonic-oscillator/}. The calculation
compares a ladder-operator representation with a harmonic-oscillator potential
enclosed by a finite Dirichlet box after both have been placed in the same
retained ladder coordinates. The results provide numerical verification for
the declared finite construction, not semiconductor validation.}
...
reference operator for the retained Frobenius comparison.
...
\citationtodo{medium priority}{Check the exact one-dimensional harmonic-
oscillator ladder representation, spectrum, oscillator length, and retained
finite-ladder qualification against a standard quantum-mechanics source.
Candidate: Shankar (1994), bibliography key \texttt{shankar1994} (already
present).}

\section{Harmonic potential inside a Dirichlet box}
\index{Dirichlet boundary}
...
\citationtodo{low priority}{Check the inverse-square-root symmetric
orthonormalization terminology and conclusion against Higham (1986),
bibliography key \texttt{higham1986} (already present).}
...
the declared ladder-state identification inaccurate or ill-conditioned. An
...
\end{equation}
This is the principal retained output of the experiment. Its interpretation is
...
\index{discretization!comparison}
...
\end{equation}
for a uniform grid. For fixed $K$, the controlled limiting question is whether
\begin{equation}
    \delta_F(X,h;K)
    \longrightarrow
    0
\end{equation}
when the box expands and the spatial representation is refined. The retained
sweep uses $b=4,6,8$, $\eta=0.2,0.1,0.05$, and $K=2,4,6$. At fixed $b=8$, the
final observed spatial-refinement orders are $1.99875$, $1.99792$, and
$1.99696$ for $K=2$, $4$, and $6$, respectively. This supports the expected
second-order behavior for the declared fixed-box sequence; it does not
establish uniform convergence in $K$.
...
\section{Calculated graphics and retained result}

The retained \texttt{convergence-summary.png} reports the relative Frobenius
discrepancy under box expansion and fixed-box spatial refinement, the diagonal
and off-diagonal discrepancy components, and the sampled-state Gram deviation.
The retained \texttt{operator-difference-heatmap.png} displays
\begin{equation*}
    \frac{\mathbf A_{X,h}^{(K)}-
    \mathbf H_{\mathrm{lad}}^{(K)}}{\hbar\omega}
\end{equation*}
for $b=8$, $\eta=0.05$, and $K=6$. Exact comparison-map reconstruction
contracts and content identities, compact common-coordinate operators,
cross-grid comparisons, and diagnostics are in \texttt{result.json}; the
figures are visualizations of those retained values, not independent evidence.

At $\eta=0.05$, expansion from $b=4$ to $b=6$ substantially reduces the total
discrepancy for $K=4$ and $K=6$. The $K=2$ total discrepancy is instead
slightly smaller at $b=4$ than at $b=6$ or $b=8$. This does not establish that
the narrower box is independently more accurate: boundary-domain and
spatial-discretization contributions can partially cancel in the total matrix
difference. A continuous two-parameter surface beyond the retained grid remains
proposed work and is not represented as a calculated result.

\section{Interpretation boundary}

The calculated experiment tests represented finite-box harmonic oscillators
against the exact retained ladder operator under an explicit state-space map.
It keeps boundary-domain effects, spatial-representation error, retained-space
sensitivity, and map conditioning distinct. The Frobenius norm is applied only
after the two operators have been placed in the same orthonormal ladder
coordinates, as described in
Appendix~\ref{app:hilbert-schmidt-and-frobenius}.

The retained verifier establishes numerical agreement with the declared finite
mathematics and independent analytic oscillator states under binary64
tolerances. The observed refinement behavior is numerical verification for
this illustrative comparison under the declared conditions. It does not
