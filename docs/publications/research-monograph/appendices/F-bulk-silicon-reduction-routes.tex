\chapter{Direct and Wannier-Mediated Tight-Binding Reduction in Silicon}
\index{reduction route!bulk comparison|textbf}

\label{app:bulk-reduction-routes}

\chapterstatus{mathematical framework and proposed numerical experiment. No
...
this appendix. The physical and numerical parent remain governed by the applicable
...
This appendix first states the mathematical issues that determine which
...
windows are not fixed by this appendix. They must be inherited from the accepted
...
\citationtodo{medium priority}{Check the separation among disentanglement,
gauge localization, Fourier interpolation, and real-space truncation against
Souza--Marzari--Vanderbilt (2001) and Marzari et al. (2012), bibliography keys
\texttt{souza2001} and \texttt{marzari2012} (already present).}
...
\citationtodo{medium priority}{Check the declared orthogonal
$sp^3s^{\ast}$ silicon hierarchy, Slater--Koster structure, and neighbor-shell
restrictions against Slater--Koster (1954) and Vogl et al. (1983), bibliography
keys \texttt{slater1954} and \texttt{vogl1983} (already present).}
...
\index{Frobenius norm!bulk residual}
...
\index{path dependence!bulk reduction}
...
\end{enumerate}
No scalar total error is defined by this appendix. Combining these components would
...
No production parent is asserted to exist by this appendix. The following stages
