...
first follow-on is now a human-accepted bounded synthetic result; the sequence
remains:
\begin{enumerate}
...
\end{enumerate}
The first item is the accepted bounded synthetic result reported above. Items
two through five remain proposed follow-on experiments rather than extensions of
the accepted result, and none is an active material calculation. Keeping the
alignment inverse problem separate preserves attribution: failure to infer the
map is not confused with failure of subtraction after a correct map is supplied.
