...
\chapterstatus{synthesis with one human-accepted calculated synthetic
numerical-verification result and a proposed subsequent research program. The
controlled spin-space benchmark is reported below within its bounded finite-model
scope. The appendix does not claim an accepted production Kohn--Sham parent,
Wannier operator, aligned material impurity operator, fitted lattice or
continuum model,
crossover radius, calculated material binding energy, or scientific validation
result.}
...
The first item has now been completed as the bounded calculated synthetic
verification above; every later item remains proposed work.

\section{Remaining warmup plot placeholders}
