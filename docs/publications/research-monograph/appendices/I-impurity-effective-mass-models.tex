...
\subsubsection{Proposed one-dimensional follow-on sequence}

The accepted known-map exercise remains the fixed ground-truth baseline. Its
follow-on experiments should proceed in the following order:
\begin{enumerate}
...
\end{enumerate}
These are proposed follow-on experiments, not extensions of the accepted result
and not active material calculations. Keeping the alignment inverse problem
separate preserves attribution: failure to infer the map is not confused with
failure of subtraction after a correct map is supplied.
