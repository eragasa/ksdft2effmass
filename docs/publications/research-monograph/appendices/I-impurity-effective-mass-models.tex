\chapter[Controlled Impurity-Extraction Benchmarks]{Controlled Impurity-Extraction Benchmarks: Spin Spaces and One-Dimensional Defects}
\index{impurity operator!model classes|textbf}
\index{warmup!impurity extraction|textbf}

\label{app:impurity-model-classes}

\chapterstatus{synthesis with six human-accepted calculated synthetic
numerical-verification results and a human-accepted bounded primary-literature
comparison. The spin-space and one-dimensional impurity calculations remain
within bounded finite-model scopes; the proposed two-dimensional and material
program is separated into Appendix~\ref{app:two-dimensional-defect-extraction}.
This appendix does not claim an accepted production Kohn--Sham parent, Wannier
...
The relevant literature separates limits that cannot be merged into one
``continuum'' sequence. Discrete-to-continuum convergence sends a numerical or
physical lattice spacing to zero for a fixed limiting operator and can be posed
in norm-resolvent form~\cite{nakamuraTadano2021}. Band-edge homogenization
instead sends a weak, slowly varying defect scale to zero and derives an
effective-mass envelope from the curvature and Bloch state at a spectral
edge~\cite{hoeferWeinstein2011,ducheneVukicevicWeinstein2015}. Finite-volume
refinement enlarges the periodic domain at fixed local physics; exponential
image effects are known in finite-volume bound-state settings, although their
specific form must be re-established for the represented lattice problem rather
than imported unchanged~\cite{koenigLeeHammer2011}. Finally, changing defect
width or normalization changes the physical profile unless a scaling family is
specified. Semiconductor donor theory adds a distinct central-cell and
multivalley boundary: long-wavelength effective-mass behavior does not by itself
resolve the atomically sharp core~\cite{kohnLuttinger1955donor,gamble2015}.
Consequently, continuum-grid spacing, supercell size, band-edge scale, and
defect-profile parameters must be refined independently. Agreement along a
path that changes several of them simultaneously is not a continuum-limit
result.
...
\section{Controlled benchmark program}

The impurity-reduction chain is exercised first on controlled models in which
the relevant state-space maps and defect operators are known. The spin-space
and one-dimensional exercises below are human accepted. Their purpose is to
verify the mathematical construction and separate failure modes before a
first-principles silicon impurity operator is interpreted.

The two completed headline warmups compare spinless and spinful state spaces
and extract planted operators from matched one-dimensional pristine and defect
supercells. The corresponding two-dimensional extension is specified
separately in Appendix~\ref{app:two-dimensional-defect-extraction}. Each stage
retains applicable folding, null, planted-defect, alignment, finite-size,
model-class, and observable controls.
...
Use the one-dimensional periodic parent developed in
Appendix~\ref{app:one-dimensional-reduction}. Let a supercell
...
\subsubsection{Calculated finite-rank analytical oracle}

A third synthetic follow-on supplies a spectral oracle that does not diagonalize
the full defect Hamiltonian. For the accepted two-orbital parent and local
rank-one defect $V_g=-g|v\rangle\langle v|$, the matrix determinant lemma gives
\begin{equation}
  1-gG_N(E)=0,
  \qquad
  G_N(E)=\frac{1}{N}\sum_j
  u^\dagger[H(k_j)-EI]^{-1}u .
\end{equation}
The oracle evaluates only the primitive $2\times2$ Bloch-fiber inverses and
resolves the unique root below the finite-host lower edge. This use of a host
resolvent for a Wannier-basis impurity follows the classical Koster--Slater
construction~\cite{kosterSlater1954impurity}; the one-dimensional weak-binding
and periodic-defect literature motivates the threshold and sign controls but
does not supply the numerical result reported here~\cite{simon1976weak,parzygnat2010}.
An independent route assembles and diagonalizes the full site-space defect
operator. Inputs, source
identities, root rules, retained results, an independent verifier, and the
mini-paper are under
\path{calculations/research-monograph/impurity-defect-1d-analytical-oracle/}.

For $N=16,24,32,48,64$ and attractive magnitudes
$g/E_G=0.02,0.08,0.20,0.50$, all 20 controls have exactly one state below the
finite-host edge. The maximum oracle/numerical energy discrepancy is
$6.11\times10^{-16}E_G$, the maximum secular residual is
$8.16\times10^{-13}$, and the maximum rank-one projector defect is
$6.14\times10^{-13}$. A zero-coupling threshold and a repulsive control have no
state below the lower edge. A spin-degenerate rank-two lift agrees by projector
to $1.55\times10^{-14}$, while a proposed rank-one comparison against that
rank-two sector stops explicitly.

Figure~\ref{fig:impurity-defect-1d-analytical-oracle} retains the energy,
finite-size, projector, threshold, no-state, and rank diagnostics separately.
The supercell sequence is not an infinite-system or continuum extrapolation.
This is a human-accepted bounded synthetic software and numerical-verification
result, not material validation.
...
\begin{figure}[p]
  ...
  \includegraphics[
    ...
    height=0.78\textheight,
    keepaspectratio
  ]{../../../calculations/research-monograph/impurity-defect-1d-analytical-oracle/summary.png}
  \caption{Finite-rank resolvent oracle. Panel (a) compares secular roots with
  independent site-space eigenvalues. Panel (b) reports finite-size binding
  without a continuum claim. Panel (c) compares equal-rank bound-state
  projectors. Panel (d) retains threshold, repulsive no-lower-bound-state,
  spin-degenerate rank-two, and unequal-rank stopping controls.}
  \label{fig:impurity-defect-1d-analytical-oracle}
\end{figure}

\subsubsection{Calculated continuum refinement}

The earlier smoothness scan changed Gaussian width on one band-limited grid and
therefore did not identify a continuum limit. The fourth synthetic follow-on
separates continuum-mesh, continuum-domain, lattice-supercell, represented
lattice-scale, and profile-width axes. For lattice spacing $a$, the fixed-profile
scaled parent is
\begin{equation}
  E_a(q)=E_0+\frac{E_{\mathrm{lat}}(aq)-E_0}{a^2},
\end{equation}
which preserves the band-edge curvature while changing the represented lattice
scale. Inputs, criteria, source identities, retained results, and an independent
site-space verifier are under
\path{calculations/research-monograph/impurity-defect-1d-continuum-refinement/}.

The continuum-mesh, continuum-domain, and lattice-supercell support axes pass
their frozen criteria. At fixed integrated profile with
$\sigma=2a_{\mathrm{ref}}$, the unrefined lattice fails five of six comparison
criteria. From $a=0.5a_{\mathrm{ref}}$ through the finest tested spacing
$a=0.125a_{\mathrm{ref}}$, all binding, bound-count, projector, compressed
operator, cross-coupling, and Brillouin-edge criteria pass persistently. At the
finest spacing the relative binding error is $4.67\times10^{-5}$ and the
projector defect is $2.69\times10^{-4}$.

Profile broadening at the original lattice spacing gives a different result.
For the fixed-integrated family at $\sigma=12a_{\mathrm{ref}}$, the relative
binding error is $8.63\times10^{-5}$ and the projector defect is
$1.21\times10^{-3}$, but the compressed parent-dispersion residual remains
$3.78\times10^{-3}E_G$, above the frozen $10^{-3}E_G$ rule. The fixed-peak
family additionally retains unequal below-edge counts, 41 versus 37, at that
width. Neither family therefore has a profile-defined crossover over the tested
width domain.

Figure~\ref{fig:impurity-defect-1d-continuum-refinement} distinguishes the
successful bounded lattice-scale sequence from the negative width-crossover
result. This is a human-accepted bounded synthetic numerical-verification
result. It is not an asymptotic theorem, material validation, transferability,
...
\begin{figure}[p]
  ...
  \includegraphics[
    ...
    height=0.78\textheight,
    keepaspectratio
  ]{../../../calculations/research-monograph/impurity-defect-1d-continuum-refinement/summary.png}
  \caption{Separated continuum refinement. Panel (a) retains independent mesh,
  domain, and lattice-supercell controls. Panel (b) shows the fixed-profile
  lattice-scale metrics relative to their frozen tolerances. Panel (c) shows
  that profile broadening improves spectral errors but cannot remove the fixed
  parent-dispersion residual. Panel (d) records the bounded scale pass and the
  absence of a profile-defined crossover.}
  \label{fig:impurity-defect-1d-continuum-refinement}
\end{figure}

\subsubsection{Literature position of the one-dimensional chain}

The finite-rank and continuum ingredients are established separately in the
literature. Host-resolvent impurity reasoning is classical in the
Koster--Slater construction~\cite{kosterSlater1954impurity}; the retained
review uses that source only at abstract level and does not attribute the
benchmark's exact finite-matrix formula to unseen text. Weak-coupling existence
results for continuum Schrödinger operators and periodic gaps do not supply the
finite-supercell oracle reported here~\cite{simon1976weak,parzygnat2010}.
Likewise, band-edge homogenization varies a weak, slowly varying defect, whereas
discrete-to-continuum convergence varies an identified lattice spacing and
finite-volume analysis varies the periodic domain~\cite{hoeferWeinstein2011,ducheneVukicevicWeinstein2015,nakamuraTadano2021,koenigLeeHammer2011}.
These distinct hypotheses motivate the separated axes; none proves the bounded
crossover observed in the synthetic sequence.

Localized-basis defect operators also have substantial precedent.
Berlijn, Volja, and Ku require normal-cell and impurity-supercell Hamiltonians
to share a Wannier basis before impurity contributions are embedded in larger
disordered configurations~\cite{berlijnVoljaKu2011}. Lu and co-workers form and
Wannier-interpolate electron--defect matrix elements from an aligned
Kohn--Sham potential difference~\cite{luParkZhouBernardi2020}, while
Corsetti and Mostofi use localized Wannier onsite elements as one route to the
scalar potential alignment needed in silicon-vacancy supercells and analyze
system-size effects separately~\cite{corsettiMostofi2011}. These works establish
localized defect differences, interpolation, energy alignment, and image-error
precedents; they do not establish the hidden-map or model-class diagnostics of
this exercise.

The nearest alignment precedent is the stitching analysis of Lihm and Park,
which treats orbital pairing, spin orientation, phase, potential reference, and
iterative Hamiltonian matching between independently generated MLWF
models~\cite{lihmPark2019}. A 2026 preprint by Shi and co-workers provides close
contemporary evidence: its clean and defect Wannier models use Hungarian
assignment, a dense unitary Procrustes rotation, an explicit nonorthogonal
cross-metric, and residual boundary checks before defect-patch embedding in a
superconducting BdG calculation~\cite{shiXieZhangChuGong2026}. The preprint has
a different physical objective and does not supply a uniqueness theorem,
undercomplete-sector analysis, or the structured stops reported here. Thus the
blind-alignment result is not positioned as the invention of Wannier gauge
matching.

A second 2026 preprint uses an onsite-only Wannier substitution as a rapid
defect-screening class and reports limitations from omitted hopping, charge
redistribution, and spin effects~\cite{kangMuechler2026}. That objective is
closer to one member of the benchmark hierarchy than to extraction of the full
aligned operator. Classical and modern silicon-donor work further shows that
multivalley and central-cell structure must be retained before a material
effective-mass claim is made~\cite{kohnLuttinger1955donor,gamble2015}.

The defensible contribution is therefore methodological integration: one
reproducible, fail-closed synthetic chain retains compatibility, hidden-map
identifiability, operator-model residuals, route commutativity, a finite-rank
oracle, finite-volume diagnostics, and separated continuum axes. It is not a
new impurity-resolvent, alignment, localization, or continuum theorem and is
not material validation. The bounded search protocol, source-access register,
claim matrix, and novelty disposition are retained under
\path{docs/research/literature-reviews/impurity-defect-1d/}. The search is
exhaustive enough only relative to its declared concepts, interfaces, dates,
and stopping rule; it does not establish priority or comprehensive coverage.
...
\end{enumerate}
The first four items are the human-accepted blind-alignment,
independent-route, analytical-oracle, and continuum-refinement results reported
above. The fifth is the bounded literature synthesis just reported; it adds no
numerical or material evidence. None is an active material calculation.
Keeping the alignment inverse problem separate preserves attribution: failure
to infer the map is not confused with failure of subtraction after a correct
map is supplied.

\section{Benchmark conclusion and two-dimensional handoff}

The accepted spin-space and one-dimensional calculations establish the bounded
synthetic prerequisites for extending aligned impurity extraction to two
spatial dimensions. They verify spin embeddings, known-map and blind
alignment, independent extraction routes, finite-rank spectral oracles,
finite-volume diagnostics, model-class separation, and independently refined
continuum comparisons under their declared contracts. They do not establish a
two-dimensional defect result or transferability to a material calculation.

The proposed two-dimensional defect controls and subsequent material-transfer
sequence are separated into
Appendix~\ref{app:two-dimensional-defect-extraction}. That appendix consumes
the accepted periodic parent from
Appendix~\ref{app:two-dimensional-reduction} and the spin and extraction
contracts established here; it does not require these completed benchmarks to
be repeated.
