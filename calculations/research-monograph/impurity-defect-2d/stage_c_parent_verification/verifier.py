"""Independent reconstruction and verification of Stage C retained records."""
...
class IndependentStageCParentVerifier:
    ...
    def execute_accepted(
        ...
        bindings = tuple(
            self._json.mapping(value, "artifact binding") for value in artifact_values
        )
        roles = tuple(
        ...
        for binding in bindings:
            ...
            paths[role] = path
            digests[role] = digest
        if paths["verifier"] != VERIFIER_PATH.resolve(strict=True):
        ...
        attempt = self._json.mapping(authorization["attempt_policy"], "attempt policy")
        if attempt != {
            "maximum_attempts": 1,
        ...
        _, parents = self._adapter_parents({"sources": cast(JsonValue, sources)})
        provenance = self._json.mapping(result["provenance"], "provenance")
