...
LEGACY_RUNNER_SHA256 = (
    "97000e01a86f87dba0bebe515b2e8befe30904a0c921de72a126f34ea28b2d58"
)
...
class SpinSpaceResultVerifier:
    ...
    def execute(self, result_path: Path, repository_root: Path) -> None:
        ...
        if implementation_path_value is None and implementation_sha256_value is None:
            ...
            if self._sha256(script_path) != recorded_script_sha256:
                raise ValueError("runner identity mismatch")
            implementation_path = repository_root / self._string(
                implementation_path_value, "implementation_path"
            )
            if self._sha256(implementation_path) != self._string(
                implementation_sha256_value, "implementation_sha256"
            ):
