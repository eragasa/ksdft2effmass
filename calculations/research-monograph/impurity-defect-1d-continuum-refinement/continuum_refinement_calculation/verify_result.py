...
class ContinuumRefinementVerificationWorkflow:
    """Load sources and independently verify one retained result."""
    ...
    def execute(self, result_path: Path) -> None:
        ...
        self._verify_implementation_identities(repository_root, provenance)
    ...
    def _verify_implementation_identities(
        repository_root: Path, provenance: dict[str, JsonValue]
    ) -> None:
        identities_value = provenance.get("implementation_identities")
        if identities_value is None:
            return
        identities = WireReader.sequence(identities_value, "implementations")
        if len(identities) != 1:
            raise ValueError("one implementation identity is required")
        identity = WireReader.mapping(identities[0], "implementation")
        path = WireReader.string(identity["path"], "implementation path")
        expected = WireReader.string(identity["sha256"], "implementation sha256")
        observed = hashlib.sha256((repository_root / path).read_bytes()).hexdigest()
