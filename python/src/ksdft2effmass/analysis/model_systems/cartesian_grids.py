"""Immutable unit-aware uniform Cartesian grids for model-system analyses."""
...
class UniformCartesianGrid1D:
    ...
    requested_spacing
        Positive spacing compatible with the bounds. The domain length must be an
        integer multiple of this spacing to absolute tolerance ``1e-12``.
    """
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in values[1:]:
            if not MODEL_SYSTEM_UNIT_CONVERTER.compatible(
                value.unit, self.lower_bound.unit
            ):
                raise ValueError(f"{name} has incompatible coordinate dimensions")
    ...
    @property
    def coordinate_unit(self) -> PhysicalUnit | Unitless:
    ...
    def upper_magnitude(self) -> float:
        ...
        return MODEL_SYSTEM_UNIT_CONVERTER.convert_scalar(
            self.upper_bound, self.coordinate_unit
        ).magnitude

    @property
    def spacing_magnitude(self) -> float:
        """Return the requested spacing in the canonical coordinate unit."""
        return MODEL_SYSTEM_UNIT_CONVERTER.convert_scalar(
            self.requested_spacing, self.coordinate_unit
        ).magnitude
    ...
    def interval_count(self) -> int:
        ...
        return int(
            round(
                (self.upper_magnitude - self.lower_magnitude) / self.spacing_magnitude
            )
        )
    ...
    def spacing(self) -> ScalarQuantity:
        """Return the realized uniform spacing in the canonical unit."""
        magnitude = (self.upper_magnitude - self.lower_magnitude) / self.interval_count
    ...
    def interior_coordinates(self) -> VectorQuantity:
        ...
        return VectorQuantity(self.coordinates().magnitude[1:-1], self.coordinate_unit)


@dataclass(frozen=True, slots=True)
class UniformCartesianGrid2D:
    """Represent a two-axis Cartesian tensor-product grid with ``ij`` indexing.

    Parameters
    ----------
    first_axis
        Ordered first axis; its point index varies before the second axis under
        ``ij`` indexing.
    second_axis
        Ordered compatible second axis.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not MODEL_SYSTEM_UNIT_CONVERTER.compatible(
            self.first_axis.coordinate_unit, self.second_axis.coordinate_unit
        ):
            raise ValueError("Cartesian axes must have compatible coordinate units")

    @property
    def shape(self) -> tuple[int, int]:
        """Return the tensor-product point shape including boundaries."""
        return (self.first_axis.point_count, self.second_axis.point_count)
    ...
    def coordinate_axes(self) -> tuple[VectorQuantity, VectorQuantity]:
        ...
        return (self.first_axis.coordinates(), self.second_axis.coordinates())


@dataclass(frozen=True, slots=True)
class UniformCartesianGrid3D:
    """Represent a three-axis Cartesian tensor-product grid with ``ij`` indexing.

    Parameters
    ----------
    first_axis
        Ordered first compatible Cartesian axis.
    second_axis
        Ordered second compatible Cartesian axis.
    third_axis
        Ordered third compatible Cartesian axis.
    """

    first_axis: UniformCartesianGrid1D
    second_axis: UniformCartesianGrid1D
    third_axis: UniformCartesianGrid1D

    def __post_init__(self) -> None:
        axes = (self.first_axis, self.second_axis, self.third_axis)
        if any(not isinstance(axis, UniformCartesianGrid1D) for axis in axes):
            raise TypeError("every axis must be UniformCartesianGrid1D")
        if any(
            not MODEL_SYSTEM_UNIT_CONVERTER.compatible(
                self.first_axis.coordinate_unit, axis.coordinate_unit
            )
            for axis in axes[1:]
        ):
            raise ValueError("Cartesian axes must have compatible coordinate units")
    ...
    def interior_shape(self) -> tuple[int, int, int]:
        ...
        return (
            self.first_axis.interior_point_count,
            self.second_axis.interior_point_count,
            self.third_axis.interior_point_count,
        )
    ...
    def coordinate_axes(
        self,
    ) -> tuple[VectorQuantity, VectorQuantity, VectorQuantity]:
        """Return the three ordered one-dimensional coordinate axes."""
        return (
            self.first_axis.coordinates(),
            self.second_axis.coordinates(),
            self.third_axis.coordinates(),
        )
