...
class _SkillCapabilityValidationResult:
    """Immutable repository skill-capability inventory outcome."""

    skill_records: int
    filesystem_skills: int
    review_blocks: int
    tool_blocks: int
    errors: tuple[str, ...]


class _SkillCapabilityInventoryValidator:
    """Validate one explicit inventory against one explicit repository root."""

    __slots__ = ()

    def execute(
        self, repository_root: Path, inventory_path: Path
    ) -> _SkillCapabilityValidationResult:
        ...
        if "outside guard evaluation" not in boundary.get("invocation_policy", ""):
            errors.append(
                "CPN boundary must require external invocation outside guards"
            )
        ...
        if unexpected_tokens:
            errors.append(
                f"unexpected token responsibilities: {sorted(unexpected_tokens)}"
            )
        ...
        for record in skill_records:
            ...
            if classification not in CLASSIFICATIONS:
                errors.append(
                    f"{name}: invalid primary classification {classification!r}"
                )
            ...
            for consumer in record.get("current_consumers", []):
                ...
                if not isinstance(consumer, str) or not consumer_path.exists():
                    errors.append(
                        f"{name}: missing concrete consumer path {consumer!r}"
                    )
                    continue
            ...
            for reference in record.get("authoritative_references", []):
                if not isinstance(reference, str) or not (root / reference).exists():
                    errors.append(
                        f"{name}: missing authoritative reference {reference!r}"
                    )
            for field in (
                "trigger_description",
                "authorization_requirements",
        ...
        return _SkillCapabilityValidationResult(
            len(skill_records),
            len(actual_paths),
            len(review_names),
            len(tool_names),
