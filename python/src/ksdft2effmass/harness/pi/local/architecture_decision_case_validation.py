...
class _ArchitectureDecisionCaseValidationResult:
    """Immutable controlled-case validation outcome."""

    fixture_scope: str
    applicable_cases: int
    non_applicable_cases: int
    issues: tuple[str, ...]
...
class _ArchitectureDecisionCaseSetValidator:
    """Validate controlled architecture-decision cases below an explicit root."""

    __slots__ = ()

    def execute(
        self, repository_root: Path
    ) -> _ArchitectureDecisionCaseValidationResult:
        ...
        if set(by_id) != APPLICABLE | set(NON_APPLICABLE):
            fail(
                "controlled case identities differ from the closed expected set", issues
            )
        for case_id in APPLICABLE:
            ...
            if not isinstance(options, dict) or list(options) != ["A", "B", "C"]:
                ...
                fail(
                    f"{case_id}: architectures must be three distinct "
                    "nonempty summaries",
                    issues,
                )
            ...
            if not isinstance(case.get("decision_document_path"), str) or not case.get(
                ...
                fail(
                    f"{case_id}: checkpoint must identify the decision document", issues
                )
        ...
        for case_id, classification in NON_APPLICABLE.items():
            ...
            if (
                ...
                fail(
                    f"{case_id}: non-applicable case must stop without "
                    "options/checkpoint",
                    issues,
                )
        ...
        return _ArchitectureDecisionCaseValidationResult(
            "harness-phase-six-only-controlled-fixture",
            len(APPLICABLE),
            len(NON_APPLICABLE),
            tuple(issues),
        )
