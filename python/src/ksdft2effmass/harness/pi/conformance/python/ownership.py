...
type _OwnershipValue = (
    None | bool | int | float | str | list[_OwnershipValue] | dict[str, _OwnershipValue]
)
type _PythonOwnershipEntry = dict[str, _OwnershipValue]
...
class _PythonOwnershipRule:
    ...
    def _validate_owner_profile(
        mode: _OwnershipValue,
        subject: _OwnershipValue,
        evidence_profile: _OwnershipValue,
    ) -> tuple[str, str] | None:
        """Reject private class ownership and malformed profile identity."""
        if (
            mode == "class_owned"
            and isinstance(subject, str)
            and subject.startswith("_")
        ):
            return (
                "TE.PRIVATE_CLASS_OWNER",
                "class_owned is limited to one public class as the sole system "
                "under test",
            )
    ...
    def _validate_ownership(
        cls, model: PythonTestModuleModel, owner: _PythonOwnershipEntry
        ...
        opening = (
            _OPENINGS.get(evidence_class) if isinstance(evidence_class, str) else None
        )
        expected = (
            f"{opening} of ``{sut}``."
        ...
        first = (
            (model.module_doc or "").splitlines()[0].strip() if model.module_doc else ""
        )
        ...
        if mode not in {"class_owned", "artifact_owned"}:
            ...
            if sut not in model.imported_names:
                findings.append(
                    (
                        "TE.SUT_IMPORT",
                        "supplied SUT must be imported through an explicit public "
                        "import",
                        None,
                    )
                )
    ...
    def execute(
        self, model: PythonTestModuleModel, owner: _PythonOwnershipEntry
    ) -> tuple[tuple[str, str, int | None], ...]:
        """Validate one module against its explicit ownership declaration."""
        return self._validate_ownership(model, owner)
...
class _PythonOwnershipInputLoader:
    ...
    def execute(
        ...
        tuple[_PythonOwnershipEntry, ...],
        dict[str, _PythonOwnershipEntry],
        ...
        by_path: dict[str, _PythonOwnershipEntry] = {}
        seen: set[str] = set()
        for index, item in enumerate(value["modules"]):
            ...
            if mode not in {"class_owned", "artifact_owned"}:
                findings.append(
                    (
                        "TE.OWNERSHIP_MODE",
                        path,
                        f"modules[{index}].mode is invalid",
                        None,
                    )
                )
            ...
            if (
                problem := _PythonOwnershipRule._validate_owner_profile(
                    mode, subject, profile
                )
            ) is not None:
                findings.append(
                    (problem[0], path, f"modules[{index}]: {problem[1]}", None)
                )
