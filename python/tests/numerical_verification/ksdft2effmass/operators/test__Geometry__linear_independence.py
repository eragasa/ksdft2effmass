...
class TestGeometry:
    ...
    def construct_geometry(
        ...
        Acceptance: The public Geometry constructor returns or raises its documented
        error.
        ...
        calculation produced them. The helper establishes no scientific validity,
        scientific
    ...
    def test_constructor__well_conditioned_orthogonal_cell_is__is_enforced() -> None:
        ...
        Requirement: A clearly full-rank orthogonal row cell is admitted without
        warnings.
        ...
        Acceptance: Construction succeeds and preserves the exact selected nested
        floats.
        ...
        with warnings.catch_warnings():
            ...
            geometry = TestGeometry.construct_geometry(cell)
    ...
    def test_constructor__exactly_dependent_cell_is_rejected__is_enforced() -> None:
        ...
        Requirement: Exactly linearly dependent row vectors violate the intrinsic
        invariant.

        Method: Duplicate the first row exactly and construct under warning-as-error.

        Oracle: ``row_2 = row_1`` gives the nontrivial exact relation ``row_2 - row_1 =
        0``,
        so
        :math:`\sigma_{\min}=0`.

        Acceptance: Exactly ``ValueError`` identifies linear independence; no warning
        leaks.

        Interpretation: Passing verifies the exact-dependence rejection baseline.

        Limitations: It does not quantify conditioning of arbitrary matrices or
        establish
        scientific
        validation, UQ, or Rust conformance.
        """
        ...
        with warnings.catch_warnings():
            ...
            with pytest.raises(ValueError, match="linearly independent"):
                TestGeometry.construct_geometry(cell)
    ...
    def test_constructor__skew_independent_cell_is_admitted__is_enforced() -> None:
        ...
        Limitations: No crystallographic or physical realism, scientific validation, UQ,
        or
        Rust
    ...
    @staticmethod
    def test_constructor__left_handed_independent_cell_is_admitted__is_enforced() -> (
        None
    ):
        ...
        Oracle: The diagonal product ``2 * 3 * -4 = -24`` is nonzero and negative,
        proving
        full rank
    ...
    def test_constructor__cell_clearly_below_public_threshold__is_rejected() -> None:
        ...
        Oracle: Positive diagonal singular values are their entries, so the ratio is
        exactly
        half
        ...
        Interpretation: A pass verifies clearly-below-threshold rejection without
        testing
        equality.

        Limitations: This does not select a scientific tolerance, validate a cell,
        perform
        UQ, or
    ...
    def test_constructor__cell_at_strict_public_threshold__is_rejected() -> None:
        ...
        Oracle: The positive diagonal singular values give ratio exactly the stored
        binary64
        ``RTOL`` and the public inequality is strict.

        Acceptance: Construction raises independence ``ValueError`` without warning.

        Interpretation: A pass verifies the inclusive rejection side of the exact
        threshold
        boundary.
    ...
    def test_constructor__cell_clearly_above_public_threshold_is__is_enforced() -> None:
        ...
        Oracle: Positive diagonal singular values are their absolute diagonal entries,
        so
        ...
        Interpretation: Passing verifies above-threshold admission without probing
        equality.

        Limitations: This does not select a scientific tolerance, perform UQ, or
        establish
        scientific
    ...
    def test_field__uniform_scale_invariance_for_well_conditioned_cells__is_exact(
        ...
        Method: Scale the identity cell by both signs of ``1e-200``, ``1``, and
        ``1e200``
        and
        construct each under warning-as-error.

        Oracle: All singular values of :math:`s\mathbf I` are analytically ``abs(s)``,
        so
        :math:`\rho=1` at every tested nonzero signed scale.

        Acceptance: Every construction succeeds, preserves ``scale`` on the diagonal,
        and
        leaks no
        ``RuntimeWarning``.
    ...
    def test_field__uniform_scale_invariance_for_near_dependent_cells__is_exact(
        ...
        Requirement: Uniform nonzero scaling cannot change a clearly invalid relative
        ratio.

        Method: Scale ``diag(1, RTOL / 2, 1)`` by both signs of each representative
        finite
        normal
        ...
        Acceptance: Every construction raises exactly independence ``ValueError`` and
        leaks
        no
        ``RuntimeWarning``.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ((2.0, 0.0, 0.0), (0.5, -1.0, 4.0), (1.0, 3.0, 0.0)),
                id="valid_rows_acb",
            ),
            pytest.param(
                ((1.0, 3.0, 0.0), (2.0, 0.0, 0.0), (0.5, -1.0, 4.0)),
                id="valid_rows_bac",
            ),
            pytest.param(
                ((1.0, 3.0, 0.0), (0.5, -1.0, 4.0), (2.0, 0.0, 0.0)),
                id="valid_rows_bca",
            ),
            pytest.param(
                ((0.5, -1.0, 4.0), (2.0, 0.0, 0.0), (1.0, 3.0, 0.0)),
                id="valid_rows_cab",
            ),
            pytest.param(
                ((0.5, -1.0, 4.0), (1.0, 3.0, 0.0), (2.0, 0.0, 0.0)),
                id="valid_rows_cba",
            ),
        ],
    )
    ...
    def test_constructor__valid_row_permutation__preserves_independence(
        ...
        Oracle: Row permutation preserves the nonzero triangular determinant magnitude
        and
        singular
        values.
        ...
        Interpretation: A pass verifies valid permutation invariance for this
        represented
        cell.
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("error", RuntimeWarning)
            geometry = TestGeometry.construct_geometry(permuted)
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ((1.0, 2.0, 0.0), (1.0, 2.0, 0.0), (0.0, 0.0, 1.0)),
                id="dependent_rows_abc",
            ),
            pytest.param(
                ((1.0, 2.0, 0.0), (0.0, 0.0, 1.0), (1.0, 2.0, 0.0)),
                id="dependent_rows_acb",
            ),
            pytest.param(
                ((1.0, 2.0, 0.0), (1.0, 2.0, 0.0), (0.0, 0.0, 1.0)),
                id="dependent_rows_bac",
            ),
            pytest.param(
                ((1.0, 2.0, 0.0), (0.0, 0.0, 1.0), (1.0, 2.0, 0.0)),
                id="dependent_rows_bca",
            ),
            pytest.param(
                ((0.0, 0.0, 1.0), (1.0, 2.0, 0.0), (1.0, 2.0, 0.0)),
                id="dependent_rows_cab",
            ),
            pytest.param(
                ((0.0, 0.0, 1.0), (1.0, 2.0, 0.0), (1.0, 2.0, 0.0)),
                id="dependent_rows_cba",
            ),
        ],
    )
    @staticmethod
    def test_constructor__dependent_row_permutation__preserves_rejection(
        ...
        Interpretation: A pass verifies invalid permutation invariance for this
        represented
        cell.
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("error", RuntimeWarning)
            with pytest.raises(ValueError, match="linearly independent"):
                TestGeometry.construct_geometry(permuted)
