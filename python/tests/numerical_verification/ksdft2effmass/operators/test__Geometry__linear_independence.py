...
def construct_geometry(
    ...
    Requirement
    Input rows/components pass unchanged; coercion with ``np.asarray``, ``float``, or
    ``tuple`` is intentionally absent.
    Method
    Supply fixed metadata ``synthetic``, ``open``, ``row Cartesian``, and synthetic
    length unit ``test-L``.
    Oracle
    Each owning test documents an independent analytical rank or singular- value-ratio
    argument.
    Acceptance
    ...
    Limitations
    Fixtures are synthetic; no DFT, Wannier, experimental, crystallographic, or impurity
...
def test_constructor__well_conditioned_orthogonal_cell_is__is_enforced() -> None:
    ...
    Limitations
    It does not validate arbitrary orthogonal cells, physical units, scientific
    validation, UQ, or Rust conformance.
    """
...
def test_constructor__exactly_dependent_cell_is_rejected__is_enforced() -> None:
    ...
    Oracle
    ``row_2 = row_1`` gives the nontrivial exact relation ``row_2 - row_1 = 0``, so
    :math:`\sigma_{\min}=0`.
    Acceptance
    ...
    Limitations
    It does not quantify conditioning of arbitrary matrices or establish scientific
    validation, UQ, or Rust conformance.
    """
...
def test_constructor__skew_independent_cell_is_admitted__is_enforced() -> None:
    ...
    Oracle
    Forward substitution proves only zero coefficients solve the row linear combination;
    ...
    Limitations
    No crystallographic or physical realism, scientific validation, UQ, or Rust
...
def test_constructor__left_handed_independent_cell_is_admitted__is_enforced() -> None:
    ...
    Oracle
    The diagonal product ``2 * 3 * -4 = -24`` is nonzero and negative, proving full rank
    ...
    Limitations
    Admission is not a physical or crystallographic validity claim and establishes no
...
def test_constructor__cell_clearly_below_public_threshold__is_rejected() -> None:
    ...
    NV-G-005
    Requirement
    A diagonal cell with singular-value ratio ``RTOL / 2`` is invalid.
    Method
    Construct the analytical diagonal case under RuntimeWarning-as-error.
    Oracle
    Positive diagonal singular values are their entries, so the ratio is exactly half
    the independently fixed threshold.
    Acceptance
    ``RTOL`` is exactly ``1e-12`` and construction raises independence ``ValueError``.
    Interpretation
    A pass verifies clearly-below-threshold rejection without testing equality.
    Limitations
    This does not select a scientific tolerance, validate a cell, perform UQ, or
...
def test_constructor__cell_at_strict_public_threshold__is_rejected() -> None:
    ...
    NV-G-010
    Requirement
    Equality with the strict public singular-value-ratio threshold is invalid.
    Method
    Construct ``diag(1, RTOL, 1)`` under RuntimeWarning-as-error.
    Oracle
    The positive diagonal singular values give ratio exactly the stored binary64
    ``RTOL`` and the public inequality is strict.
    Acceptance
    Construction raises independence ``ValueError`` without warning.
    Interpretation
    A pass verifies the inclusive rejection side of the exact threshold boundary.
    Limitations
    This does not select a scientific tolerance, validate a cell, perform UQ, or
    establish Rust agreement.
    """
...
def test_constructor__cell_clearly_above_public_threshold_is__is_enforced() -> None:
    ...
    Oracle
    Positive diagonal singular values are their absolute diagonal entries, so
    ...
    Limitations
    This does not select a scientific tolerance, perform UQ, or establish scientific
...
def test_field__uniform_scale_invariance_for_well_conditioned_cells__is_exact(
    ...
    Method
    Scale the identity cell by both signs of ``1e-200``, ``1``, and ``1e200`` and
    ...
    Acceptance
    Every construction succeeds, preserves ``scale`` on the diagonal, and leaks no
    ``RuntimeWarning``.
    Interpretation
    Passing establishes tested scale robustness of accepted cells.
    Limitations
    Scales are representative finite normal binary64 values, not an exhaustive platform
...
def test_field__uniform_scale_invariance_for_near_dependent_cells__is_exact(
    ...
    Method
    Scale ``diag(1, RTOL / 2, 1)`` by both signs of each representative finite normal
    scale and construct under warning-as-error.
    Oracle
    Singular values scale uniformly, leaving :math:`\rho=r_{\mathrm{tol}}/2`
    analytically at every scale.
    Acceptance
    Every construction raises exactly independence ``ValueError`` and leaks no
    ``RuntimeWarning``.
    Interpretation
    Passing establishes tested scale robustness of rejected near-dependence.
    Limitations
    This does not cover exact-threshold behavior, select scientific tolerance, perform
...
def test_constructor__valid_row_permutation__preserves_independence(
    ...
    NV-G-009
    Requirement
    Every row permutation of the analytical full-rank triangular cell remains valid.
    Method
    Construct all six literal permutations under RuntimeWarning-as-error.
    Oracle
    Row permutation preserves the nonzero triangular determinant magnitude and singular
    values.
    Acceptance
    Construction succeeds and preserves selected row order exactly.
    Interpretation
    A pass verifies valid permutation invariance for this represented cell.
    Limitations
    Other matrices, physical validity, validation, UQ, and Rust are excluded.
    """
    with warnings.catch_warnings():
        warnings.simplefilter("error", RuntimeWarning)
...
def test_constructor__dependent_row_permutation__preserves_rejection(
    ...
    NV-G-011
    Requirement
    Every row permutation of the duplicated-row cell remains invalid.
    Method
    Construct all six literal permutations under RuntimeWarning-as-error.
    Oracle
    Permutation preserves the exact relation between duplicated rows.
    Acceptance
    Every case raises independence ``ValueError`` without warning.
    Interpretation
    A pass verifies invalid permutation invariance for this represented cell.
    Limitations
    Near dependence, physical validity, validation, UQ, and Rust are excluded.
    """
    with warnings.catch_warnings():
        warnings.simplefilter("error", RuntimeWarning)
        with pytest.raises(ValueError, match="linearly independent"):
