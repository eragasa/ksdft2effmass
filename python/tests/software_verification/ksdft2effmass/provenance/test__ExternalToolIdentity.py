...
def test_constructor__field_mapping__stores_exact_declared_types_and_values() -> None:
    ...
    SV-PROV-024
    Requirement
    Construction stores both declared fields unchanged as built-in strings.
    Method
    Construct from fixed valid metadata and inspect public dataclass fields.
    Oracle
    The accepted signature and supplied literals define exact order, values, and types.
    Acceptance
    The field sequence, stored tuple, and exact types match the two literals.
    Interpretation
    A pass confirms constructor mapping; a failure indicates source, input, or
    contract drift.
    Limitations
    Synthetic metadata only; this does not establish tool availability, validation,
...
def test_constructor__text_semantic_type__rejects_non_builtin_string(
    ...
    SV-PROV-025
    Requirement
    Each textual field requires an exact built-in string.
    Method
    Replace the named valid field with bytes while retaining the other field.
    Oracle
    The public type contract excludes bytes from both string fields.
    Acceptance
    Every field partition raises ``TypeError`` and emits no warning.
    Interpretation
    A pass confirms strict field typing; a failure identifies an accepted lookalike.
    Limitations
    Only bytes represents wrong types; no availability, validation, UQ, portability,
...
def test_constructor__portable_text_value__rejects_invalid_grammar(
    ...
    SV-PROV-191
    Requirement
    Each identifier is nonempty, NFC, surrogate-free, grammar-valid, and at most 128
    characters.
    Method
    Replace each field with empty, spaced, non-NFC, surrogate, and 129-character
    partitions.
    Oracle
    The literal ``[A-Za-z0-9][A-Za-z0-9._:-]{0,127}`` grammar and Unicode invariants
    reject every table entry.
    Acceptance
    Every field-and-partition case raises ``ValueError`` without a warning.
    Interpretation
    A pass confirms field-wise malformed-value rejection; a failure identifies an
    unchecked field or partition.
    Limitations
    This finite boundary set does not exhaust Unicode or establish availability,
...
def test_field__frozen_assignment__rejects_reassignment() -> None:
    ...
    SV-PROV-192
    Requirement
    The identity record is operationally immutable after construction.
    Method
    Assign a different valid value to the public ``tool_id`` field.
    Oracle
    Frozen dataclass assignment semantics require ``FrozenInstanceError``.
    Acceptance
    Assignment raises that exception and does not mutate the record.
    Interpretation
    A pass confirms reassignment protection; a failure identifies mutable state.
    Limitations
    This does not test hostile reflection, availability, validation, UQ,
...
def test_method__eq__compares_complete_represented_state(
    ...
    SV-PROV-193
    Requirement
    Exact equality includes each of the two represented fields.
    Method
    ...
    field.
    Limitations
    This does not establish ordering, hashing, availability, validation, UQ,
    portability, or cross-language agreement.
    """
...
def test_field__durable_surface__excludes_runtime_credentials_and_handles() -> None:
    ...
    SV-PROV-194
    Requirement
    The durable identity surface excludes named runtime and credential state.
    Method
    Compare public field names with the accepted prohibited-name vocabulary.
    Oracle
    The durable-token boundary excludes commands, secrets, clients, processes,
    handles, and schedulers.
    Acceptance
    The public and prohibited field-name sets are disjoint.
    Interpretation
    A pass confirms this named surface boundary; a failure identifies prohibited
    durable state.
    Limitations
