...
def test_constructor__correlation_fields__maps_exact_values() -> None:
    ...
    SV-PROV-175
    Requirement
    Construction maps request identity, outcome identity, and issue tuple exactly.
    Method
    Construct a result with distinct identifiers and one canonical issue.
    Oracle
    Literal inputs and constructor field order fix represented state.
    Acceptance
    All three public fields equal their inputs exactly.
    Interpretation
    Failure indicates constructor mapping or represented-state drift.
    Limitations
    The constructor does not inspect request or outcome records.
    """
...
def test_property__status__derives_from_issue_emptiness(
    ...
    SV-PROV-051
    Requirement
    Empty issues derive CORRELATED and every nonempty canonical tuple derives MISMATCH.
    Method
    Construct empty and singleton issue partitions.
    Oracle
    Tuple emptiness independently predicts the exact status member.
    Acceptance
    Each case returns the listed enum member.
    Interpretation
    Failure indicates incorrect status derivation.
    Limitations
    Issue reachability from real records is covered by the correlator.
    """
...
def test_field__status_storage__excludes_derived_property() -> None:
    ...
    SV-PROV-164
    Requirement
    Derived status is absent from represented result fields.
    Method
    Inspect the public dataclass field inventory.
    Oracle
    The public ResultObject contract declares exactly request_id, outcome_id, and
    issues.
    Acceptance
    Field names equal the literal three-name tuple and exclude status.
    Interpretation
    Failure indicates duplicated derived state.
    Limitations
...
def test_field__issue_tuple_cases__accepts_canonical_subsets(
    ...
    SV-PROV-165
    Requirement
    Every singleton, ordered two-issue subset, and complete issue tuple is
    representable.
    Method
    Construct all seven nonempty canonical subsets with explicit semantic case IDs.
    Oracle
    The declared request-correlation-attempt enum order fixes each valid tuple.
    Acceptance
    The stored issues tuple equals the supplied tuple exactly and status is MISMATCH.
    Interpretation
    Failure indicates overrestriction, reordering, or storage drift.
    Limitations
    The cases are synthetic and do not perform external execution.
    """
    value = SUT("request-1", "outcome-1", issues)
...
def test_field__identifier_semantic_type__rejects_non_string(field_name: str) -> None:
    ...
    SV-PROV-166
    Requirement
    Both identifiers accept only built-in strings.
    Method
    Replace one identifier at a time with bytes.
    Oracle
    The public semantic type contract requires TypeError without coercion.
    Acceptance
    Each partition raises TypeError.
    Interpretation
    Failure indicates weakened identifier typing.
    Limitations
    Malformed built-in strings are independently partitioned.
    """
...
def test_field__identifier_value__rejects_nonportable_text(
    ...
    SV-PROV-167
    Requirement
    Both identifiers obey the nonempty portable identifier grammar.
    Method
    Partition empty, embedded-space, non-NFC, surrogate, and overlength values for
    each field.
    Oracle
    The public identifier grammar rejects every literal.
    Acceptance
    Each case raises ValueError.
    Interpretation
    Failure indicates malformed identifier acceptance.
    Limitations
    The declared grammar partitions are sampled, not every possible string.
    """
...
def test_field__issues_tuple_type__rejects_list() -> None:
    ...
    SV-PROV-052
    Requirement
    issues must be a built-in tuple rather than a list lookalike.
    Method
    Construct with an empty list and valid identifiers.
    Oracle
    The public immutable tuple contract requires TypeError.
    Acceptance
    Construction raises TypeError.
    Interpretation
    Failure indicates coercion or mutable issue storage.
    Limitations
    Tuple member typing is independently covered.
    """
    with pytest.raises(TypeError):
...
def test_field__issue_member_type__rejects_string_lookalike() -> None:
    ...
    SV-PROV-168
    Requirement
    Every issue member must be a CorrelationIssue enum value.
    Method
    Construct with a tuple containing the matching raw string.
    Oracle
    The public enum semantic type boundary requires TypeError without coercion.
    Acceptance
    Construction raises TypeError.
    Interpretation
    Failure indicates raw-string issue coercion.
    Limitations
    Future enum expansion is outside P2.
    """
    with pytest.raises(TypeError):
...
def test_field__issue_canonicalization__rejects_duplicates_and_wrong_order(
    ...
    SV-PROV-169
    Requirement
    Issues are unique and ordered request, correlation, then attempt.
    Method
    Partition a duplicate tuple and a reverse-order tuple.
    Oracle
    The closed enum declaration order and set semantics reject both states.
    Acceptance
    Each case raises ValueError.
    Interpretation
    Failure indicates noncanonical retained correlation evidence.
    Limitations
    Only the version-1 issue vocabulary is covered.
    """
    with pytest.raises(ValueError):
...
def test_field__frozen_assignment__rejects_reassignment() -> None:
    ...
    SV-PROV-170
    Requirement
    Correlation results are operationally immutable through ordinary assignment.
    Method
    Assign another valid outcome identifier after construction.
    Oracle
...
def test_method__eq__compares_complete_represented_state() -> None:
    ...
    SV-PROV-171
    Requirement
    Equality compares request ID, outcome ID, and issues exactly.
    Method
    Compare equal constructions and three variants, each changing exactly one field.
    Oracle
    Each independent literal field difference and dataclass value semantics predict
    inequality.
    Acceptance
    Equal state compares equal and every single-field variant compares unequal.
    Interpretation
    Failure indicates incomplete or nonexact value semantics.
    Limitations
    Equality does not prove provenance truth or actual execution correlation.
    """
