...
def test_field__wire_vocabulary__has_exact_order_names_values_and_count() -> None:
    ...
    SV-PROV-075
    Requirement
    The version-1 manifest lifecycle vocabulary has three exact alias-free members.
    Method
    Inspect inheritance, paired declaration order, names, values, count, and the
    complete member mapping.
    Oracle
    The accepted pairs are DECLARED/declared, COMPLETE/complete, and FAILED/failed
    in that order.
    Acceptance
    The enum subclasses StrEnum; paired order, names, values, and count match
    exactly; ``__members__`` contains exactly those identities without aliases.
    Interpretation
    Passing establishes the closed lifecycle vocabulary; failure indicates member,
    order, value, inheritance, count, or alias drift.
    Limitations
    This does not validate a manifest, timestamp relation, execution, or result.
    """
    expected_members = (
...
def test_method__call__constructs_each_state_from_wire_value(
    ...
    SV-PROV-368
    Requirement
    Enum value construction resolves each lifecycle wire value to its exact member.
    Method
    Call the enum with each explicit wire value and an independently supplied
    expected member.
    Oracle
    The fixed version-1 value/member pairs define expected identities without
    consulting ``__members__``.
    Acceptance
    ``ManifestState(value)`` is the independently specified member.
    Interpretation
    Passing establishes successful value construction for all lifecycle states;
    failure indicates lookup or vocabulary drift.
    Limitations
    Construction selects a state only and does not validate manifest relations.
    """
...
def test_method__getitem__returns_each_state_from_declared_name(
    ...
    SV-PROV-369
    Requirement
    Declared-name lookup resolves each exact lifecycle name to its exact member.
    Method
    Index the enum class with each explicit name and independently supplied expected
    member.
    Oracle
    The fixed version-1 name/member pairs define expected identities without using
    ``__members__`` as the successful oracle.
    Acceptance
    ``ManifestState[name]`` is the independently specified member.
    Interpretation
    Passing establishes declared-name lookup for every lifecycle state; failure
    indicates name lookup or declaration drift.
    Limitations
    Name lookup does not validate manifest data or serialized wire values.
    """
...
def test_method__call__rejects_unknown_wire_value() -> None:
    ...
    SV-PROV-370
    Requirement
    Value construction rejects wire text outside the closed lifecycle vocabulary.
    Method
    Call ``ManifestState`` with the explicit string ``unknown``.
    Oracle
    The accepted wire values are declared, complete, and failed only.
    Acceptance
...
def test_method__call__rejects_wrong_semantic_type() -> None:
    ...
    Method
    Call ``ManifestState`` with integer 1.
    Oracle
    Standard enum value construction raises ValueError when no member has that exact
    value.
    Acceptance
    Construction raises exactly ValueError.
    Interpretation
    Passing establishes wrong-type value rejection without string coercion; failure
...
def test_method__getitem__rejects_unknown_member_name() -> None:
    ...
    SV-PROV-372
    Requirement
    Declared-name lookup rejects names outside the closed lifecycle inventory.
    Method
    Index ``ManifestState`` with the explicit name ``UNKNOWN``.
    Oracle
    The accepted declared names are DECLARED, COMPLETE, and FAILED only.
    Acceptance
    ...
    Limitations
    This does not assess unknown wire values or manifest construction.
    """
