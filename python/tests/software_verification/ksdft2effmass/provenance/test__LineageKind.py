...
def test_field__wire_vocabulary__has_exact_order_names_values_and_count() -> None:
    ...
    SV-PROV-019
    Requirement
    The version-1 lineage vocabulary has three exact alias-free members.
    Method
    Inspect inheritance, paired declaration order, names, values, count, and the
    complete member mapping.
    Oracle
    The accepted pairs are DERIVED/derived, REPRESENTATION/representation, and
    ...
    Interpretation
    Passing establishes the closed lineage vocabulary; failure indicates member,
    order, value, inheritance, count, or alias drift.
    Limitations
    This does not validate an edge, endpoint relation, lineage claim, or graph.
    """
    expected_members = (
        LineageKind.DERIVED,
...
def test_method__call__constructs_each_kind_from_wire_value(
    ...
    SV-PROV-373
    Requirement
    Enum value construction resolves each lineage wire value to its exact member.
    Method
    ...
    Acceptance
    ``LineageKind(value)`` is the independently specified member.
    Interpretation
    Passing establishes successful value construction for all lineage kinds;
    failure indicates lookup or vocabulary drift.
    Limitations
    Construction selects a kind only and does not validate or prove a lineage edge.
    """
...
def test_method__getitem__returns_each_kind_from_declared_name(
    ...
    SV-PROV-374
    Requirement
    Declared-name lookup resolves each exact lineage name to its exact member.
    Method
    ...
    Acceptance
    ``LineageKind[name]`` is the independently specified member.
    Interpretation
    Passing establishes declared-name lookup for every lineage kind; failure
    indicates name lookup or declaration drift.
    Limitations
    Name lookup does not validate endpoint identities or serialized wire values.
    """
...
def test_method__call__rejects_unknown_wire_value() -> None:
    ...
    SV-PROV-375
    Requirement
    Value construction rejects wire text outside the closed lineage vocabulary.
    Method
    Call ``LineageKind`` with the explicit string ``unknown``.
    Oracle
    The accepted wire values are derived, representation, and retry only.
    Acceptance
...
def test_method__call__rejects_wrong_semantic_type() -> None:
    ...
    Method
    Call ``LineageKind`` with integer 1.
    Oracle
...
def test_method__getitem__rejects_unknown_member_name() -> None:
    ...
    SV-PROV-377
    Requirement
    Declared-name lookup rejects names outside the closed lineage inventory.
    Method
    Index ``LineageKind`` with the explicit name ``UNKNOWN``.
    Oracle
    The accepted declared names are DERIVED, REPRESENTATION, and RETRY only.
    Acceptance
    ...
    Limitations
    This does not assess unknown wire values or lineage-relation construction.
    """
