...
def test_constructor__error_taxonomy__stores_message_and_inherits_value_error() -> None:
    """Evidence ID: SV-PROV-056

    Requirement: Direct public construction stores the supplied message and creates
    exactly a
    ProvenanceJsonError within the ValueError taxonomy.

    Method: Construct ProvenanceJsonError with the fixed text ``detail`` and inspect its
    exact runtime type, base-class membership, args tuple, and string form.

    Oracle: The public class declaration requires ValueError inheritance, while Python's
    exception constructor defines the exact args and string payload semantics.

    Acceptance: The exact type is ProvenanceJsonError, the instance is a ValueError,
    args is
    ``("detail",)``, and ``str(error)`` is exactly ``"detail"``.

    Interpretation: Failure identifies drift in the public exception class, inheritance,
    or
    directly stored message payload.

    Limitations: This direct-construction check verifies only public exception taxonomy
    and
