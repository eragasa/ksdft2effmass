...
def test_constructor__field_mapping__stores_exact_declaration() -> None:
    ...
    SV-PROV-028
    Requirement
    Construction stores five exact fields, including the semantic enum member.
    Method
    Construct fixed valid metadata and inspect public dataclass state.
    Oracle
    The accepted signature and literals define exact field order, values, and types.
    Acceptance
    Names, the complete stored tuple, string types, and enum identity match exactly.
    Interpretation
    A pass confirms constructor mapping; a failure indicates source, input, or
    contract drift.
    Limitations
    Synthetic metadata only; no invocation, validation, UQ, portability, or cross-
...
def test_constructor__text_semantic_type__rejects_non_string(
    ...
    SV-PROV-029
    Requirement
    Each capability text field requires an exact built-in string.
    Method
    ...
    Limitations
    Only bytes represents wrong types; no invocation, validation, UQ, portability,
...
def test_constructor__kind_semantic_type__rejects_string_lookalike() -> None:
    ...
    Method
    Pass ``execute`` with otherwise valid declaration fields.
    Oracle
    The accepted semantic type is the public enum class, not built-in ``str``.
    Acceptance
    Construction raises ``TypeError`` and emits no warning.
    Interpretation
    A pass confirms enum typing; a failure identifies coercion of a string lookalike.
    Limitations
    This tests one lookalike, not invocation, validation, UQ, portability, or cross-
    language agreement.
    """
    kwargs = dict(VALID_VALUES)
...
def test_constructor__portable_text_value__rejects_invalid_identifier(
    ...
    SV-PROV-201
    Requirement
    Every capability identifier is nonempty, NFC, surrogate-free, grammar-valid, and
    at most 128 characters.
    Method
    Replace each text field with empty, spaced, non-NFC, surrogate, and overlength
    partitions.
    Oracle
    The literal portable-identifier grammar and Unicode invariants reject every
    table entry.
    Acceptance
    ...
    field or partition.
    Limitations
    This finite set does not exhaust Unicode or establish invocation, validation,
...
def test_field__frozen_assignment__rejects_reassignment() -> None:
    ...
    SV-PROV-202
    Requirement
    A capability declaration is operationally immutable after construction.
    Method
    Assign a different valid value to the public ``name`` field.
    Oracle
    ...
    Limitations
    This excludes hostile reflection, invocation, validation, UQ, portability, and
...
def test_method__eq__compares_complete_represented_state(
    ...
    SV-PROV-203
    Requirement
    Exact equality includes each of the five represented capability fields.
    Method
    ...
    field.
    Limitations
    This does not test ordering, invocation, validation, UQ, portability, or cross-
    language agreement.
    """
...
def test_field__durable_surface__excludes_runtime_state() -> None:
    ...
    SV-PROV-204
    Requirement
    The durable capability surface excludes named runtime and credential state.
    Method
    Compare public field names with the accepted prohibited-name vocabulary.
    Oracle
    The durable boundary excludes commands, credentials, clients, processes, and
    handles.
    Acceptance
