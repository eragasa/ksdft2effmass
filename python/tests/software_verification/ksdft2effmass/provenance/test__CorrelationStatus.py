...
def test_field__correlation_status_vocabulary__is_exact() -> None:
    ...
    SV-PROV-053
    Requirement
    Correlation outcomes are exactly CORRELATED and MISMATCH with stable wire values.
    Method
    Enumerate the public string enum without invoking a correlator.
    Oracle
    The accepted P2 vocabulary independently fixes names, values, and order.
    Acceptance
    Name and value tuples equal the literal expected tuples exactly.
    Interpretation
    Failure indicates public or serialized status-vocabulary drift.
    Limitations
    Enumeration does not establish correlation of any real external outcome.
    """
