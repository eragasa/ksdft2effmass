...
def test_field__wire_vocabulary__has_exact_order_names_values_and_count() -> None:
    ...
    SV-PROV-011
    Requirement
    The version-1 location-kind vocabulary has two exact alias-free members.
    Method
    Inspect inheritance, iteration order, declared names, wire values, count, and
    the complete member mapping.
    Oracle
    The accepted vocabulary is ROOT_RELATIVE/root_relative followed by
    EXTERNAL_DESCRIPTOR/external_descriptor.
    Acceptance
    The enum subclasses StrEnum; order, names, values, and count match exactly;
    ``__members__`` has exactly those names and identities with no aliases.
    Interpretation
    Passing establishes the closed representation vocabulary; failure indicates
    member, ordering, value, inheritance, count, or alias drift.
    Limitations
    This does not construct or validate an ArtifactLocation or resolve either
    representation form.
    """
    expected_members = (
...
def test_method__call__constructs_each_kind_from_wire_value(
    ...
    SV-PROV-363
    Requirement
    Enum value construction resolves each declared wire value to its exact member.
    Method
    Call the enum with each explicit wire value and an independently named expected
    member.
    Oracle
    The fixed version-1 value/member pairs define the expected identities without
    consulting ``__members__``.
    Acceptance
    ``ArtifactLocationKind(value)`` is the independently specified member.
    Interpretation
    Passing establishes successful value construction for both representation
    forms; failure indicates lookup or vocabulary drift.
    Limitations
    Construction selects a kind only and does not validate a location payload.
    """
...
def test_method__getitem__returns_each_kind_from_declared_name(
    ...
    SV-PROV-364
    Requirement
    Declared-name lookup resolves each exact member name to its exact member.
    Method
    Index the enum class with each explicit name and independently named expected
    member.
    Oracle
    The fixed version-1 name/member pairs define expected identities without using
    ``__members__`` as the successful oracle.
    Acceptance
    ``ArtifactLocationKind[name]`` is the independently specified member.
    Interpretation
    Passing establishes successful declared-name lookup for both members; failure
    indicates name lookup or declaration drift.
    Limitations
    Name lookup does not validate location payloads or serialized wire values.
    """
...
def test_method__call__rejects_unknown_wire_value() -> None:
    ...
    SV-PROV-365
    Requirement
    Value construction rejects wire text outside the closed version-1 vocabulary.
    Method
    Call ``ArtifactLocationKind`` with the explicit string ``unknown``.
    Oracle
    The accepted vocabulary contains only root_relative and external_descriptor.
    Acceptance
    Construction raises exactly ValueError.
    Interpretation
    Passing establishes unknown-value rejection; failure indicates unintended
    vocabulary expansion or coercion.
    Limitations
    This does not assess unknown names or wrong semantic Python types.
    """
...
def test_method__call__rejects_wrong_semantic_type() -> None:
    ...
    SV-PROV-366
    Requirement
    Value construction rejects an integer rather than coercing it to wire text.
    Method
    Call ``ArtifactLocationKind`` with integer 1.
    Oracle
    Standard enum value construction raises ValueError when no member has that
    exact value.
    Acceptance
    Construction raises exactly ValueError.
    Interpretation
    Passing establishes wrong-type value rejection without string coercion;
    failure indicates broadened construction behavior.
    Limitations
    Integer 1 represents this wrong semantic type partition only.
    """
    with pytest.raises(ValueError):
...
def test_method__getitem__rejects_unknown_member_name() -> None:
    ...
    SV-PROV-367
    Requirement
    Declared-name lookup rejects names outside the closed member inventory.
    Method
    Index ``ArtifactLocationKind`` with the explicit name ``UNKNOWN``.
    Oracle
    The accepted declared names are ROOT_RELATIVE and EXTERNAL_DESCRIPTOR only.
    Acceptance
    Lookup raises exactly KeyError.
    Interpretation
    Passing establishes unknown-name rejection; failure indicates an unexpected
    alias or member declaration.
    Limitations
    This does not assess unknown wire values or location construction.
    """
