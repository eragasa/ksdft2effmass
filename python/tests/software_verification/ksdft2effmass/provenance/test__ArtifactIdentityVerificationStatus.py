...
def test_field__verification_status_vocabulary__is_exact() -> None:
    ...
    SV-PROV-048
    Requirement
    Exact identity outcomes are named VERIFIED and MISMATCH with stable wire values.
    Method
    Enumerate the public string enum without executing the verifier.
    Oracle
    The accepted P2 vocabulary independently fixes names, values, and declaration order.
    Acceptance
    Name and value tuples equal the two literal expected tuples exactly.
    Interpretation
    Failure indicates public or serialized vocabulary drift.
    Limitations
    Enumeration does not establish that represented bytes were observed or compared.
    """
