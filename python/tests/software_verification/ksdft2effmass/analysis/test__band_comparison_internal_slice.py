...
class TestBandComparisonInternalSlice:
    ...
    def test_artifact__comparison__compares_complete_aligned_synthetic_spectra(
        self,
    ) -> None:
        ...
        right = self.make_observation(
            "right", DftBackend.ABINIT, ((0.005, 1.0), (2.0, 3.0))
        )

        result = BandStructureComparator().execute(
            left, right, self.make_specification()
        )
    ...
    def test_artifact__comparison__rejects_missing_alignment_and_spectrum(self) -> None:
        ...
        """
        left = self.make_observation("left", DftBackend.QUANTUM_ESPRESSO, None)
        right = self.make_observation("right", DftBackend.ABINIT, None)
        left = replace(
            left,
    ...
    def test_artifact__comparison__reports_every_incompatibility_in_stable_order(
        self,
    ) -> None:
        ...
        """
        left = replace(
            self.make_observation("left", DftBackend.QUANTUM_ESPRESSO, None),
        ...
        right = replace(
            self.make_observation("right", DftBackend.ABINIT, None),
    ...
    def test_artifact__result__rejects_false_workflow_compatibility(self) -> None:
    ...
    def test_artifact__public_api__keeps_private_comparison_names_unexported(
        self,
    ) -> None:
        ...
        Method: Inspect the package namespace for every private comparison contract
        name.

        Oracle: The accepted initial-slice architecture explicitly defers public
        exports.
