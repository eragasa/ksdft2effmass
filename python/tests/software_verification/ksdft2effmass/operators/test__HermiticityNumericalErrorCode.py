r"""Software verification for ``HermiticityNumericalErrorCode``.
...
def test_exact_closed_member_sequence_and_stable_value() -> None:
    """SV-HNEC-001: verify the exact sole name, value, count, and order.

    Evidence ID
        ``SV-HNEC-001``.
    Requirement
        Public iteration contains exactly ``NONFINITE_RESIDUAL`` with stable
        value ``nonfinite_residual`` in the sole declaration-order position.
    Method
        Compare public enum iteration with the independently written literal
        ``EXPECTED_MEMBERS`` tuple.
    Oracle
        The approved closed contract is the literal ordered name/value sequence,
        not a sequence generated from production members.
    Acceptance
        Iterated name/value pairs equal ``EXPECTED_MEMBERS`` exactly.
    Interpretation
        Passing establishes one member, exact name and value, deterministic
        order, and absence of unapproved additional iterable members.
    Limitations
        This does not inspect source location or establish Analyzer emission,
        numerical accuracy, physical Hermiticity, scientific validation,
...
def test_public_member_registry_contains_no_aliases() -> None:
    """SV-HNEC-002: verify one declared name, one member, and no aliases.

    Evidence ID
        ``SV-HNEC-002``.
    Requirement
        The public Enum registry contains only ``NONFINITE_RESIDUAL`` mapped to
        the sole canonical member.
    Method
        Compare documented ``Enum.__members__`` with an independently explicit
        one-entry dictionary and compare registry and iteration counts.
    Oracle
        The approved no-alias contract permits exactly one declared public name
        and one iterable member.
    Acceptance
    ...
    Limitations
        No private Enum internals, Analyzer behavior, numerical verification,
        serialization, scientific validation, uncertainty quantification, or
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            HermiticityNumericalErrorCode.NONFINITE_RESIDUAL,
            id="SV-HNEC-003-nonfinite-residual",
        ),
    ],
)
def test_python_314_strenum_and_machine_readable_value_behavior(
    ...
    """SV-HNEC-003: verify Python 3.14 ``StrEnum`` string behavior.

    Evidence ID
        ``SV-HNEC-003``; the parameter ID identifies the sole stable value.
    Requirement
        The enum subclasses ``StrEnum`` and its member behaves as the ASCII
        lowercase snake-case machine-readable string ``nonfinite_residual``.
    Method
        Inspect public inheritance, string type/equality, ``str()``, lexical
        full match, and ASCII encoding.
    Oracle
        Python 3.14 ``StrEnum`` semantics and the approved literal machine value
        and lexical convention.
    Acceptance
        Every inheritance, string, lexical, and ASCII check succeeds.
    Interpretation
        Passing establishes deterministic in-memory Python machine-string
        behavior for the sole member.
    Limitations
        ``repr()``, hash, pickle, JSON, wire formats, numerical detection,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            HermiticityNumericalErrorCode.NONFINITE_RESIDUAL,
            id="SV-HNEC-004-nonfinite-residual",
        ),
    ],
)
def test_value_based_lookup_round_trips(
    ...
    """SV-HNEC-004: verify lookup by the stable machine-readable value.

    Evidence ID
        ``SV-HNEC-004``; the parameter ID identifies the stable value.
    Requirement
        ``EnumClass(value)`` returns the canonical member for both its public
        value and the independently literal ``nonfinite_residual`` string.
    Method
    ...
    Limitations
        ...
        approved as successful behavior. No Analyzer execution, numerical
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            HermiticityNumericalErrorCode.NONFINITE_RESIDUAL,
            id="SV-HNEC-005-nonfinite-residual",
        ),
    ],
)
def test_name_based_lookup_round_trips(code: HermiticityNumericalErrorCode) -> None:
    """SV-HNEC-005: verify lookup by the public member name.

    Evidence ID
        ``SV-HNEC-005``; the parameter ID identifies the corresponding member.
    Requirement
        ``EnumClass[name]`` returns the canonical member for both its public
        name and the independently literal ``NONFINITE_RESIDUAL`` name.
    Method
    ...
    Limitations
        The member name is not the lowercase machine value. No Analyzer
        emission, numerical verification, serialization, scientific validation,
...
@pytest.mark.parametrize(
    "lookup_kind",
    [
        pytest.param("invalid-value", id="SV-HNEC-006-invalid-value"),
        pytest.param("invalid-name", id="SV-HNEC-006-invalid-name"),
    ],
)
def test_invalid_lookup_exception_taxonomy(lookup_kind: str) -> None:
    """SV-HNEC-006: verify exact standard Enum lookup exceptions.

    Evidence ID
        ``SV-HNEC-006``; parameter IDs distinguish value and name lookup.
    Requirement
        An unknown value raises ``ValueError`` and an unknown name raises
        ``KeyError`` through their distinct public lookup forms.
    Method
    ...
    Limitations
        Standard-library message wording is not frozen. No Analyzer, matrix,
        numerical algorithm, dependent exception, scientific validation,
        uncertainty quantification, or Rust conformance is tested.
    """
