...
class TestEnergyReference:
    ...
    def make_energy_reference(
        *,
        ...
        Requirement: Synthetic fixtures pass the two typed textual fields unchanged to
        the
        public
        constructor.

        Method: Construct ``EnergyReference(zero=zero, unit=unit)`` without
        preprocessing.
        ...
        Interpretation: The helper supplies independently constructible synthetic
        metadata.
    ...
    def test_field__stored_state_is_frozen_and_slotted__is_exact(
        ...
        Requirement: ``zero`` and ``unit`` cannot be reassigned, arbitrary attributes
        cannot
        be added,
        ...
        Oracle: The approved frozen, slotted two-field DataObject contract defines the
        exact
        public
        exception taxonomy.
        ...
        Limitations: It does not use invariant bypasses, assert hash behavior, test
        nested
        serialization,
        perform scientific validation or UQ, or establish Rust conformance.
        """

        reference = TestEnergyReference.make_energy_reference()
    ...
    def test_method__eq__exact_structural_equality_uses_both_stored_fields() -> None:
        ...
        Acceptance: The identical object compares equal and each single-field variant
        does
        not.
        ...
        reference = TestEnergyReference.make_energy_reference()
        identical = TestEnergyReference.make_energy_reference()
        different_zero = TestEnergyReference.make_energy_reference(zero="explicit zero")
        different_unit = TestEnergyReference.make_energy_reference(unit="hartree")
    ...
    def test_method__eq__preserves_exact_text(
        ...
        Requirement: Equality is sensitive to zero-label case, punctuation or spacing,
        and
        unit-label
        case or spelling.

        Method: Compare a fixed synthetic baseline with one-field representation
        variants
        passed
        unchanged through the public constructor.

        Oracle: Exact Python string inequality over stored fields is the approved
        oracle.
        ...
        Limitations: It does not claim that ``eV`` and ``EV`` or ``hartree`` and ``Ha``
        are
        physically
        inequivalent, execute the compatibility analyzer, perform scientific validation
        or
        UQ, or establish Rust conformance.
        """

        reference = TestEnergyReference.make_energy_reference()
        variant = TestEnergyReference.make_energy_reference(zero=zero, unit=unit)
