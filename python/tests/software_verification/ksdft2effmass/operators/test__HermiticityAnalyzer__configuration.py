...
class TestHermiticityAnalyzer:
    ...
    def test_constructor__explicit_public_construction_stores__is_enforced() -> None:
        ...
        Method: ------ Construct one Analyzer with distinct nonzero tolerance and
        ``"eV"``
        unit.

        Oracle: ------ The approved public constructor and field contract defines
        expected
        state.
    ...
    def test_constructor__accepted_tolerance_scalar_families_are__is_enforced(
        tolerance: int | float | np.int64 | np.float32, expected: float
    ) -> None:
        r"""Evidence ID: SV-HA-002

        Requirement: ----------- Python and NumPy integer/floating scalars are admitted
        and
        stored as
        float.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(_ArbitraryInput(), id="sv_ha_003_arbitrary_object"),
        ],
    )
    @staticmethod
    def test_constructor__invalid_tolerance_wrong_types_are_rejected__is_enforced(
        tolerance: _InvalidInput,
    ) -> None:
        ...
        Method: ------ Pass independently identified invalid public inputs without
        coercion.
        ...
        Interpretation: -------------- Passing establishes rejection rather than
        accidental
        numeric
        coercion.

        Limitations: ----------- ``_InvalidInput`` is confined to this deliberate
        invalid-input
        boundary; no
        ...
        with pytest.raises(TypeError, match="real number"):
            HermiticityAnalyzer(tolerance=tolerance, energy_unit="eV")  # type: ignore[arg-type]
    ...
    def test_constructor__nonfinite_or_overflowing_tolerances_use__is_enforced(
        ...
        Method: ------ Construct with NaN, both infinities, and huge signed Python
        integers.

        Oracle: ------ IEEE-754 classification and the approved finite-number taxonomy
        are
        used.
        ...
        Limitations: ----------- It does not test matrix arithmetic, scientific
        validation,
        UQ, or Rust.
        """
    ...
    def test_constructor__finite_negative_tolerances_are_rejected__is_enforced(
        tolerance: float | np.float64,
    ) -> None:
        r"""Evidence ID: SV-HA-005

        Requirement: ----------- A finite negative tolerance is invalid ActionObject
        policy.
        ...
        Limitations: ----------- It does not select a scientifically appropriate
        nonnegative
        tolerance or
    ...
    def test_field__energy_unit_is_a_required_explicit_argument__is_exact() -> None:
        ...
        Method: ------ Omit only ``energy_unit`` from an otherwise valid constructor
        call.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(_ArbitraryInput(), id="sv_ha_007_arbitrary_object"),
        ],
    )
    @staticmethod
    def test_constructor__invalid_energy_unit_wrong_types_are__is_enforced(
        energy_unit: _InvalidInput,
    ) -> None:
        ...
        Acceptance: ---------- Exact ``TypeError`` category includes a string
        diagnostic.

        Interpretation: -------------- Passing establishes the public unit-type
        boundary.

        Limitations: ----------- ``_InvalidInput`` is confined to invalid inputs. Unit
        registries,
        conversion,
        ...
        with pytest.raises(TypeError, match="string"):
            HermiticityAnalyzer(tolerance=0.0, energy_unit=energy_unit)  # type: ignore[arg-type]
    ...
    def test_constructor__empty_energy_unit_is_rejected__is_enforced() -> None:
        ...
        Limitations: ----------- No trimming, case folding, registry, conversion,
        scientific
        validation,
    ...
    @staticmethod
    def test_field__configuration_is_frozen_slotted_and_has_exact_fields__is_exact() -> (  # noqa: E501
        None
    ):
        r"""Evidence ID: SV-HA-009

        Requirement: ----------- Stored fields are exactly ``tolerance`` and
        ``energy_unit``
        and
        immutable.

        Method: ------ Inspect public dataclass fields and attempt ordinary assignment
        to
        each.
        ...
        Acceptance: ---------- Field names match exactly, ``__dict__`` is absent, and
        each
        assignment
        raises the exact ``FrozenInstanceError`` dataclass exception.

        Interpretation: -------------- Passing establishes ordinary API-level
        configuration
        immutability.
    ...
    def test_method__serialize__analyzer_has_no_serialization_api() -> None:
        ...
        Requirement: ----------- Analyzer policy has no approved wire or dictionary
        format.

        Method: ------ Check the six prohibited serialization/deserialization names on
        an
        instance.
