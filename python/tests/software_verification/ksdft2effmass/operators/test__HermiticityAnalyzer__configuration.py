...
def test_constructor__explicit_public_construction_stores__is_enforced() -> None:
    ...
    SV-HA-001
    Requirement
    ----------- Both configuration values are explicit and stored as built-in Python
    types.
    Method
    ...
    Limitations
    ----------- No default policy, tolerance suitability, scientific validation, UQ, or
...
def test_constructor__accepted_tolerance_scalar_families_are__is_enforced(
    ...
    SV-HA-002
    Requirement
    ----------- Python and NumPy integer/floating scalars are admitted and stored as
    float.
    Method
    ...
    Interpretation
    -------------- Passing establishes runtime admission, canonical type, and preserved
    value.
    Limitations
...
def test_constructor__invalid_tolerance_wrong_types_are_rejected__is_enforced(
    ...
    Interpretation
    -------------- Passing establishes rejection rather than accidental numeric
    coercion.
    Limitations
    ----------- ``Any`` is confined to this deliberate invalid-input boundary; no
...
def test_field__energy_unit_is_a_required_explicit_argument__is_exact() -> None:
    ...
    Acceptance
    ---------- ``TypeError`` mentions the stable argument name without freezing full
    text.
    Interpretation
    -------------- Passing establishes required explicit unit configuration.
    Limitations
    ----------- No default is inferred; unit validity, scientific validation, UQ, and
...
def test_constructor__empty_energy_unit_is_rejected__is_enforced() -> None:
    ...
    Limitations
    ----------- No trimming, case folding, registry, conversion, scientific validation,
...
def test_field__configuration_is_frozen_slotted_and_has_exact_fields__is_exact() -> (
    ...
    SV-HA-009
    Requirement
    ----------- Stored fields are exactly ``tolerance`` and ``energy_unit`` and
    immutable.
    Method
    ...
    Acceptance
    ---------- Field names match exactly, ``__dict__`` is absent, and each assignment
    ...
    Limitations
    ----------- Hash behavior is deliberately untested; scientific validation, UQ, and
    Rust conformance are not established.
    """
...
def test_method__serialize__analyzer_has_no_serialization_api() -> None:
    ...
    Limitations
    ----------- It does not prohibit a future separately approved serializer and
