...
class TestEnergyReference:
    ...
    def test_constructor__public_fields_are_mapped_exactly__is_enforced() -> None:
        ...
        Method: Construct through the supported public import and inspect public
        dataclass
        fields
        and values.
        ...
        Interpretation: Passing establishes exact constructor-to-field mapping for
        synthetic
        metadata.
        ...
        assert tuple(field.name for field in fields(EnergyReference)) == (
            "zero",
            "unit",
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                SyntheticString("synthetic zero"), id="sv_er_002_string_subclass"
            ),
        ],
    )
    @staticmethod
    def test_field__represented__zero_convention_strings_are_preserved_exactly(
        ...
        Requirement: Nonempty zero-convention strings retain case, spaces, punctuation,
        and
        hyphenation
        without normalization or interpretation.

        Method: Construct independently with each synthetic label and a fixed valid
        unit,
        passing
        the string directly to the public constructor.

        Oracle: Exact Python string equality with the independently supplied input is
        the
        approved
        preservation oracle.
        ...
        Limitations: It does not decide whether any label is physically meaningful,
        compare
        references,
    ...
    def test_field__energy_unit_strings_are_preserved_exactly__is_exact(
        ...
        Method: Construct independently with each synthetic label and compare it with
        the
        directly
        supplied input.
        ...
        Interpretation: Passing establishes literal storage, including case
        distinctions.

        Limitations: It does not recognize units, assert equivalence between labels,
        verify
        a conversion
    ...
    def test_constructor__numerical_offset_keywords__are_rejected(keyword: str) -> None:
        ...
        Requirement: ``EnergyReference`` rejects each unapproved numerical-offset
        keyword
        role.

        Method: Call the public constructor through ``_InvalidInput`` with one named
        finite offset.
        ...
        Limitations: Positional arity, stored fields, validation, UQ, and Rust are
        excluded.
        """
        with pytest.raises(TypeError):
            EnergyReference("explicit zero", "eV", **{keyword: 0.0})
    ...
    def test_constructor__numerical_offset_positional_argument__is_rejected() -> None:
        ...
        Method: Call the public constructor through ``_InvalidInput`` with an additional
        finite float.
        ...
        """
        with pytest.raises(TypeError):
            EnergyReference("explicit zero", "eV", 0.0)  # type: ignore[call-arg]
    ...
    def test_field__numerical_offset_attributes__are_absent() -> None:
        ...
        Interpretation: A pass confirms represented-state exclusion; failure indicates
        API
        drift.
    ...
    def test_public_api__serialization__is_absent() -> None:
        ...
        Method: Inspect a valid instance and the public class for all six excluded
        method
        names.
