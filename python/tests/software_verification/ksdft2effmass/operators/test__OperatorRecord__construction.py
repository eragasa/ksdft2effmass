...
class TestOperatorRecord:
    ...
    def test_constructor__public_fields_are_mapped_exactly__is_enforced() -> None:
        ...
        Limitations: It does not inspect private storage, execute ActionObjects,
        establish
        physical
        ...
        basis = OperatorRecordFixtureFactory.make_basis()
        ...
        np.testing.assert_array_equal(
            record.matrix,
            np.array([[1 + 0j, 2j], [-2j, 3 + 0j]], dtype=np.complex128),
        )
    ...
    def test_constructor__approved_matrix_inputs_canonicalize_without__is_enforced(
        ...
        Oracle: Independently literal complex values define the canonical expected
        state.
        ...
        Limitations: It does not approve arbitrary array-like containers, Booleans,
        strings,
        physical
        matrices, scientific validation, UQ, or Rust conformance.
        """

        record = OperatorRecordFixtureFactory.make_record(matrix)

        np.testing.assert_array_equal(
            record.matrix,
            np.array(expected, dtype=np.complex128),
        )
    ...
    def test_constructor__unsupported_matrix_containers__raise_type_error(
        unsupported: _InvalidInput,
    ) -> None:
        r"""Evidence ID: SV-OR-043

        Requirement: Matrix containers outside exact nested sequences and exact ndarrays
        are
        rejected.

        Method: Pass memoryview, masked-array, and ndarray-row containers without
        coercion.

        Oracle: The accepted public matrix-container boundary excludes all three
        categories.
        ...
        Limitations: Scalar invariants, numerical matrix meaning, validation, UQ, and
        Rust
        are excluded.
        """
        with pytest.raises(TypeError):
            OperatorRecordFixtureFactory.make_record(unsupported)  # type: ignore[arg-type]
    ...
    def test_field__canonical_matrix_representation__has_required_properties() -> None:
        ...
        Interpretation: Passing establishes representation state, not its private
        backing.

        Limitations: It does not inspect ``.base``, calculate a norm, establish
        scientific
        validation,
    ...
    def test_constructor__general_nonhermitian_finite_matrix_is__is_enforced() -> None:
        ...
        Requirement: OperatorRecord stores general finite represented operators and
        imposes
        no
        Hermiticity policy.
        ...
        record = OperatorRecordFixtureFactory.make_record([[0, 1], [2, 0]])

        np.testing.assert_array_equal(
            record.matrix,
            np.array([[0j, 1 + 0j], [2 + 0j, 0j]], dtype=np.complex128),
        )
    ...
    def test_field__represented_state__identifier_and_operator_kind_are_preserved(
        ...
        record = OperatorRecordFixtureFactory.make_record(
            identifier=identifier, operator_kind=operator_kind
        )
    ...
    def test_field__shape_is_the_exact_canonical_matrix_shape__is_exact() -> None:
        ...
        Acceptance: ``record.shape == record.matrix.shape == (2, 2)`` and both elements
        are
        exact
        built-in integers.
        ...
        Limitations: It introduces no dimension property, numerical algorithm,
        scientific
        validation, UQ,
        or Rust conformance.
        """
