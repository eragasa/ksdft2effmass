...
class TestOperatorRecordDifferenceNumericalError:
    ...
    def test_constructor__public_construction_and_exception_taxonomy__is_enforced() -> (
        ...
        Requirement: The supported public constructor accepts the approved structured
        code
        and produces a
        ...
        Oracle: The accepted exception contract declares ``ValueError`` inheritance and
        a
        one-code
        public constructor.
    ...
    def test_field__accepted_codes_retain_identity__is_exact(
        ...
        Limitations: Member count, aliases, ``StrEnum`` behavior, lookup taxonomy,
        exception
    ...
    @staticmethod
    def test_protocol__str__human_readable_message_summarizes_authoritative_code() -> (
        None
    ):
        ...
        numerical
        failure and contains the retained code's stable value, while ``error.code``
        remains
        authoritative machine-readable state.

        Method: Construct the exception, inspect semantic substrings in ``str(error)``,
        and
        verify
        direct code identity.

        Oracle: Accepted source documentation promises a human-readable difference-
        failure
        message
        ...
        Interpretation: Passing establishes useful human diagnostics without promoting
        the
        message to a
        machine-parsing API.
        ...
        accuracy, scientific validation, uncertainty quantification, and Rust
        conformance
        are not tested.
        """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                id="sv_ordne_004_unrelated_enum",
            ),
            pytest.param(_ArbitraryInput(), id="sv_ordne_004_arbitrary_object"),
        ],
    )
    @staticmethod
    def test_constructor__invalid_code_types_are_rejected__is_enforced(
        ...
        Requirement: Raw strings, ``None``, Booleans, unrelated enum members, and
        arbitrary
        objects are
        rejected rather than coerced to the owner enum.

        Method: Pass each representative invalid value directly to the public
        constructor,
        using
        ``_InvalidInput`` and ``cast`` only at this deliberate invalid-type boundary.
        ...
        with pytest.raises(TypeError) as exc_info:
            OperatorRecordDifferenceNumericalError(invalid_code)  # type: ignore[arg-type]
    ...
    def test_constructor__input_boundary__free_form_reason_and_extra_argument_are(
        ...
        keyword free-form
        reasons raise ``TypeError``, and successful instances expose no public
        ``reason``
        attribute.

        Method: Invoke the constructor through a deliberate ``_InvalidInput``-typed
        invalid- signature
        boundary for each reason form, then inspect an ordinary valid instance for
        absence
        of ``reason``.
        ...
        Interpretation: Passing protects the closed enum-backed taxonomy from
        contradictory
        or arbitrary
        structured diagnostic state.

        Limitations: Signature-generated ``TypeError`` wording is not frozen. Message
        text,
        differencer
        ...
        code = OperatorRecordDifferenceNumericalErrorCode.NONFINITE_DIFFERENCE
        invalid_constructor = OperatorRecordDifferenceNumericalError

        if reason_form == "positional":
            with pytest.raises(TypeError):
                invalid_constructor(code, "synthetic reason")  # type: ignore[call-arg]
        else:
            with pytest.raises(TypeError):
                invalid_constructor(code, reason="synthetic reason")  # type: ignore[call-arg]
    ...
    def test_method__serialize__exception_has_no_serialization_api() -> None:
        ...
        Oracle: Schema version 1 serializes ``OperatorRecord`` only; no numerical-
        exception
        schema
        ...
        Limitations: Pickling, traceback serialization, future schema design,
        differencer
        execution,
