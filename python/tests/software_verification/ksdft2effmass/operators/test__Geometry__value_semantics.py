...
class TestGeometry:
    ...
    def make_geometry(
        *,
        ...
        Requirement: No coercion, tuple conversion, or metadata normalization occurs
        here.
        ...
        Limitations: Fixtures use synthetic ``test-L`` units and ``row Cartesian``
        metadata.
        No DFT,
    ...
    def mutate_nested_cell(geometry: Geometry) -> None:
        ...
        Interpretation: The helper exercises ordinary public mutation, not invariant
        bypass.
    ...
    def test_field__frozen_slotted_state_rejects_public_mutation__is_exact() -> None:
        ...
        Requirement: Dataclass fields and dynamic attributes cannot be assigned, nested
        cell
        components
        cannot be mutated, and no per-instance ``__dict__`` exists.

        Method: Exercise ordinary ``setattr`` and tuple item assignment on a valid
        object.

        Oracle: Frozen dataclass assignment raises ``FrozenInstanceError`` and tuple
        item
        assignment
        ...
        with pytest.raises(TypeError):
            TestGeometry.mutate_nested_cell(geometry)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                make_geometry(
                    cell=((2.0, 0.0, 0.0), (0.5, 3.5, 0.0), (0.0, 0.25, 4.0))
                ),
                id="sv_g_021_cell",
            ),
            pytest.param(
    ...
    def test_method__eq__exact_structural_equality_uses_every_stored_field(
        ...
        Requirement: Equality is exact structural equality across all five stored
        fields.

        Method: Compare equal independently constructed objects and one-field variants.

        Oracle: Dataclass exact value semantics and independently selected variant
        values.
        ...
        reference = TestGeometry.make_geometry()
        equal = TestGeometry.make_geometry()
    ...
    def test_method__eq__cell_equality_is_row_order_and_component_sensitive(
        ...
        Requirement: Row permutation, component value, and sign changes alter exact
        value.
        ...
        Interpretation: Passing excludes approximate comparison from
        ``Geometry.__eq__``.
        ...
        assert reference == TestGeometry.make_geometry()
        assert reference != TestGeometry.make_geometry(cell=changed_cell)
