...
def make_geometry(
    *,
    ...
    Limitations
    Fixtures use synthetic ``test-L`` units and ``row Cartesian`` metadata. No DFT,
...
def mutate_nested_cell(geometry: Geometry) -> None:
    ...
    Method
    Execute direct public item assignment; the type-ignore is limited to the intentional
    ...
    Limitations
    Synthetic metadata establish no DFT, Wannier, experimental, crystallographic,
...
def test_field__frozen_slotted_state_rejects_public_mutation__is_exact() -> None:
    ...
    SV-G-020
    Requirement
    Dataclass fields and dynamic attributes cannot be assigned, nested cell components
    ...
    Oracle
    Frozen dataclass assignment raises ``FrozenInstanceError`` and tuple item assignment
    raises ``TypeError`` under the public storage contract.
    Acceptance
    Both field/dynamic assignment raise exactly ``FrozenInstanceError``; nested
    ...
    Limitations
    No hash behavior or unsupported invariant bypass is asserted; scientific validation,
...
def test_method__eq__exact_structural_equality_uses_every_stored_field(
    ...
    Acceptance
    Equal construction compares equal in both directions; each variant compares unequal
    in both directions.
    Interpretation
    Passing establishes every field participates in exact equality.
    Limitations
    It does not imply physical equivalence, approximate comparison, scientific
...
def test_method__eq__cell_equality_is_row_order_and_component_sensitive(
    ...
    Limitations
    Inequality does not establish physical inequivalence, scientific invalidity,
