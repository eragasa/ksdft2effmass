...
class TestHermiticityUnitMismatchError:
    ...
    def make_error(
        *,
        ...
        Requirement: Defaults are distinct nonempty strings in ordered Analyzer and
        record
        roles.

        Method: Pass the two caller-supplied strings unchanged to the public exception
        constructor
        without ``_InvalidInput`` or ``cast``.

        Oracle: The approved structured-error contract admits exact nonempty string
        disagreement.

        Acceptance: Construction returns ``HermiticityUnitMismatchError`` for the
        defaults.

        Interpretation: The result is deterministic synthetic in-memory software
        evidence.
    ...
    def test_constructor__public_construction_and_exception_taxonomy__is_enforced() -> (
        ...
        Requirement: Two distinct nonempty public strings directly construct the
        exception,
        which remains
        a ``ValueError`` and an ``Exception``.

        Method: Construct through the supported public import with ``"eV"`` in the
        Analyzer
        role and
        ``"hartree"`` in the record role.

        Oracle: The approved public exception contract specifies the documented
        hierarchy
        and
    ...
    @staticmethod
    def test_field__exact_analyzer_and_record_unit_roles_are_retained__is_exact() -> (
        None
    ):
        ...
        Acceptance: Each field equals its corresponding input and differs from the
        opposite
        role's
        input.

        Interpretation: Passing establishes role fidelity by value without requiring
        string
        object identity.

        Limitations: The Analyzer tolerance would use ``analyzer_energy_unit`` and the
        stored
        matrix/residual would use ``record_energy_unit``; this test constructs neither
        and
    ...
    def test_protocol__str__human_readable_unit_mismatch_summary() -> None:
        ...
        Acceptance: Case-insensitive role phrases and both exact input values occur in
        the
        message;
        structured fields retain the same values.

        Interpretation: Passing establishes a useful diagnostic without requiring
        message
        parsing for
        programmatic state.
        ...
        error = TestHermiticityUnitMismatchError.make_error()
        message = str(error)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "analyzer", _ArbitraryInput(), id="sv_hume_004_analyzer_object"
            ),
            ...
            pytest.param("record", _ArbitraryInput(), id="sv_hume_004_record_object"),
        ],
    )
    @staticmethod
    def test_constructor__invalid_unit_types_are_rejected__is_enforced(
        role: str, invalid_unit: _InvalidInput
    ) -> None:
        r"""Evidence ID: SV-HUME-004

        Requirement: ``None``, each Boolean value, integers, and arbitrary objects are
        rejected
        independently in both unit roles, with a diagnostic identifying the invalid
        role.

        Method: Pass one value per parameter through ``_InvalidInput``/``cast`` only at
        the deliberate
        invalid
        ...
        Acceptance: Every independently collected case raises exactly ``TypeError``
        naming
        either
        ``analyzer energy unit`` or ``record energy unit`` as selected.

        Interpretation: Passing establishes independent role and invalid-family
        validation
        without coercion.

        Limitations: Empty and equal strings are correctly typed invariant failures
        tested
        separately. No
        ...
        invalid_constructor = HermiticityUnitMismatchError
        expected_fragment = f"{role} energy unit"

        with pytest.raises(TypeError) as exc_info:
            if role == "analyzer":
                invalid_constructor(invalid_unit, "hartree")  # type: ignore[arg-type]
            else:
                invalid_constructor("eV", invalid_unit)  # type: ignore[arg-type]
    ...
    def test_constructor__empty_unit_strings_are_rejected__is_enforced(
        ...
        Requirement: Both role-specific unit strings must be nonempty; an empty
        correctly
        typed string
        violates the invariant rather than the type boundary.

        Method: Supply ``""`` independently in each role while the opposite role is a
        valid
        nonempty
        string.

        Oracle: The approved public taxonomy requires ``ValueError`` and a diagnostic
        naming
        the
        empty role.

        Acceptance: Each case raises exactly ``ValueError`` with its role-specific
        fragment.
    ...
    def test_field__exact_mismatch_semantics_and_equal_unit_rejection__is_exact(
        ...
        Method: Select two equal pairs and one case-only differing pair independently of
        the
        ...
        Acceptance: Equal pairs raise exactly ``ValueError`` identifying the need to
        differ;
        the
        ...
        Limitations: Approximate equality, conversion, registry lookup, Analyzer
        execution,
        numerical
    ...
    def test_constructor__input_boundary__free_form_reason_and_extra_argument_are(
        ...
        Oracle: The approved two-parameter signature and structured role fields define
        no
        free-form
        reason parameter or attribute.

        Acceptance: Both extra-argument forms raise exactly ``TypeError`` and ``reason``
        is
        absent from
        a valid instance.

        Interpretation: Passing prevents arbitrary text from competing with the ordered
        unit
        fields as
        ...
        if reason_form == "positional":
            with pytest.raises(TypeError):
                invalid_constructor("eV", "hartree", "synthetic reason")  # type: ignore[call-arg]
        else:
            with pytest.raises(TypeError):
                invalid_constructor("eV", "hartree", reason="synthetic reason")  # type: ignore[call-arg]
    ...
    def test_method__serialize__exception_has_no_serialization_api() -> None:
        ...
        Limitations: Pickling and future schemas are unspecified. No Rust serialization
        or
        conformance,
        Analyzer execution, numerical verification, scientific validation, or
        uncertainty
