...
class TestOperatorRecordCompatibilityResult:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "reference_identifier", True, TypeError, id="reference_boolean"
            ),
            ...
            pytest.param(
                "candidate_identifier", True, TypeError, id="candidate_boolean"
            ),
        ],
    )
    @staticmethod
    def test_constructor__enforce_identifier_invariants__is_enforced(
        ...
        Requirement: Each identifier is an independently validated nonempty string;
        wrong
        semantic types
        ...
        Oracle: The approved source contract documents field-specific identifier
        diagnostics
        and the
        ...
        with pytest.raises(expected_exception) as exc_info:
            OperatorRecordCompatibilityResult(**kwargs)  # type: ignore[arg-type]
    ...
    def test_method__require__require_exact_builtin_tuple_for_issues(
        ...
        Interpretation: Passing establishes container-type precedence over element
        checks.

        Limitations: No Issue or Result is used as a set member or dictionary key, so
        this
        evidence
        ...
        with pytest.raises(TypeError) as exc_info:
            OperatorRecordCompatibilityResult("reference", "candidate", invalid_issues)  # type: ignore[arg-type]
    ...
    def test_method__require__require_public_compatibility_issue_elements(
        invalid_issue: _InvalidInput,
    ) -> None:
        r"""Evidence ID: SV-ORCAR-008

        Requirement: Every exact-tuple element is an
        ``OperatorRecordCompatibilityIssue``;
        codes and raw
        ...
        Oracle: The approved element validation raises ``TypeError`` naming the public
        Issue
        type.
        ...
        with pytest.raises(TypeError) as exc_info:
            OperatorRecordCompatibilityResult(
                "reference",
                "candidate",
                (invalid_issue,),  # type: ignore[arg-type]
            )
    ...
    def test_field__reject_duplicated_mismatch_codes_from_distinct__is_exact() -> None:
        ...
        first = TestOperatorRecordCompatibilityResult.make_issue(code)
        second = TestOperatorRecordCompatibilityResult.make_issue(code)
    ...
    def test_field__reject_noncanonical_issue_ordering__is_exact() -> None:
        ...
        Interpretation: Passing complements positive canonical admission under ``the
        owning
        evidence``.
