...
class TestOperatorRecord:
    ...
    def test_constructor__invalid_matrix_rank_is_rejected__is_enforced(
        ...
        invalid
        public boundary using ``_InvalidInput`` only there.
        ...
        Interpretation: Passing establishes stable rank rejection before matrix
        analysis.
        ...
        with pytest.raises(ValueError, match="two-dimensional"):
            OperatorRecordFixtureFactory.make_record(matrix)  # type: ignore[arg-type]
    ...
    def test_constructor__nonsquare_two_dimensional_matrices_are__is_enforced(
        ...
        Limitations: No norm, Hermiticity, scientific validation, UQ, or Rust
        conformance is
        established.
        """
    ...
    def test_constructor__input__ragged_nested_sequences_have_stable_public(
        ...
        Requirement: Nested rows form a rectangular two-dimensional array before
        conversion.
        ...
        Acceptance: ``ValueError`` identifies non-ragged rectangular form; backend
        coercion
        wording is
        not exposed.
        ...
        Limitations: It does not duplicate serializer malformed-payload evidence,
        scientific
        validation,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(_ArbitraryInput(), id="sv_or_011_arbitrary_object"),
        ],
    )
    @staticmethod
    def test_constructor__invalid_matrix_scalar_wrong_types_are__is_enforced(
        invalid_entry: _InvalidInput,
    ) -> None:
        r"""Evidence ID: SV-OR-011

        Requirement: Entries are approved real/complex numeric scalars; Boolean,
        textual,
        null, and
        arbitrary values are not numbers for this contract.

        Method: Place each invalid value in one entry of an otherwise valid nested list
        and
        use
        ``_InvalidInput`` only at the invalid constructor boundary.

        Oracle: The approved scalar taxonomy requires ``TypeError`` and numeric-scalar
        wording.

        Acceptance: Every case raises ``TypeError`` identifying matrix entries and
        numeric
        scalar
        semantics.
    ...
    def test_constructor__nonfinite_real_components_are_rejected__is_enforced(
        ...
        Oracle: IEEE finiteness of the explicitly selected real component is independent
        of
        ...
        Limitations: It computes no norm or physical metric and establishes no
        scientific
        validation, UQ,
        or Rust conformance.
        """
        ...
        with pytest.raises(ValueError, match="components must be finite"):
            OperatorRecordFixtureFactory.make_record([[nonfinite_real, 0], [0, 1]])
    ...
    def test_constructor__nonfinite_imaginary_components_are_rejected__is_enforced(
        ...
        Requirement: Every stored imaginary component is finite even when its real
        component
        is finite.
        ...
        with pytest.raises(ValueError, match="components must be finite"):
            OperatorRecordFixtureFactory.make_record([[nonfinite_imaginary, 0], [0, 1]])
    ...
    def test_constructor__complex128_conversion_overflow__raises_value_error(
        ...
        Method: Construct with huge signed integers, then with signed maximum finite
        floats
        while
        promoting RuntimeWarning to an error.

        Oracle: Python arbitrary-precision magnitude and ``np.finfo(float64).max``
        define
        overflow
        ...
        Interpretation: Passing establishes storage-range policy without a norm
        calculation.
        ...
        with pytest.raises(ValueError, match="finite complex128"):
            OperatorRecordFixtureFactory.make_record([[huge_integer, 0], [0, 1]])

    @staticmethod
    def test_constructor__finite_binary64_extremes__are_admitted_without_warning() -> (
        None
    ):
        ...
        Limitations: Later algorithm overflow, physical meaning, validation, UQ, and
        Rust
        are excluded.
        """
    ...
    @staticmethod
    def test_constructor__matrix_dimension_must_match_state_space__is_enforced() -> (
        None
    ):
        ...
        Acceptance: Field-specific ``ValueError`` identifies matrix/state-space
        dimension.
        ...
        with pytest.raises(ValueError, match="matrix dimension.*state-space dimension"):
            OperatorRecordFixtureFactory.make_record(
                np.eye(3),
                state_space=OperatorRecordFixtureFactory.make_state_space(dimension=2),
                basis=OperatorRecordFixtureFactory.make_basis(ordering=("a", "b", "c")),
            )
    ...
    def test_constructor__basis_ordering_length_must_match_state__is_enforced() -> None:
        ...
        Acceptance: Field-specific ``ValueError`` identifies basis ordering and state
        space.

        Interpretation: Passing establishes index-order metadata agreement.

        Limitations: It does not mutate dependencies, test Basis intrinsic invariants,
        or
        establish
        ...
        with pytest.raises(ValueError, match="basis ordering length.*state-space"):
            OperatorRecordFixtureFactory.make_record(
                [[1, 0], [0, 1]],
                state_space=OperatorRecordFixtureFactory.make_state_space(dimension=2),
                basis=OperatorRecordFixtureFactory.make_basis(ordering=("a",)),
            )
    ...
    def test_constructor__operator_record_requires_independently__is_enforced() -> None:
        ...
        Requirement: OperatorRecord requires ``basis.orthonormal is True`` while
        standalone
        Basis permits
        ``False``.

        Method: Construct an ordinary valid two-label Basis carrying ``False`` and pass
        it
        with
        ...
        Acceptance: Basis construction succeeds; record construction raises
        field-specific
        ``ValueError``.
        ...
        Limitations: It performs no numerical orthogonality proof, scientific
        validation,
        UQ, or Rust
        conformance.
        """

        basis = OperatorRecordFixtureFactory.make_basis(orthonormal=False)
        assert basis.orthonormal is False

        with pytest.raises(ValueError, match="orthonormal basis"):
            OperatorRecordFixtureFactory.make_record(basis=basis)
