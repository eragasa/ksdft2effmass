...
def make_result(
    *,
    ...
    Requirement: Equality and immutability fixtures use only values intended to satisfy
    the public
    semantic types and pass each field unchanged.

    Method: Call the public three-field constructor with typed keyword arguments.

    Oracle: The approved ResultObject contract defines the three stored fields and their
    canonicalization.

    Acceptance: A valid public synthetic ResultObject is returned.

    Interpretation: The helper supplies independently constructible exact value states.
...
def test_field__stored_state_is_frozen_and_slotted__is_exact(
    ...
    Requirement: Stored dataclass fields are exactly ``residual``, ``tolerance``, and
    ``energy_unit``; no instance dictionary exists; ordinary assignment to each field is
    forbidden; ``is_hermitian`` is not stored state.

    Method: Inspect public dataclass metadata, inspect the instance dictionary boundary,
    and
    ...
    Acceptance: Field names match exactly, ``__dict__`` is absent, ``is_hermitian`` is
    not a field,
...
def test_method__eq__exact_structural_equality_uses_all_stored_fields() -> None:
    ...
    Method: Construct one baseline, one identical value, and three single-field variants
    and
    ...
    Acceptance: The identical object compares equal and each independently varied object
    compares
    unequal.

    Interpretation: Passing establishes exact ResultObject value semantics, not object
    identity or
