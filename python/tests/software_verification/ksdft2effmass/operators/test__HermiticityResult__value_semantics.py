...
class TestHermiticityResult:
    ...
    def make_result(
        *,
        ...
        Requirement: Equality and immutability fixtures use only values intended to
        satisfy
        the public
        ...
        Limitations: It performs no matrix analysis, unit conversion, numerical
        verification, scientific
        validation, UQ, or Rust-conformance check and establishes no scientific
        validity.
        """
    ...
    def test_field__stored_state_is_frozen_and_slotted__is_exact(
        ...
        r"""Evidence ID: SV-HR-014

        Requirement: Stored dataclass fields are exactly ``residual``, ``tolerance``,
        and
        ``energy_unit``; no instance dictionary exists; ordinary assignment to each
        field is
        forbidden; ``is_hermitian`` is not stored state.

        Method: Inspect public dataclass metadata, inspect the instance dictionary
        boundary,
        and
        ...
        Acceptance: Field names match exactly, ``__dict__`` is absent, ``is_hermitian``
        is
        not a field,
        and each assignment raises exactly ``FrozenInstanceError``.

        Interpretation: Passing establishes API-level immutable slotted stored state.

        Limitations: No invariant bypass, ``object.__setattr__``, derived-property
        assignment,
        hashability, Analyzer behavior, numerical verification, scientific validation,
        UQ,
    ...
    def test_method__eq__exact_structural_equality_uses_all_stored_fields() -> None:
        r"""Evidence ID: SV-HR-015

        Requirement: Independently constructed objects with identical canonical stored
        fields compare
        equal; changing residual, tolerance, or unit independently makes them unequal.

        Method: Construct one baseline, one identical value, and three single-field
        variants
        and
        ...
        Acceptance: The identical object compares equal and each independently varied
        object
        compares
        unequal.
        ...
        Limitations: Approximate equality, unrelated-object comparison, hash behavior,
        physical
        equivalence, Analyzer accuracy, scientific validation, UQ, and Rust conformance
        are
        unspecified or untested.
        """

        first = TestHermiticityResult.make_result()
        identical = TestHermiticityResult.make_result()
        different_residual = TestHermiticityResult.make_result(residual=3.0e-13)
        different_tolerance = TestHermiticityResult.make_result(tolerance=2.0e-12)
        different_energy_unit = TestHermiticityResult.make_result(energy_unit="Ha")
