...
class TestOperatorRecordCompatibilityAnalyzer:
    ...
    def make_record(
        *,
        ...
        ``dimension =
        len(basis_ordering)`` couples state-space dimension to the valid ordering;
        supplied
        matrices explicitly match that dimension.

        Oracle: Public record construction defines intrinsic validity. Analyzer-critical
        fields are
        dimension, state-space and operator kinds, ordered labels and basis kind, cell
        and
        geometry conventions/units, and energy zero/unit.

        Acceptance: Public construction returns an intrinsically valid record without
        broad
        coercion,
        mutation, or invariant bypass.

        Interpretation: A returned record can reach Analyzer findings without
        frozen-object
        mutation or
        invariant bypass.
        ...
        provenance are
        deliberately ignored fields. All matrices and metadata are synthetic: they come
        from
        no DFT, Wannierization, experiment, or impurity extraction and establish no
        physical
        equivalence, scientific validation, or uncertainty quantification. Notes -----
        The
        helper performs no broad test-side coercion. Explicitly supplied empty
        provenance is
    ...
    def issue_codes(
        ...
        Acceptance: Output codes exactly preserve the public Issue tuple's existing
        order.

        Interpretation: The tuple exposes exactly the order returned by ``execute()``.

        Limitations: This projection neither executes rules nor independently defines
        their
        canonical
    ...
    def make_rule_candidate(
        ...
        Requirement: Every public mismatch code must be reachable from independently
        valid
        records, with
        coupled findings retained where validity requires them.

        Method: Select an explicit public-constructor variation for the requested code;
        no
        mutation,
        invariant bypass, broad coercion, set, or dictionary drives rule execution or
        ordering.

        Oracle: The approved Analyzer rule inventory specifies each isolated variation
        and
        the
        ...
        acceptability of subtraction, scientific validation, or uncertainty
        quantification.
        """
        ...
        match code:
            ...
            case code_type.OPERATOR_KIND_MISMATCH:
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
                    identifier="candidate", operator_kind="different"
                )
            case code_type.ORDERED_BASIS_LABELS_MISMATCH:
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
                    identifier="candidate", basis_ordering=("b", "a")
                )
            case code_type.BASIS_KIND_MISMATCH:
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
                    identifier="candidate", basis_kind="different basis"
                )
            ...
            case code_type.BOUNDARY_CONDITIONS_MISMATCH:
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
                    identifier="candidate", boundary_conditions="open"
                )
            ...
            case code_type.GEOMETRY_LENGTH_UNIT_MISMATCH:
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
                    identifier="candidate", length_unit="bohr"
                )
            case code_type.ENERGY_UNIT_MISMATCH:
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
                    identifier="candidate", energy_unit="hartree"
                )
    ...
    def test_method__execute__every_public_mismatch_rule_is_reachable_from_valid(
        ...
        coupled
        ordered-label finding because valid records require matrix dimension =
        state-space
        dimension = basis-ordering length; remaining cases change only one critical
        field
        ...
        Oracle: The parameter table is the approved rule-to-finding contract, including
        the
        ...
        Limitations: Exact synthetic metadata mismatches are software evidence only;
        they do
        not assess
        ...
        candidate = TestOperatorRecordCompatibilityAnalyzer.make_rule_candidate(code)

        result = OperatorRecordCompatibilityAnalyzer().execute(reference, candidate)

        assert (
            TestOperatorRecordCompatibilityAnalyzer.issue_codes(result)
            == expected_codes
        )
    ...
    def test_method__execute__orders_all_mismatches() -> None:
        ...
        Acceptance: Ordered issue codes equal the enum tuple and observed membership
        equals
        the set of
        all public enum members.

        Interpretation: Passing establishes deterministic public audit order and
        complete
        code reachability;
        set equality is coverage only and does not drive ordering.

        Limitations: The candidate remains intrinsically valid but synthetic. This does
        not
        establish
        physical equivalence, alignment, scientific validation, or uncertainty
        quantification.
        ...
        result = OperatorRecordCompatibilityAnalyzer().execute(reference, candidate)
        ordered_codes = TestOperatorRecordCompatibilityAnalyzer.issue_codes(result)
