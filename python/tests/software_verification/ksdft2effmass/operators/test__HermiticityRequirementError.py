...
class TestHermiticityRequirementError:
    ...
    def make_failed_result(
        *,
        ...
        Acceptance: The returned public result has ``is_hermitian`` false for the
        defaults.

        Interpretation: The result is deterministic synthetic software evidence; the
        helper
        performs no
        Hermiticity analysis.

        Limitations: No matrix, DFT record, Wannier representation, or physical operator
        is
        involved. The
    ...
    def test_constructor__public_construction_and_exception_taxonomy__is_enforced() -> (
        ...
        result = TestHermiticityRequirementError.make_failed_result()
    ...
    @staticmethod
    def test_field__represented_state__exact_failed_result_identity_is_retained() -> (
        None
    ):
        ...
        result = TestHermiticityRequirementError.make_failed_result(
            residual=2.5, tolerance=0.25, energy_unit="meV"
        )
    ...
    def test_protocol__str__human_readable_hermiticity_failure_summary() -> None:
        ...
        matrix is not
        Hermitian within tolerance while ``error.result`` remains the programmatic
        evidence.

        Method: Construct a failed result with distinct readable residual, tolerance,
        and
        unit, then
        ...
        Acceptance: The stable semantic phrase occurs in ``str(error)`` and the exact
        result
        remains
        retained.
        ...
        Limitations: Punctuation, capitalization, separators, float formatting, and
        numeric
        duplication
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(_ArbitraryInput(), id="object_wrong_type"),
        ],
    )
    @staticmethod
    def test_constructor__invalid_result_types_are_rejected__is_enforced(
        ...
        Requirement: Only ``HermiticityResult`` instances are accepted; ``None``,
        strings,
        both Boolean
        values, and arbitrary objects are wrong semantic types.

        Method: Pass each representative value at a deliberate invalid constructor
        boundary,
        using
        ``_InvalidInput`` and ``cast`` only at that boundary.

        Oracle: The public constructor documents ``TypeError`` and the stable owner-type
        diagnostic
        fragment ``HermiticityResult``.

        Acceptance: Every invalid value raises exactly ``TypeError`` and names the
        required
        owner type.

        Interpretation: Passing establishes wrong-type taxonomy without coercion.

        Limitations: Correctly typed successful Results are a separate invariant family.
        No
        Analyzer,
        ...
        with pytest.raises(TypeError) as exc_info:
            HermiticityRequirementError(invalid_result)  # type: ignore[arg-type]
    ...
    def test_constructor__successful_results_are_rejected__is_enforced(
        ...
        Requirement: Results satisfying ``residual <= tolerance`` are successful and
        cannot
        form
        ...
        Oracle: The approved public property defines the inclusive acceptance boundary;
        the
        exception contract requires a failed Result.

        Acceptance: Every Result reports success, then raises exactly ``ValueError``
        with
        the stable
        ...
        Limitations: The test does not recompute a residual, choose a physical
        tolerance, or
        execute the
    ...
    def test_constructor__input_boundary__free_form_reason_and_extra_argument_are(
        ...
        Method: Invoke the constructor through an ``_InvalidInput``-typed
        invalid-signature boundary
        for each
        ...
        invalid_constructor = HermiticityRequirementError

        if reason_form == "positional":
            with pytest.raises(TypeError):
                invalid_constructor(  # type: ignore[call-arg]
                    TestHermiticityRequirementError.make_failed_result(),
                    "synthetic reason",
                )
        else:
            with pytest.raises(TypeError):
        ...
        error = HermiticityRequirementError(
            TestHermiticityRequirementError.make_failed_result()
        )
    ...
    def test_method__serialize__exception_has_no_serialization_api() -> None:
        ...
        Limitations: Pickling and future schemas are unspecified. Future Rust error
        mapping
        is conceptual
