...
class TestStateSpace:
    ...
    def make_state_space(
        *,
        ...
        Requirement: Valid fixtures use explicit typed fields and pass them unchanged to
        the
        public
        constructor.
        ...
        storage, and
        establishes no physical or scientific validity, scientific validation,
        uncertainty
    ...
    def test_constructor__public_fields_are_mapped_exactly__is_enforced() -> None:
        ...
        Interpretation: Passing establishes supported construction and stored-field
        mapping.

        Limitations: It does not establish basis or matrix compatibility, physical
        meaning,
        scientific
    ...
    def test_constructor__accepted_dimension_scalars_canonicalize_to__is_enforced(
        dimension: int | np.int32 | np.int64,
    ) -> None:
        r"""Evidence ID: SV-SS-002

        Requirement: Python and NumPy integer scalars are admitted with value
        preservation
        and canonical
        ...
        Oracle: The approved runtime and typing contracts admit these integer families
        and
        require
        canonical stored ``int`` state.

        Acceptance: Stored dimension equals ``int(dimension)`` and has exact type
        ``int``.
        ...
        Limitations: It does not approve Boolean semantics or every third-party
        integer-like
        protocol and
    ...
    def test_field__arbitrary_positive_structural_dimension_has_no__is_exact() -> None:
        ...
        Oracle: The intrinsic contract treats dimension as structural metadata; matrix
        and
        basis
        constraints remain separate ``OperatorRecord`` invariants.

        Acceptance: The arbitrary-precision value is preserved exactly as built-in
        ``int``.

        Interpretation: Passing establishes that construction itself adds no dimension
        cap
        or vector/matrix
        allocation.

        Limitations: It does not promise that an ``OperatorRecord`` matrix of this
        dimension
        can be
        ...
        dimension = 10**1000
        state_space = TestStateSpace.make_state_space(dimension=dimension)

        assert state_space.dimension == dimension
        assert type(state_space.dimension) is int

    @staticmethod
    def test_field__represented_state__valid_strings_are_preserved_exactly_without() -> (  # noqa: E501
        None
    ):
        ...
        Interpretation: Passing establishes preservation of these valid descriptive
        strings.

        Limitations: It does not approve every possible semantic label or
        whitespace-only
        metadata and
        establishes no physical validity, scientific validation, UQ, or Rust
        conformance.
        """

        identifier = "State-Space-A"
        kind = "Finite Synthetic"
        state_space = TestStateSpace.make_state_space(identifier=identifier, kind=kind)

        assert state_space.identifier == identifier
        assert state_space.kind == kind

    @staticmethod
    def test_method__serialize__state_space_has_no_standalone_serialization_api() -> (
        None
    ):
        r"""Evidence ID: SV-SS-005

        Requirement: Neither instance nor class exposes the six unapproved standalone
        JSON,
        dictionary,
        ...
        Interpretation: Passing establishes the current nested-only serialization
        boundary.

        Limitations: Pickling and future approved schemas are unspecified. No record
        round
        trip,
