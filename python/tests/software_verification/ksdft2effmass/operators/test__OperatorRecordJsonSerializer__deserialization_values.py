...
class TestOperatorRecordJsonSerializer:
    ...
    @staticmethod
    def mutated(path: tuple[str | int, ...], value: _JsonValue) -> str:
        r"""Evidence ID: Owns no identifier; supports evidence in this module.

        Requirement: Value-deserialization cases require deterministic replacement of
        one
        declared JSON
        ...
        """
        payload: _JsonValue = OperatorJsonTestDecoder.decode_object(
        ...
        if len(path) > 1:
            ...
            if isinstance(first_key, str):
                ...
                target = target[first_key]
            else:
                assert isinstance(target, list)
                target = target[first_key]
        if len(path) > 2:
            second_key = path[1]
            if isinstance(second_key, str):
                assert isinstance(target, dict)
                target = target[second_key]
            else:
                assert isinstance(target, list)
                target = target[second_key]
        if len(path) > 3:
    ...
    def raw_number(path: tuple[str | int, ...], token: str) -> str:
        ...
        Requirement: Overflow evidence requires inserting one raw JSON numeric token
        without
        accidental
        ...
        cross-language agreement.
        """
        return TestOperatorRecordJsonSerializer.mutated(path, "__RAW__").replace(
            '"__RAW__"', token
        )
    ...
    def test_method__deserialize__booleans_are_not_numeric_values(
        ...
        with pytest.raises(TypeError, match="integer|real number"):
            OperatorRecordJsonSerializer().deserialize(
                TestOperatorRecordJsonSerializer.mutated(path, True)
            )
    ...
    def test_method__deserialize__numeric_strings_are_not_numbers(
        ...
        with pytest.raises(TypeError, match="integer|real number"):
            OperatorRecordJsonSerializer().deserialize(
                TestOperatorRecordJsonSerializer.mutated(path, "1")
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ("matrix", 0, 0, 1), "positive", id="positive_matrix_imaginary"
            ),
            pytest.param(
            ...
            pytest.param(
                ("matrix", 0, 0, 1), "negative", id="negative_matrix_imaginary"
            ),
            pytest.param(
    ...
    def test_constructor__numeric_overflow_maps_to_finite_value_error__is_enforced(
        ...
        Method: Invoke serialize() or deserialize() on the explicit schema-version-1
        partition
        (numeric overflow maps to finite value error: is enforced); warnings and
        coercive
        ...
        with pytest.raises(ValueError, match="finite"):
            OperatorRecordJsonSerializer().deserialize(
                TestOperatorRecordJsonSerializer.raw_number(path, token)
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ("basis", "ordering"), "a", TypeError, id="ordering_container"
            ),
            pytest.param(
                ("basis", "ordering"), ["a", 2], TypeError, id="ordering_label"
            ),
            pytest.param(("basis", "orthonormal"), 1, TypeError, id="orthonormal_type"),
            pytest.param(
                ("provenance",), {"source": 1}, TypeError, id="provenance_value"
            ),
            pytest.param(
    ...
    def test_method__deserialize__json_scalar_and_container_semantics(
        path: tuple[str | int, ...], value: _JsonValue, expected: type[Exception]
    ) -> None:
        ...
        Method: Invoke serialize() or deserialize() on the explicit schema-version-1
        partition
        (deserialize: json scalar and container semantics); warnings and coercive
        fallback
        behavior are not accepted.
        ...
        with pytest.raises(expected):
            OperatorRecordJsonSerializer().deserialize(
                TestOperatorRecordJsonSerializer.mutated(path, value)
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ("basis", "ordering"), ["a", "a"], "unique", id="duplicate_label"
            ),
            pytest.param(
                ("basis", "orthonormal"),
                False,
                "orthonormal",
                id="nonorthogonal_record",
            ),
            pytest.param(
                ("geometry", "cell"),
                [[1, 0, 0], [2, 0, 0], [3, 0, 0]],
    ...
    def test_method__deserialize__public_dataobject_invariants_propagate(
        path: tuple[str | int, ...], value: _JsonValue, message: str
    ) -> None:
