...
class TestOperatorRecordCompatibilityAnalyzer:
    ...
    def make_record(
        *,
        ...
        Requirement: Fixtures must satisfy every intrinsic record, state-space, basis,
        cell,
        ...
        Oracle: The public ``OperatorRecord`` construction contract defines those
        intrinsic
        ...
        Acceptance: Public construction succeeds and returns an intrinsically valid
        record
        without
        ...
        impurity extraction. Construction proves neither physical equivalence nor
        scientific
        validity. It performs no scientific validation or uncertainty quantification.
        Notes
    ...
    def issue_codes(
        ...
        Limitations: This helper performs no compatibility analysis and does not
        establish
        the enum's
        canonical order independently, scientific validation, or uncertainty
        quantification.
        """
    ...
    def make_ignored_metadata_candidate(variation: str) -> OperatorRecord:
        ...
        Acceptance: Construction returns a valid candidate with the requested isolated
        or
        explicitly
        combined ignored-field variation.

        Interpretation: Each returned candidate isolates one ignored-field boundary,
        except
        the explicitly
        named combined case.

        Limitations: Compatibility of these synthetic records establishes
        subtractability
        only, not
        ...
        match variation:
            case "operator-record-identifier":
                return TestOperatorRecordCompatibilityAnalyzer.make_record(
    ...
    @staticmethod
    def test_method__execute__execute_constructs_complete_compatible_audit_result() -> (
        None
    ):
        r"""Evidence ID: SV-ORCA-002

        Requirement: ``execute()`` returns the public compatible audit with input roles
        and
        the complete
        canonical applied-rule sequence.

        Method: Analyze two independently constructed records with equal critical
        metadata
        and
        ...
        reference = TestOperatorRecordCompatibilityAnalyzer.make_record(
            identifier="reference-role"
        )
        candidate = TestOperatorRecordCompatibilityAnalyzer.make_record(
    ...
    def test_field__ignored_identity_descriptive_and_provenance_metadata__is_exact(
        ...
        Requirement: Record, state-space, and basis identifiers, geometry-system text,
        and
        provenance do
        ...
        Interpretation: Passing distinguishes representation compatibility from identity
        and
        provenance
        equality.

        Limitations: Ignoring a field does not prove a common physical system,
        equivalent
        DFT or Wannier
        provenance, scientific acceptability of subtraction, or scientific validation;
        no
        ...
        reference = TestOperatorRecordCompatibilityAnalyzer.make_record()
        candidate = (
            TestOperatorRecordCompatibilityAnalyzer.make_ignored_metadata_candidate(
                variation
            )
        )
    ...
    @staticmethod
    def test_method__require__require_returns_value_equivalent_compatible_audit() -> (
        None
    ):
        r"""Evidence ID: SV-ORCA-016

        Requirement: ``require()`` returns a compatible audit equal to ``execute()``
        output.
        ...
        Limitations: No incompatible branch, physical equivalence, scientific
        validation, or
        uncertainty
    ...
    @staticmethod
    def test_method__require__require_raises_error_retaining_complete_incompatible() -> (  # noqa: E501
        None
    ):
        ...
        Acceptance: Error type, retained value, exact code, and role identifiers all
        match.

        Interpretation: Passing establishes Analyzer-to-error structured propagation.

        Limitations: It does not retest the exception constructor's complete invariant
        set
        or perform
        ...
        candidate = TestOperatorRecordCompatibilityAnalyzer.make_record(
            identifier="candidate-role", energy_unit="hartree"
        )
        execute_result = analyzer.execute(reference, candidate)
        ...
        assert TestOperatorRecordCompatibilityAnalyzer.issue_codes(retained) == (
    ...
    def test_method__execute__execute_rejects_each_invalid_public_input(
        ...
        Method: Create valid records inside each case and use ``_InvalidInput``/``cast``
        only to cross
        the
        ...
        reject before
        Analyzer execution, or perform scientific validation or uncertainty
        quantification.
        """
        ...
        with pytest.raises(TypeError) as exc_info:
            if invalid_role == "reference":
                analyzer.execute(_ArbitraryInput(), candidate)  # type: ignore[arg-type]
            else:
                analyzer.execute(reference, _ArbitraryInput())  # type: ignore[arg-type]
    ...
    def test_method__require__require_rejects_each_invalid_public_input(
        ...
        Requirement: ``require()`` independently documents and enforces both record
        inputs.

        Method: Create valid records inside each case and cross only the selected public
        type
        boundary with deliberate ``_InvalidInput``/``cast`` usage.
        ...
        Interpretation: Passing establishes ``require()`` as an independently tested
        public
        API.

        Limitations: The test makes no assertion about internal delegation to
        ``execute()``
        and performs
        ...
        with pytest.raises(TypeError) as exc_info:
            if invalid_role == "reference":
                analyzer.require(_ArbitraryInput(), candidate)  # type: ignore[arg-type]
            else:
                analyzer.require(reference, _ArbitraryInput())  # type: ignore[arg-type]
