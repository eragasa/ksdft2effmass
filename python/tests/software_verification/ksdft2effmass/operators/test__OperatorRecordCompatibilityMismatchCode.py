r"""Software verification for the public compatibility mismatch-code enum.
...
class ExpectedMismatchCodeContract(NamedTuple):
    """One immutable row of the independently specified public enum contract.
...
def test_exact_public_names_values_and_canonical_ordering() -> None:
    """SV-OCMC-001: verify exact public names, values, and canonical order.

    Requirement
        Enum iteration must equal the approved ordered name/value sequence
        without extra, missing, or reordered members.
    Method
    ...
    Limitations
        This does not establish rule reachability or analyzer behavior. Order is
        public because compatibility results and analyzers use
...
@pytest.mark.parametrize(
    "expected",
    [pytest.param(row, id=row.value) for row in EXPECTED_CONTRACT],
)
def test_python_strenum_and_machine_string_behavior(
    expected: ExpectedMismatchCodeContract,
) -> None:
    """SV-OCMC-002: verify ``StrEnum`` and stable machine-string behavior.

    Requirement
        The enum must subclass ``enum.StrEnum`` and every value must be a
        nonempty ASCII string satisfying ``^[a-z][a-z0-9_]*$``.
    Method
        Resolve each approved name and inspect inheritance, string compatibility,
        exact value type, ``str()`` behavior, ASCII encoding, and ``re.fullmatch``.
    Acceptance
        Every property holds for every expected member.
    Interpretation
        Passing verifies the Python representation needed for deterministic
...
def test_name_and_value_lookup_round_trips(
    expected: ExpectedMismatchCodeContract,
) -> None:
    """SV-OCMC-003: verify deterministic name and value lookup round trips.

    Requirement
        Public construction by stable value and subscription by public name must
        return the same canonical enum singleton.
    Method
        Resolve the expected member by name, then perform both documented public
        lookup forms.
    Acceptance
...
def test_unique_values_and_absence_of_enum_aliases() -> None:
    """SV-OCMC-004: verify unique values and explicit absence of enum aliases.

    Requirement
        The complete documented ``Enum.__members__`` mapping must contain only
        the expected ordered public names, with no aliases, and values are unique.
    Method
        Compare member-map and iteration counts, compare ordered member-map names
        with the independent contract, and compare value count with set size.
    Acceptance
        Counts match, names match exactly, and every stable value is unique.
    Interpretation
        Passing closes the alias gap left by normal enum iteration, which omits
        aliases.
    Limitations
...
def test_canonical_public_descriptions(
    expected: ExpectedMismatchCodeContract,
) -> None:
    """SV-OCMC-005: verify approved canonical public descriptions.
    ...
    Method
        Compare two public property reads with the independent description row
        and inspect exact type and boundary whitespace.
    Acceptance
        Both reads equal the approved text and all string invariants hold.
    Interpretation
        Passing establishes synchronized human-facing compatibility findings;
        the enum value remains the stable machine-readable code.
    Limitations
        Descriptions are not replacements for values in serialization or
        cross-language logic and do not prove analyzer reachability.
    """
...
def test_invalid_name_and_value_lookup_failures() -> None:
    """SV-OCMC-006: verify exact invalid-lookup exception taxonomy.

    Requirement
        An unknown stable value raises ``ValueError`` and an unknown public name
        raises ``KeyError`` through standard enum lookup APIs.
    Method
        Perform one invalid value construction and one invalid name subscription.
    Acceptance
        Each operation raises exactly its intended exception category; no broad
        exception tuple or complete message match is used.
    Interpretation
