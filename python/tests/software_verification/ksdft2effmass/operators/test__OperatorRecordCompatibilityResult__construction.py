...
class TestOperatorRecordCompatibilityResult:
    ...
    @staticmethod
    def test_constructor__construct_compatible_empty_issue_result__is_enforced() -> (
        None
    ):
        ...
        Acceptance: Fields and annotations match exactly; identifiers and empty tuple
        are
        retained; the
        ...
        assert tuple(
            field.name for field in fields(OperatorRecordCompatibilityResult)
        ) == (
    ...
    def test_constructor__construct_canonical_single_and_multi_issue__is_enforced(
        ...
        Requirement: One, multiple distinct, and all public mismatch codes are admitted
        when
        represented
        ...
        Limitations: It does not establish that an analyzer can produce these sequences
        from
        independently valid records.
        """

        issues = tuple(
            TestOperatorRecordCompatibilityResult.make_issue(code) for code in codes
        )
    ...
    def test_field__represented_state__derive_complete_canonical_rules_applied(
        ...
        Requirement: ``rules_applied`` always equals the complete public enum tuple and
        is
        an exact
    ...
    def test_field__represented__derive_compatibility_only_from_issue_emptiness(
        ...
        Requirement: Empty issues imply true compatibility and every nonempty tuple
        implies
        false
        compatibility.
        ...
        Interpretation: Passing prevents independently stored contradictory
        compatibility
        state.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                {"rules_applied": CANONICAL_RULES}, id="rules_applied_keyword"
            ),
        ],
    )
    @staticmethod
    def test_constructor__derived_property_keywords__are_rejected(
        derived_kwargs: dict[str, bool | tuple[str, ...]],
    ) -> None:
        r"""Evidence ID: SV-ORCAR-005

        Requirement: Neither ``is_compatible`` nor ``rules_applied`` is constructor
        state.
        ...
        Acceptance: Every unsupported call raises ``TypeError`` without relying on
        complete
        ...
        Limitations: Interpreter diagnostic wording is not made part of the public
        contract.
        """
    ...
    def test_constructor__fourth_positional_derived_state__is_rejected() -> None:
        ...
        Requirement: Derived compatibility state cannot be supplied as a fourth
        positional
        field.

        Method: Call the public constructor with valid stored fields and one extra
        Boolean.
        ...
        Limitations: Keyword rejection, physical compatibility, validation, UQ, and Rust
        are
        excluded.
        """
        with pytest.raises(TypeError):
