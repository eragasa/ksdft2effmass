...
-----------------------------
This class-owned module owns the construction facet. Facet and contract
------------------
...
def make_result(
    *,
    ...
    Requirement
    Ordinary fixtures use only values already intended to satisfy public semantic types
    rather than disguising invalid construction behind ``Any``.
    Method
    Pass typed residual, tolerance, and energy-unit values unchanged to the public
    constructor.
    Oracle
    The approved ResultObject contract defines these three constructor fields and
    performs any documented scalar canonicalization itself.
    Acceptance
    The constructor returns a public synthetic ``HermiticityResult``.
    Interpretation
    The helper provides concise valid fixtures without performing analysis or coercion
    outside the object.
    Limitations
    It performs no matrix analysis or unit conversion and establishes no numerical
...
def test_constructor__public_construction_and_stored_field__is_enforced() -> None:
    ...
    SV-HR-001
    Requirement
    The ResultObject stores residual, tolerance, and common energy unit in their
    declared roles and canonical built-in boundary types.
    Method
    Construct directly with distinct synthetic values and inspect the three public
    fields without invoking the Analyzer.
    Oracle
    The approved ResultObject and Sphinx contracts define the field mapping and built-in
    scalar storage.
    Acceptance
    Values equal their inputs and have exact types ``float``, ``float``, and ``str``
    respectively.
    Interpretation
    Passing establishes public construction and stored-field mapping.
    Limitations
    No residual computation, tolerance suitability, physical Hermiticity, scientific
...
def test_constructor__accepted_scalar_families_canonicalize_in__is_enforced(
    ...
    SV-HR-002
    Requirement
    Representative Python integer, Python float, NumPy integer scalar, and NumPy
    floating scalar inputs are accepted independently as residual and tolerance and
    stored as built-in floats.
    Method
    Construct one result with the scalar in the residual position and a second with it
    in the tolerance position.
    Oracle
    The approved public scalar contract admits exactly these representative families and
    canonicalizes them at the ResultObject boundary.
    Acceptance
    Both independently exercised positions store exact built-in ``float`` values for
    every family.
    Interpretation
    Passing establishes documented family admission and canonical storage in both
    fields.
    Limitations
    This does not claim arbitrary numeric-protocol, Decimal, Fraction, array, complex,
...
def test_property__is_hermitian__has_inclusive_boundary(
    ...
    SV-HR-003
    Requirement
    ``is_hermitian`` is true exactly when :math:`\varepsilon_{\mathrm H}\leq\tau`;
    equality, including zero equals zero, is accepted.
    Method
    Construct synthetic results on each side of the boundary and inspect the derived
    Boolean without approximate comparison.
    Oracle
    The approved mathematical and Sphinx contracts define direct binary64 comparison
    ...
    Limitations
    It does not establish that the residual was computed correctly or that the selected
...
def test_constructor__is_hermitian_argument__is_rejected() -> None:
    ...
    SV-HR-004
    Requirement
    ``is_hermitian`` is derived rather than stored and cannot be supplied as keyword or
    extra positional constructor state; changing stored scalars changes the predicate
    naturally.
    Method
    Invoke an ``Any``-typed constructor only at deliberate invalid-signature boundaries,
    then compare valid accepted and rejected scalar states.
    Oracle
    The approved three-field constructor and derived-property contract admit no fourth
    state field.
    Acceptance
    ...
    Limitations
    Signature-generated error wording is not frozen. Analyzer behavior, tolerance
...
def test_method__serialize__result_has_no_serialization_api() -> None:
    ...
    Oracle
    The approved wire-format contract assigns ``OperatorRecordJsonSerializer`` only to
    ...
    Limitations
    Exception and Result serialization remain outside this contract; pickling and future
