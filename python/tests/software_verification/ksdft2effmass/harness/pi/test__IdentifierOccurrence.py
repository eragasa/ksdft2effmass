r"""Software verification of ``IdentifierOccurrence``.
...
Software verification of the public ``IdentifierOccurrence`` surface; no
physical model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``IdentifierOccurrence``.  Accepted H1 field/wire
contracts and read-only H3 fixtures are independent exact oracles.
...
from ksdft2effmass.harness.pi import JsonRecordDeserializer, WireRecordKind
from ksdft2effmass.harness.pi.evidence import IdentifierOccurrence

ROOT = Path(__file__).resolve().parents[6]

pytestmark = pytest.mark.software_verification
SUT = IdentifierOccurrence


def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-008

    Requirement: IdentifierOccurrence accepts the complete valid version-1 H3
    wire
    instance and is immutable.

    Method: Decode the accepted ``identifier-occurrence.json`` fixture through
    the
    ...
    result = JsonRecordDeserializer().execute(
        WireRecordKind.IdentifierOccurrence, payload
    )
    assert result.validation.status == "PASS"
    assert type(result.record) is SUT

    def exercise_value_case_65_1(value: Any) -> Any:
        assert type(value) is not list

    _ = [
        exercise_value_case_65_1(value)
        for value in (
