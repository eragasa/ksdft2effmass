r"""Software verification of ``OwnershipManifestView``.

Facet and represented meaning

Software verification of the public ``OwnershipManifestView`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``OwnershipManifestView``.  Accepted H1 field/wire contracts and
...
SUT = OwnershipManifestView


def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-009

    Requirement: OwnershipManifestView accepts the complete valid version-1 H3 wire
    instance and
    is immutable.

    Method: Decode the accepted ``ownership-manifest-view.json`` fixture through the
    ...
    """
    payload = (
        ROOT / "harness/pi/fixtures/valid/ownership-manifest-view.json"
    ).read_bytes()
    result = JsonRecordDeserializer().execute(
        WireRecordKind.OwnershipManifestView, payload
    )
