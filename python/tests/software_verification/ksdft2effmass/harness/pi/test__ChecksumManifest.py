r"""Software verification of ``ChecksumManifest``.
...
Software verification of the public ``ChecksumManifest`` surface; no physical model,
mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ChecksumManifest``.  Accepted H1 field/wire contracts and
...
SUT = ChecksumManifest


def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-014

    Requirement: ChecksumManifest accepts the complete valid version-1 H3 wire instance
    and is
    immutable.

    Method: Decode the accepted ``checksum-manifest.json`` fixture through the
    ...
    """
    payload = (ROOT / "harness/pi/fixtures/valid/checksum-manifest.json").read_bytes()
    result = JsonRecordDeserializer().execute(WireRecordKind.ChecksumManifest, payload)
