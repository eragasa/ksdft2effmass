...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-015

    Requirement: ValidationIssue accepts the complete valid version-1 H3 wire instance
    and is
    immutable.

    Method: Decode the accepted ``validation-issue.json`` fixture through the
    ...
    result = JsonRecordDeserializer().execute(WireRecordKind.ValidationIssue, payload)
...
def test_constructor__diagnostic_path_corpus__accepts_and_rejects_exactly() -> None:
    """Evidence ID: SV-HARNESS-048

    Requirement: DiagnosticPath accepts file, directory-scope, None, and NFC spellings
    and
    rejects the complete 19-case malformed corpus without normalization.

    Method: Construct ValidationIssue directly for every accepted H3 indexed spelling.

    Oracle: The accepted H1 DiagnosticPath grammar and H3 oracle index fix the
    partition.

    Acceptance: Four valid paths are retained exactly; all 19 invalid paths raise
    ValueError
    containing the exact registered issue code.

    Interpretation: Failure identifies constructor, H3 corpus, or H1 lexical-contract
    drift.

    Limitations: Paths are lexical; existence and file kind are not established.
    """
    import json
