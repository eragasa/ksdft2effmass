r"""Software verification of identifier audit command API agreement.
...
``IdentifierAuditor`` ActionObject.
...
def test_artifact__command_api__agrees_on_success_and_exact_inventory(
    ...
    direct = IdentifierAuditor().execute(
...
def test_artifact__command_request__rejects_empty_inventory(
    ...
    monkeypatch.setattr(IdentifierAuditor, "execute", fail_if_audit_runs)
    exit_status = main(
        (
            "--root",
            str(root.resolve()),
