...
def make_cli_resource_manifest() -> ResourceManifest:
    """Evidence ID
    Owns no identifier; supports command/API agreement evidence.
    Requirement
    Agreement tests require one independently valid manifest input.
    Method
    ...
    Interpretation
    Failure indicates invalid setup rather than CLI behavior.
    Limitations
...
def test_artifact__command_api__emits_equal_canonical_read_only_proposal(
    ...
    SV-HARNESS-096
    Requirement
    The stable CLI emits the same canonical manifest proposed by the public API and
    does not modify the explicit manifest file.
    Method
    Serialize a controlled manifest, invoke CLI main with explicit absolute paths,
    and compare its JSON projection with direct ActionObject and serializer output.
    Oracle
    The public ActionObject plus maintained serializer provide the independent
    command-projection oracle.
    Acceptance
    Exit is 0, status and changed IDs agree, proposed text is byte-for-byte canonical,
    and manifest input bytes remain unchanged.
    Interpretation
    Failure indicates wrapper parsing, projection, or write-boundary drift.
    Limitations
    Direct ``main`` invocation does not test shell or interpreter installation.
    """
    root = tmp_path / "root"
    root.mkdir()
    (root / "resource.txt").write_bytes(b"new bytes")
    manifest = make_cli_resource_manifest()
    serialized = SerializeJsonRecord().execute(manifest).payload
    ...
    api_result = RefreshResourceManifest().execute(request)
    assert api_result.manifest is not None
    expected = SerializeJsonRecord().execute(api_result.manifest).payload
...
def test_artifact__command_api__returns_structured_failure_without_write(
    ...
    SV-HARNESS-097
    Requirement
    Action validation failure produces exit 1, deterministic findings, no proposal,
    and no manifest-file mutation.
    Method
    Invoke the CLI for one manifest-declared file absent from the explicit root.
    Oracle
    Public ActionObject failure semantics fix the PATH.MISSING projection.
    Acceptance
    Exit is 1, proposal is null, exact finding is present, and input bytes are equal.
    Interpretation
    Failure indicates partial output, wrong exit mapping, or hidden write.
    Limitations
    Invalid JSON and command-construction errors are neighboring serializer/CLI
    partitions rather than this agreement case.
    """
    root = tmp_path / "root"
    root.mkdir()
    serialized = SerializeJsonRecord().execute(make_cli_resource_manifest()).payload
    assert serialized is not None
    manifest_path = tmp_path / "manifest.json"
    manifest_path.write_bytes(serialized)
