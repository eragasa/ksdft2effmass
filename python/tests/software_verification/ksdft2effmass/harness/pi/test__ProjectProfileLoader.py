r"""Software verification of ``ProjectProfileLoader``.
...
Software verification of the public ``ProjectProfileLoader`` surface; no physical model,
mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ProjectProfileLoader``.  Accepted H1 field/wire contracts and
...
from ksdft2effmass.harness.pi import ProjectProfileLoader

pytestmark = pytest.mark.software_verification
SUT = ProjectProfileLoader


def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    """Evidence ID: SV-HARNESS-027

    Requirement: ProjectProfileLoader is a concrete stateless ActionObject.
