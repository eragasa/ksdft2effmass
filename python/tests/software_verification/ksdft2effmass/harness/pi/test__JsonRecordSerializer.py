r"""Software verification of ``JsonRecordSerializer``.

Facet and represented meaning

Software verification of the public ``JsonRecordSerializer`` surface; no physical model,
mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``JsonRecordSerializer``.  Accepted H1 field/wire contracts and
...
from ksdft2effmass.harness.pi import JsonRecordSerializer

pytestmark = pytest.mark.software_verification
SUT = JsonRecordSerializer


def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    """Evidence ID: SV-HARNESS-025

    Requirement: JsonRecordSerializer is a concrete stateless ActionObject.
...
def test_method__execute_valid_and_invalid__returns_exact_partition() -> None:
    ...
    decoded = JsonRecordDeserializer().execute(WireRecordKind.ArtifactIdentity, payload)
