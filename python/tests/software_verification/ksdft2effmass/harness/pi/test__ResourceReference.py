r"""Software verification of ``ResourceReference``.
...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-002

    Requirement: ResourceReference accepts the complete valid version-1 H3 wire instance
    and is
    immutable.

    Method: Decode the accepted ``resource-reference.json`` fixture through the
    ...
    result = JsonRecordDeserializer().execute(WireRecordKind.ResourceReference, payload)
...
def test_constructor__self_dependency__constructs_and_deserializes() -> None:
    """Evidence ID: SV-HARNESS-060

    Requirement: A resource self-dependency is structurally representable and is not an
    intrinsic
    ResourceReference violation.

    Method: Construct a reference with its own ID in ``dependency_ids`` and publicly
    decode
    the corrected H3 self-dependency candidate.

    Oracle: The corrected H1 field contract assigns self-edge validity to
    ResourceManifestValidator, while the H3 candidate fixes the wire instance.

    Acceptance: Direct construction preserves the self-edge tuple and public
    deserialization
    returns PASS with a ResourceReference containing that same self-edge.

    Interpretation: Failure identifies constructor/decoder enforcement at the wrong
    contract layer or
    H1/H3 fixture drift.

    Limitations: Manifest acceptance is intentionally not tested here; there is no
    numerical,
    scientific-validation, UQ, physical, or Rust-conformance claim.
    """
    import json
