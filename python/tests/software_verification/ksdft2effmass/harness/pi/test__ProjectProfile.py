r"""Software verification of ``ProjectProfile``.
...
Software verification of the public ``ProjectProfile`` surface; no physical model,
mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ProjectProfile``.  Accepted H1 field/wire contracts and
...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-004

    Requirement: ProjectProfile accepts the complete valid version-1 H3 wire instance
    and is
    immutable.

    Method: Decode the accepted ``project-profile.json`` fixture through the
    caller-selected
    ...
    result = JsonRecordDeserializer().execute(WireRecordKind.ProjectProfile, payload)
