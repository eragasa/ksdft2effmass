...
def make_finding() -> HumanReviewFinding:
    """Evidence ID: Owns no identifier; supports finding evidence.

    Requirement: Finding tests require one intrinsically valid value.

    Method: Construct one finding from fixed public inputs.
...
def test_constructor__exact_value__owns_supporting_identifiers_and_text() -> None:
    """Evidence ID: ``SV-HARNESS-133``.

    Requirement: A finding retains exact issue text and defensively owns supporting
    identifiers.

    Method: Construct one complete finding and inspect every stored value and collection
    type.

    Oracle: The accepted field contract and exact tuple/string semantics are exact.

    Acceptance: Text is unchanged, identifiers preserve order, and storage uses built-in
    tuple.

    Interpretation: Failure identifies field, text, or canonical-ownership drift.

    Limitations: Supporting-observation existence is not intrinsic to this record.
    """
...
def test_field__immutability__has_exact_value_semantics() -> None:
    """Evidence ID: ``SV-HARNESS-134``.

    Requirement: Finding state is frozen with exact dataclass equality.

    Method: Compare separate equal instances and attempt severity mutation.

    Oracle: Frozen dataclass semantics are exact.

    Acceptance: Instances compare equal and mutation raises FrozenInstanceError.

    Interpretation: Failure identifies mutability or equality drift.

    Limitations: No packet relationship is exercised.
    """
    first = make_finding()
...
def test_constructor__severity__accepts_closed_vocabulary(severity: str) -> None:
    """Evidence ID: ``SV-HARNESS-135``.

    Requirement: Finding severity accepts the complete closed five-value vocabulary.

    Method: Replace one valid severity with each declared member.

    Oracle: The public severity vocabulary supplies exact expected strings.

    Acceptance: Every declared severity is retained exactly.

    Interpretation: Failure identifies vocabulary drift.

    Limitations: Severity does not determine human disposition.
    """
...
def test_constructor__fields__rejects_malformed_semantic_partitions(
    ...
    """Evidence ID: ``SV-HARNESS-136``.

    Requirement: Finding identity, severity, text, path, support, and limitation fail
    closed.

    Method: Replace one valid field with each malformed semantic partition.

    Oracle: Public identifier, vocabulary, path, uniqueness, and exact-type contracts
    are
    exact.

    Acceptance: Every partition raises the specified exception.

    Interpretation: Failure identifies intrinsic-validation or exception-taxonomy drift.

    Limitations: Unknown but well-formed supporting identifiers are checked by
    preparation.
    """
...
def test_constructor__optional_path__accepts_explicit_none_without_disposition() -> (
    ...
    """Evidence ID: ``SV-HARNESS-137``.

    Requirement: A finding may concern the whole target and stores no human disposition
    field.

    Method: Construct with path None and inspect the public dataclass field set.

    Oracle: The accepted optional-path contract and declared public fields are exact.

    Acceptance: Path is None and neither disposition nor human_decision is a field.

    Interpretation: Failure identifies optional-state or authority-boundary drift.

    Limitations: Attribute absence does not itself prove runtime noninteraction.
    """
