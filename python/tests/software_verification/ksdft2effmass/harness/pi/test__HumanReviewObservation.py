...
def make_observation() -> HumanReviewObservation:
    """Evidence ID: Owns no identifier; supports observation evidence.

    Requirement: Observation tests require one intrinsically valid value.

    Method: Construct one observation from fixed public inputs.
...
def test_constructor__exact_value__preserves_substantive_text_and_optional_fields() -> (
    ...
    """Evidence ID: ``SV-HARNESS-128``.

    Requirement: An observation retains exact built-in field values without rewriting
    substantive
    text.

    Method: Construct one complete observation and compare every field with explicit
    inputs.

    Oracle: The accepted field contract and exact string equality are independent
    oracles.

    Acceptance: Every value and built-in stored type is retained exactly.

    Interpretation: Failure identifies field mapping, type, or text-preservation drift.

    Limitations: The observed check is represented data and is not executed by
    construction.
    """
...
def test_field__immutability__has_exact_value_semantics() -> None:
    """Evidence ID: ``SV-HARNESS-129``.

    Requirement: Observation state is frozen with exact dataclass equality.

    Method: Compare separate equal instances and attempt summary mutation.
...
def test_constructor__status__accepts_closed_vocabulary(status: str) -> None:
    """Evidence ID: ``SV-HARNESS-130``.

    Requirement: Observation status accepts the complete closed four-value vocabulary.

    Method: Replace one valid observation status with each declared member.

    Oracle: The public status vocabulary supplies exact expected strings.

    Acceptance: Every declared status is retained exactly.

    Interpretation: Failure identifies vocabulary drift.

    Limitations: Status records occurrence only and is not an acceptance decision.
    """
...
def test_constructor__fields__rejects_malformed_semantic_partitions(
    ...
    """Evidence ID: ``SV-HARNESS-131``.

    Requirement: Observation identifiers, text, status, path, and optional detail fail
    closed.

    Method: Replace one valid field with each malformed semantic partition.

    Oracle: Identifier, text, root-relative path, exact-type, and closed-vocabulary
    contracts
    ...
    Limitations: Target path membership is checked by HumanReviewPreparer instead.
    """
...
def test_constructor__optional_fields__accepts_explicit_none() -> None:
    """Evidence ID: ``SV-HARNESS-132``.

    Requirement: Path and supporting detail are independently optional.

    Method: Construct an observation with both optional fields set to None.

    Oracle: The public optional-field contract fixes exact None storage.

    Acceptance: Both fields are None and all substantive fields remain unchanged.

    Interpretation: Failure identifies optional-state drift.

    Limitations: Absence of detail does not imply absence of limitations.
    """
