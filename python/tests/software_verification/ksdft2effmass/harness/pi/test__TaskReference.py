...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-011

    Requirement: TaskReference accepts the complete valid version-1 H3 wire instance and
    is
    immutable.

    Method: Decode the accepted ``task-reference.json`` fixture through the
    caller-selected
    ...
    """
    payload = (ROOT / "harness/pi/fixtures/valid/task-reference.json").read_bytes()
    result = JsonRecordDeserializer().execute(WireRecordKind.TaskReference, payload)
