...
def make_target() -> HumanReviewTarget:
    """Evidence ID
    Owns no identifier; supports target evidence.
    Requirement
    Target tests require one intrinsically valid value.
    Method
    Construct one target from fixed public inputs.
    Oracle
...
def test_constructor__exact_value__owns_tuples_and_preserves_declared_order() -> None:
    ...
    ``SV-HARNESS-123``.
    Requirement
    A target stores exact built-in values and defensively owns ordered path tuples.
    Method
    Construct from explicit tuples and inspect every public field.
    Oracle
    The accepted target field contract and built-in tuple semantics are exact.
    Acceptance
    Values are unchanged, stored collections are exact tuples, and order is preserved.
    Interpretation
    Failure identifies field, canonical ownership, or ordering drift.
    Limitations
    No path existence or Git identity is checked.
    """
...
def test_field__immutability__has_exact_value_semantics() -> None:
    ...
    ``SV-HARNESS-124``.
    Requirement
    Target state is frozen and equal targets have exact value semantics.
    Method
    Compare independently constructed targets and attempt field mutation.
    Oracle
    Frozen dataclass and exact field equality semantics define acceptance.
    Acceptance
    Equal construction compares equal and assignment raises FrozenInstanceError.
    Interpretation
    Failure identifies mutability or equality drift.
    Limitations
    Hash stability across interpreter implementations is not claimed.
    """
    first = make_target()
    second = make_target()
    assert first == second
...
def test_constructor__evidence_class__accepts_closed_vocabulary(
    ...
    ``SV-HARNESS-125``.
    Requirement
    The target accepts every and only every declared evidence-class value.
    Method
    Replace the evidence class with each semantic member of the closed vocabulary.
    Oracle
    The public five-value vocabulary is the independent exact oracle.
    Acceptance
    Every declared value is retained exactly.
    Interpretation
    Failure identifies vocabulary or storage drift.
    Limitations
    Construction does not establish evidence adequacy.
    """
    assert (
...
def test_constructor__identity_fields__rejects_malformed_values(
    ...
    ``SV-HARNESS-126``.
    Requirement
    Identity, revision, subject, and vocabulary fields fail closed by semantic type.
    Method
    Replace one otherwise valid field with each malformed semantic partition.
    Oracle
    Built-in type, identifier grammar, exact revision grammar, and closed vocabulary
    are exact public rules.
    Acceptance
    Each partition raises its specified TypeError or ValueError.
    Interpretation
    Failure identifies exception-taxonomy or lexical-validation drift.
    Limitations
    Git object existence is deliberately excluded.
    """
...
def test_constructor__root_relative_paths__rejects_invalid_partitions(
    ...
    ``SV-HARNESS-127``.
    Requirement
    Review and contract paths are normalized root-relative POSIX paths and review paths
    are nonempty and unique.
    Method
    Replace one path tuple with each independent malformed partition.
    Oracle
    The public lexical path contract supplies exact rejection behavior.
    Acceptance
    Every malformed partition raises ValueError without filesystem access.
    Interpretation
    Failure identifies path-normalization or uniqueness drift.
    Limitations
    The test makes no existence, symlink, or repository-membership claim.
    """
