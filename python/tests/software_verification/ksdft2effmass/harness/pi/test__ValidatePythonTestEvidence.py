...
def test_artifact__literal_value__equals_itself():
    ...
    SV-TEV-FIX-001
    Requirement
    The controlled literal retains exact equality.
    Method
    Compare one literal with itself.
    Oracle
    Python integer equality fixes the result.
    Acceptance
    The equality is exactly true.
    Interpretation
    Failure identifies controlled fixture drift.
    Limitations
...
def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    ...
    SV-TEV-017
    Requirement
    ValidatePythonTestEvidence is a concrete stateless ActionObject.
    Method
    Construct two instances and inspect their instance storage boundary.
    Oracle
    The accepted placement contract requires no root, filesystem, cache, or state.
    Acceptance
    Both construct, lack instance dictionaries, and the class slots are empty.
    Interpretation
    Failure identifies unauthorized retained state or contract drift.
    Limitations
    Execute semantics are covered separately.
    """
    first = SUT()
    second = SUT()
...
def test_method__execute_valid_source__returns_exact_inventory() -> None:
    ...
    SV-TEV-018
    Requirement
    Execute accepts a controlled conforming artifact-owned module and reports its
    exact static inventory.
    Method
    Supply literal source and ownership bytes directly through the public request.
    Oracle
    Manual inspection gives one module, one test, one evidence owner, no helpers or
    parameter cases, and one artifact-owned software-verification owner.
    Acceptance
    The exact status, paths, findings, owner counts, and function counts match.
    Interpretation
    Failure identifies validator, fixture, or accepted-rule drift.
    Limitations
    A structural pass does not establish semantic quality or scientific claims.
    """
...
def test_method__execute_invalid_source__retains_multiple_finding_order() -> None:
    ...
    SV-TEV-019
    Requirement
    Execute retains multiple findings in deterministic rule traversal order.
    Method
    Supply one controlled source with a nonraw opening, missing headings, malformed
    test name, and incomplete function documentation.
    Oracle
    The documented traversal checks module opening, module documentation, then the
    top-level function name and documentation in that order.
    Acceptance
    Status is FAIL and the first four codes equal the literal expected sequence.
    Interpretation
    Failure identifies rule presence, aggregation, or deterministic-order drift.
    Limitations
    This representative invalid source does not exhaust every validator code.
    """
...
def test_method__execute_metadata_contract__reports_controlled_invalidity(
    ...
    SV-TEV-020
    Requirement
    Malformed ownership and migration metadata become their stable public findings.
    Method
    Supply one correct source with a controlled malformed JSON object at each layer.
    Oracle
    The closed version-one metadata contracts assign distinct exact finding codes.
    Acceptance
    The result fails and contains the declared code for the selected layer.
    Interpretation
    Failure identifies metadata routing, code, or accepted-contract drift.
    Limitations
    This covers representative shape defects, not every metadata rule.
    """
    request = PythonTestEvidenceRequest(
        (PythonTestEvidenceSource(PATH, VALID_SOURCE),),
        "ownership.json",
...
def test_method__execute_explicit_bytes__is_repeatable_without_filesystem_reads(
    ...
    SV-TEV-021
    Requirement
    Execute depends only on the explicit request and returns equal repeated results.
    Method
    Construct one request, make ``Path.read_bytes`` fail if called, and execute twice.
    Oracle
    The placement contract prohibits hidden filesystem access and immutable value
    semantics require equal outputs for identical explicit inputs.
    Acceptance
    No injected filesystem failure occurs and the two results are exactly equal.
    Interpretation
    Failure identifies hidden I/O, retained state, nondeterminism, or equality drift.
    Limitations
    Other process-global services and performance characteristics are excluded.
    """
    request = PythonTestEvidenceRequest(
        (PythonTestEvidenceSource(PATH, VALID_SOURCE),),
        "ownership.json",
...
def test_method__execute_request_type__rejects_foreign_object() -> None:
    ...
    SV-TEV-022
    Requirement
    Execute accepts exactly PythonTestEvidenceRequest values.
    Method
    Invoke the public action with a generic foreign object.
    Oracle
    The public method contract assigns TypeError to a wrong semantic request type.
    Acceptance
    The invocation raises TypeError.
    Interpretation
    Failure identifies action-boundary type-policy drift.
    Limitations
    Correctly typed malformed evidence is represented as findings and covered above.
    """
