r"""Software verification of ``ResourceResolutionResult``.
...
Software verification of the public ``ResourceResolutionResult`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ResourceResolutionResult``.  Accepted H1 field/wire contracts
...
from ksdft2effmass.harness.pi import ResourceResolutionResult

pytestmark = pytest.mark.software_verification
SUT = ResourceResolutionResult


def test_constructor__failed_primary_values__are_rejected() -> None:
    """Evidence ID: SV-HARNESS-018

    Requirement: ResourceResolutionResult enforces the H1 failed-result no-partial-value
    invariant.

    Method: Construct a minimal valid-shaped result and verify exact operational
    immutability; class-specific partial-value checks are covered by owning actions.

    Oracle: The H1 operation-result table requires concrete immutable result types and
    empty
    primary values on FAIL.

    Acceptance: The public type has slots, is immutable, and exposes only declared
    fields.
