r"""Software verification of ``RefreshResourceManifest``.

Facet and represented meaning
Software verification of deterministic explicit-path manifest identity refresh.

Intrinsic and cross-object scope
The sole primary SUT is ``RefreshResourceManifest``. Explicit selection, confined
...
SUT = RefreshResourceManifest


def make_artifact_identity(data: bytes) -> ArtifactIdentity:
    """Evidence ID
    Owns no identifier; supports exact-byte refresh evidence.
    Requirement
    Tests require independently computed SHA-256 identities.
    Method
    Hash caller-supplied controlled bytes with hashlib.
    Oracle
    Python's maintained hashlib SHA-256 supplies the oracle.
    Acceptance
    Return one valid ArtifactIdentity for the exact bytes.
    Interpretation
    Failure indicates test setup or environment failure.
    Limitations
...
def make_resource_reference(
    ...
    """Evidence ID
    Owns no identifier; supports refresh-action evidence.
    Requirement
    Tests require explicit valid manifest references.
    Method
    Construct a ResourceReference from fixed fields and an independent identity.
    Oracle
    Public DataObject constructors define valid support input.
    Acceptance
    Return one valid ResourceReference.
    Interpretation
...
def make_resource_manifest(*resources: ResourceReference) -> ResourceManifest:
    ...
    Owns no identifier; supports refresh-action evidence.
    Requirement
    Tests require one canonical manifest over explicit references.
    Method
    Construct a local ResourceManifest from caller-supplied references.
    Oracle
    Public ResourceManifest construction defines canonical ordering.
    Acceptance
    Return one valid canonical ResourceManifest.
    Interpretation
    Failure indicates invalid setup rather than Action behavior.
    Limitations
    This helper owns no independent evidence claim.
...
def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    ...
    SV-HARNESS-088
    Requirement
    RefreshResourceManifest is a concrete stateless ActionObject.
    Method
    Construct two instances and inspect their storage boundary.
    Oracle
    The accepted action contract requires empty slots and no instance dictionary.
    Acceptance
    Both instances construct and expose no mutable instance storage.
    Interpretation
    Failure indicates hidden state or public ActionObject drift.
    Limitations
    Structural statelessness does not establish execution correctness.
    """
...
def test_method__execute__refreshes_one_identity_and_preserves_manifest(
    ...
    SV-HARNESS-089
    Requirement
    Refreshing one selected resource replaces only its content identity using
    SHA-256 of exact observed bytes and performs no mutation or write.
    Method
    Execute against one controlled file whose bytes differ from the manifest.
    Oracle
    hashlib SHA-256, immutable input equality, and exact field values are independent
    oracles for the proposed manifest.
    Acceptance
    Only the selected identity changes, all other fields and input objects are
    preserved, the output is new, and root bytes and entries remain unchanged.
    Interpretation
    Failure indicates hashing, preservation, mutation, or write-boundary drift.
    Limitations
    Byte equality does not establish semantic correctness or provenance.
    """
    ...
    original_payload = SerializeJsonRecord().execute(manifest).payload
    ...
    assert SerializeJsonRecord().execute(manifest).payload == original_payload
...
def test_method__execute__refreshes_multiple_with_mixed_changed_selection(
    ...
    SV-HARNESS-090
    Requirement
    Explicit multiple selection distinguishes selected IDs from actually changed IDs
    and returns canonical manifest ordering.
    Method
    Select three resources supplied in reverse order; two files differ and one
    already matches.
    Oracle
    Exact bytes and ResourceManifest's accepted canonical ordering fix the result.
    Acceptance
    Changed IDs contain only the two differing resources in sorted order and output
    resources are canonically ordered with every nonidentity field preserved.
    Interpretation
    Failure indicates selection, change reporting, preservation, or ordering drift.
    Limitations
    The test does not validate generic/local profile relationships.
    """
...
def test_method__execute__matching_selection_returns_equal_new_manifest(
    ...
    SV-HARNESS-091
    Requirement
    Selected bytes that already match return successful unchanged manifest state.
    Method
    Execute with a file whose exact SHA-256 is already recorded.
    Oracle
    Exact dataclass equality and known bytes fix the unchanged result.
    Acceptance
    Changed IDs are empty and the returned newly constructed manifest equals input.
    Interpretation
    Failure indicates false change reporting or missing successful no-op behavior.
    Limitations
    Equality is represented-state equality only.
    """
...
def test_method__execute__unknown_and_missing_resources_return_ordered_findings(
    ...
    SV-HARNESS-092
    Requirement
    Unknown IDs and missing selected files produce deterministic structured failure
    with no partial manifest.
    Method
    Select one absent manifest ID and one declared path absent from an existing root.
    Oracle
    The registered issue ordering sorts PIH.PATH before PIH.RESOURCE codes.
    Acceptance
    Codes have the exact deterministic order and result values are empty.
    Interpretation
    Failure indicates discovery, partial refresh, or issue-ordering drift.
    Limitations
    Only two independent failure partitions are aggregated.
    """
    manifest = make_resource_manifest(
...
def test_method__execute__unsafe_or_nonregular_path_is_rejected(
    ...
    SV-HARNESS-093
    Requirement
    Refresh reuses exact-case, nonsymlink, regular-file resource confinement.
    Method
    Execute controlled case-mismatch, symlink, and directory partitions.
    Oracle
    The existing explicit-root resource-resolution contract fixes each issue code.
    Acceptance
    The exact expected singleton code is returned with no manifest.
    Interpretation
    Failure indicates divergence from maintained resource path observation.
    Limitations
    Platform-independent lexical traversal rejection is covered separately.
    """
...
def test_method__execute__validated_resource_path_prevents_traversal_input() -> None:
    ...
    SV-HARNESS-094
    Requirement
    Public refresh input cannot contain a traversal path because ResourcePath owns
    lexical traversal rejection before Action execution.
    Method
    Attempt to construct a selected ResourceReference with a parent segment.
    Oracle
    The existing ResourcePath intrinsic contract requires INVALID_SEGMENT rejection.
    Acceptance
    Public input construction raises ValueError containing the registered code.
    Interpretation
    Failure would expose a traversal route before confined observation.
    Limitations
    This checks the accepted DataObject gate rather than bypassing it with private
    object mutation.
    """
...
def test_method__execute__is_cwd_independent_read_only_and_repeatable(
    ...
    SV-HARNESS-095
    Requirement
    Execution uses only explicit inputs, performs no implicit write, and repeated
    identical invocation returns an equal result from a nonrepository CWD.
    Method
    Change CWD to an unrelated directory, snapshot explicit-root state, and execute
    the same request twice.
    Oracle
    Exact result equality and unchanged directory bytes/names are independent oracles.
    Acceptance
    Results compare equal and no explicit-root file or directory entry changes.
    Interpretation
    Failure indicates ambient discovery, nondeterminism, mutation, or hidden write.
    Limitations
    The test does not trace operating-system reads outside Python's public APIs.
    """
