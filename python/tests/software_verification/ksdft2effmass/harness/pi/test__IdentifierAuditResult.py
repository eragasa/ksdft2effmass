r"""Software verification of ``IdentifierAuditResult``.

Facet and represented meaning

Software verification of the public ``IdentifierAuditResult`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``IdentifierAuditResult``.  Accepted H1 field/wire contracts and
...
from ksdft2effmass.harness.pi.evidence import IdentifierAuditResult

pytestmark = pytest.mark.software_verification
SUT = IdentifierAuditResult


def test_constructor__failed_primary_values__are_rejected() -> None:
    """Evidence ID: SV-HARNESS-020

    Requirement: IdentifierAuditResult enforces the H1 failed-result no-partial-value
    invariant.
