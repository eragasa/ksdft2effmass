...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    ...
    SV-HARNESS-010
    Requirement
    CheckpointRecord accepts the complete valid version-1 H3 wire instance and is
    immutable.
    Method
    Decode the accepted ``checkpoint-record.json`` fixture through the
    ...
    result = DeserializeJsonRecord().execute(WireRecordKind.CheckpointRecord, payload)
