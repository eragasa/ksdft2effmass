...
def make_resource_manifest_request_fixture() -> ResourceManifest:
    ...
    Requirement
    Tests require one independently valid manifest input.
    Method
    ...
    Interpretation
    Failure indicates invalid test setup rather than refresh behavior.
    Limitations
...
def test_constructor__valid_fields__maps_exact_values_and_canonical_selection(
    ...
    SV-HARNESS-081
    Requirement
    The request stores the explicit absolute root and exact manifest while
    canonicalizing a nonempty resource selection to sorted unique identifiers.
    Method
    Construct the request with unsorted duplicate valid identifiers.
    Oracle
    The accepted request contract and Python sorted-set semantics fix the fields.
    Acceptance
    Root and manifest retain object identity and IDs equal the sorted unique tuple.
    Interpretation
    Failure indicates field mapping or canonicalization drift.
    Limitations
    No filesystem resource is observed and no manifest refresh is executed.
    """
...
def test_constructor__invalid_field__raises_semantic_exception(
    ...
    SV-HARNESS-082
    Requirement
    Wrong semantic types raise TypeError and correctly typed invariant violations
    raise ValueError.
    Method
    Replace one valid constructor field with each invalid semantic partition.
    Oracle
    The public request invariant table fixes the exception class for every case.
    Acceptance
    Construction raises exactly the declared TypeError or ValueError family.
    Interpretation
    Failure indicates exception-taxonomy or intrinsic-validation drift.
    Limitations
    Filesystem existence and selected-ID membership are ActionObject concerns.
    """
    values: dict[str, Any] = {
        "root": tmp_path.resolve(),
        "manifest": make_resource_manifest_request_fixture(),
...
def test_field__frozen_assignment__raises_attribute_error(tmp_path: Path) -> None:
    ...
    Method
    Assign to a public field on a valid request.
    Oracle
    Frozen dataclass semantics are the exact language-level oracle.
    Acceptance
    Assignment raises AttributeError and stored state is unchanged.
    Interpretation
    Failure indicates an unauthorized mutable request boundary.
    Limitations
    Nested records are independently immutable under their own contracts.
    """
    request = SUT(
        tmp_path.resolve(),
