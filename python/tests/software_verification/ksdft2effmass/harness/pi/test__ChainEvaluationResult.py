r"""Software verification of ``ChainEvaluationResult``.

Facet and represented meaning

Software verification of the public ``ChainEvaluationResult`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ChainEvaluationResult``.  Accepted H1 field/wire contracts and
...
from ksdft2effmass.harness.pi import ChainEvaluationResult

pytestmark = pytest.mark.software_verification
SUT = ChainEvaluationResult


def test_constructor__failed_primary_values__are_rejected() -> None:
    """Evidence ID: SV-HARNESS-019

    Requirement: ChainEvaluationResult enforces the H1 failed-result no-partial-value
    invariant.
