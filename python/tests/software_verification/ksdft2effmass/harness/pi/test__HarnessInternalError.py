...
def test_constructor__attributes__are_exact_and_immutable() -> None:
    """Evidence ID: SV-HARNESS-024

    Requirement: The internal-failure channel stores immutable operation and detail
    strings.

    Method: Construct the error and attempt mutation.

    Oracle: The accepted H1 support-error contract fixes both attributes and
    RuntimeError
    ancestry.

    Acceptance: Exact strings are stored and later assignment raises AttributeError.
    ...
    """
    error = SUT("ResourceResolver", "read race")
