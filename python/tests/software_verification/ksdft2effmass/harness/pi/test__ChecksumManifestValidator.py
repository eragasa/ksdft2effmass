r"""Software verification of ``ChecksumManifestValidator``.

Facet and represented meaning

Software verification of the public ``ChecksumManifestValidator`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ChecksumManifestValidator``.  Accepted H1 field/wire contracts
...
from ksdft2effmass.harness.pi import ChecksumManifestValidator

pytestmark = pytest.mark.software_verification
SUT = ChecksumManifestValidator


def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    """Evidence ID: SV-HARNESS-034

    Requirement: ChecksumManifestValidator is a concrete stateless ActionObject.
