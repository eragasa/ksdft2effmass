...
The sole primary SUT is ``AgentDescriptorView``.  Accepted H1 field/wire contracts and
...
SUT = AgentDescriptorView


def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-007

    Requirement: AgentDescriptorView accepts the complete valid version-1 H3 wire
    instance and is
    immutable.

    Method: Decode the accepted ``agent-descriptor-view.json`` fixture through the
    ...
    """
    payload = (
        ROOT / "harness/pi/fixtures/valid/agent-descriptor-view.json"
    ).read_bytes()
    result = JsonRecordDeserializer().execute(
        WireRecordKind.AgentDescriptorView, payload
    )
