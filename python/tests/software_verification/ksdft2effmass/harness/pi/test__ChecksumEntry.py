r"""Software verification of ``ChecksumEntry``.

Facet and represented meaning

Software verification of the public ``ChecksumEntry`` surface; no physical model,
mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ChecksumEntry``.  Accepted H1 field/wire contracts and
...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-013

    Requirement: ChecksumEntry accepts the complete valid version-1 H3 wire instance and
    is
    immutable.

    Method: Decode the accepted ``checksum-entry.json`` fixture through the
    caller-selected
    ...
    """
    payload = (ROOT / "harness/pi/fixtures/valid/checksum-entry.json").read_bytes()
    result = JsonRecordDeserializer().execute(WireRecordKind.ChecksumEntry, payload)
