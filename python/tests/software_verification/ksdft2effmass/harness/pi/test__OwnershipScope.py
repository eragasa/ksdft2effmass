r"""Software verification of ``OwnershipScope``.
...
Software verification of the public ``OwnershipScope`` surface; no physical model,
mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``OwnershipScope``.  Accepted H1 field/wire contracts and
...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    """Evidence ID: SV-HARNESS-006

    Requirement: OwnershipScope accepts the complete valid version-1 H3 wire instance
    and is
    immutable.

    Method: Decode the accepted ``ownership-scope.json`` fixture through the
    caller-selected
    ...
    """
    payload = (ROOT / "harness/pi/fixtures/valid/ownership-scope.json").read_bytes()
    result = JsonRecordDeserializer().execute(WireRecordKind.OwnershipScope, payload)
