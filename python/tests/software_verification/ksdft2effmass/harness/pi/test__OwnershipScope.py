...
def test_constructor__h3_valid_fixture__preserves_exact_public_value() -> None:
    ...
    SV-HARNESS-006
    Requirement
    OwnershipScope accepts the complete valid version-1 H3 wire instance and is
    immutable.
    Method
    Decode the accepted ``ownership-scope.json`` fixture through the caller-selected
    ...
    result = DeserializeJsonRecord().execute(WireRecordKind.OwnershipScope, payload)
