r"""Software verification of ``ResourceManifestValidator``.
...
Software verification of the public ``ResourceManifestValidator`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``ResourceManifestValidator``.  Accepted H1 field/wire contracts
...
from ksdft2effmass.harness.pi import ResourceManifestValidator

pytestmark = pytest.mark.software_verification
SUT = ResourceManifestValidator


def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    """Evidence ID: SV-HARNESS-029

    Requirement: ResourceManifestValidator is a concrete stateless ActionObject.
...
def test_method__execute_relational_precedence__returns_exact_existing_code(
    ...
    """Evidence ID: SV-HARNESS-062

    Requirement: Manifest validation owns duplicate IDs, duplicate paths, and self-edge
    cycles and
    reports their existing capability-specific code under accepted precedence.

    Method: Publicly deserialize each corrected H3 relational candidate and its profile,
    compute the candidate's public serialized identity, and validate it.

    Oracle: Corrected H1 action precedence and the H3 oracle index independently fix the
    three
    exact singleton issue-code sequences.

    Acceptance: Candidate deserialization passes; validation fails; the issue-code list
    is exactly
    the parameter's singleton code, without a wire or lower-precedence substitute.

    Interpretation: Failure identifies decoder/validator ownership drift, precedence
    drift, or an H3
    oracle defect requiring independent review.

    Limitations: The three corrected relational partitions are covered; other
    invalid-manifest
...
def test_method__execute_profile_compatibility__distinguishes_kind_from_version(
    ...
    """Evidence ID: SV-HARNESS-065

    Requirement: Manifest validation distinguishes a closed-enum kind absent from the
    profile
    from an unsupported version of a kind that the profile does support.

    Method: Mutate only kind or format_version in the valid H3 generic-leaf candidate,
    deserialize it publicly, compute its public identity, and validate it.

    Oracle: The accepted H1 issue/action contract assigns KIND_UNSUPPORTED to an absent
    profiled kind and VERSION_INCOMPATIBLE to an absent supported-kind pair.

    Acceptance: Deserialization passes and validation returns the exact singleton code.

    Interpretation: Failure identifies compatibility-classification or precedence drift.
