r"""Software verification of ``JsonRecordDeserializer``.
...
Software verification of the public ``JsonRecordDeserializer`` surface; no physical
model, mathematical operator, or numerical representation is represented.

Intrinsic and cross-object scope

The sole primary SUT is ``JsonRecordDeserializer``.  Accepted H1 field/wire contracts
and
...
from ksdft2effmass.harness.pi import JsonRecordDeserializer

pytestmark = pytest.mark.software_verification
SUT = JsonRecordDeserializer


def test_constructor__action_object__is_stateless_and_fieldless() -> None:
    """Evidence ID: SV-HARNESS-026

    Requirement: JsonRecordDeserializer is a concrete stateless ActionObject.
