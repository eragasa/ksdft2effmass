...
def test_artifact__import_dependency_direction__follows_approved_layers() -> None:
    ...
    Requirement
    Named Workflow CPN Python modules import only their approved predecessor layers
    through relative imports.

    Method
    Parse each named production module with standard-library AST and collect level-one
    relative import modules; no warnings are expected.

    Oracle
    The explicit per-module predecessor map is the approved architecture oracle.

    Acceptance
    For every named module, the observed relative-import set is a subset of its approved
    predecessors.

    Interpretation
    Pass supports the inspected dependency direction; failure may indicate architecture,
    source, or evidence-map drift.

    Limitations
    New modules and absolute intra-package imports can evade this bounded oracle.
