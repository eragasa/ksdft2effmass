...
def test_artifact__snakes_isolation__excludes_deferred_engine_scope() -> None:
    """Evidence ID: SV-CPN-033

    Requirement: The P1 Workflow CPN Python runtime has no named deferred
    engine/persistence path and
    no direct SNAKES import root.

    Method: Check absence of ``engines`` and ``persistence.py``; parse all top-level
    Python
    modules and collect direct import roots; no warnings are expected.

    Oracle: The accepted P1 scope excludes those paths and the ``snakes`` import root.

    Acceptance: Both deferred paths are absent and no inspected direct import root
    equals
    ``snakes``.

    Interpretation: Pass supports bounded P1 isolation; failure may indicate
    deferred-scope, dependency,
    or evidence drift.

    Limitations: Dynamic and transitive dependencies are not detected, and no SNAKES
    implementation
