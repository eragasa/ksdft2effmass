...
def load_provenance_v1_schema() -> dict[str, Any]:
    """Evidence ID: Owns no identifier; supports SV-PROV-064, SV-PROV-065, SV-PROV-066,
    and
    SV-PROV-398.

    Requirement: Load the exact checked-in public version-1 schema as one JSON object.

    Method: Read SCHEMA_PATH as UTF-8 and decode it through the standard JSON parser
    without
    caching or transformation.

    Oracle: The fixed repository path and JSON object grammar independently define the
    input.

    Acceptance: Decoding returns a built-in dict for every call.

    Interpretation: Failure indicates schema-path, UTF-8, JSON syntax, or object-shape
    setup drift.

    Limitations: Loading establishes no schema validity, Python agreement, fixture
    classification,
...
def assert_exact_top_level_references(references: tuple[str, ...]) -> None:
    """Evidence ID: Owns no identifier; supports SV-PROV-064.

    Requirement: Require the top-level schema references to be exactly the unique
    17-record family.

    Method: Compare count, unique count, and unordered reference set with
    EXPECTED_DEFINITIONS.

    Oracle: The explicit accepted definition inventory independently fixes every
    expected ref.

    Acceptance: There are exactly 17 references, all are unique, and their set is exact.

    Interpretation: Failure identifies a missing, extra, duplicated, or misdirected
    top-level reference.

    Limitations: This helper does not compile schema content, inspect fixtures, execute
    Python
...
def is_nfc_text(value: object) -> bool:
    """Evidence ID: Owns no identifier; supports SV-PROV-066.

    Requirement: Identify exact built-in strings already represented in Unicode NFC.

    Method: Require exact built-in str type and compare with unicodedata NFC
    normalization.

    Oracle: Python unicodedata.normalize independently defines the NFC transformation.

    Acceptance: Return true exactly for built-in strings unchanged by NFC normalization.

    Interpretation: Failure indicates format-checker oracle or Unicode-database behavior
    drift.

    Limitations: This representative predicate does not establish full path grammar,
    fixtures,
    provenance truth, scientific validation, UQ, persistence, or portability.
    """
...
def test_artifact__schema_declaration__matches_draft_and_record_inventory() -> None:
    """Evidence ID: SV-PROV-064

    Requirement: The schema declares Draft 2020-12, compiles under that metaschema, and
    contains the
    exact 17-definition family with exactly 17 unique top-level references.

    Method: Load the schema, assert its dialect, invoke
    ...
    Oracle: The Draft 2020-12 URI, metaschema, and explicit EXPECTED_DEFINITIONS are
    independent.

    Acceptance: The dialect is exact; compilation succeeds; definitions and unique
    references are
    exact; and a 17-element inventory containing one duplicate causes the same helper
    to fail at the uniqueness guard.

    Interpretation: Failure identifies dialect, schema compilation, definition
    inventory, reference
    inventory, or duplicate-detection drift.

    Limitations: This does not establish Python record behavior, fixture-family
    validity, provenance
...
def test_artifact__stored_vocabularies__agree_with_public_python_enums() -> None:
    """Evidence ID: SV-PROV-065

    Requirement: All eight stored schema vocabularies agree exactly with public Python
    enums, and the
    stored execution-result status equals the public COMPLETED value.

    Method: Build literal-key schema and Python mappings from eight explicit locations,
    compare
    them exactly, and compare the explicit completed constant.

    Oracle: The checked-in schema locations and public enum iteration are independent
    sides.

    Acceptance: The two eight-key mappings are exactly equal and completed const equals
    COMPLETED.value.

    Interpretation: Failure identifies a missing location or stored Python/schema
    vocabulary drift.

    Limitations: Enum construction and lookup semantics remain class-owned; this does
    not establish
...
def test_artifact__derived_statuses__remain_unstored_public_properties() -> None:
    """Evidence ID: SV-PROV-398

    Requirement: Verification and correlation statuses remain public derived properties
    absent from
    dataclass storage, schema properties, and serialized version-1 JSON.

    Method: Inspect public dataclass fields and property descriptors, inspect two
    explicit
    schema property maps, serialize valid representative records, and compare one map.

    Oracle: Dataclass reflection, explicit schema definitions, Python property
    semantics, and
    public serializer output independently expose the four storage relations.

    Acceptance: For both classes, status is absent from fields, schema, and JSON but is
    a property.

    Interpretation: Failure identifies derived-property exposure or duplicated
    wire/storage state drift.

    Limitations: This does not retest derived outcomes, prove persistence, establish
    provenance
...
def test_artifact__nfc_format__distinguishes_normalized_text() -> None:
    """Evidence ID: SV-PROV-066

    Requirement: The schema NFC format checker accepts normalized text and rejects a
    canonically
    equivalent decomposed representative.

    Method: Register is_nfc_text for format ``nfc`` and validate explicit composed and
    decomposed logical paths against ArtifactSpecification.

    Oracle: Python unicodedata NFC normalization independently predicts the two
    outcomes.

    Acceptance: ``é/result.json`` validates and ``e\u0301/result.json`` raises
    ValidationError.

    Interpretation: Failure identifies schema format wiring, Unicode oracle, or checker
    activation
    drift.

    Limitations: This representative pair does not establish full path grammar,
    fixture-family
