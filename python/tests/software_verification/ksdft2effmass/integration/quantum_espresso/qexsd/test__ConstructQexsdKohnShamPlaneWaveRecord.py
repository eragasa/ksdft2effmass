...
class TestConstructQexsdKohnShamPlaneWaveRecord:
    """Own this module's maintained software-verification evidence."""

    def test_method__execute__constructs_quantity_specific_semantics(self) -> None:
        """Evidence ID: SV-PERIODIC-009

        Requirement: Construction assigns explicit quantity-specific units and
        conventions.
        ...
        assert (
            record.spectrum.energy_reference_availability
    ...
    def test_method__execute__enforces_direct_reciprocal_duality(self) -> None:
        ...
        ) == pytest.approx(2.0 * math.pi, abs=1.0e-12, rel=0.0)
        assert math.fsum(
            (a1[0] * b2[0], a1[1] * b2[1], a1[2] * b2[2])
        ) == pytest.approx(0.0, abs=1.0e-12, rel=0.0)
        assert math.fsum(
            (a1[0] * b3[0], a1[1] * b3[1], a1[2] * b3[2])
        ) == pytest.approx(0.0, abs=1.0e-12, rel=0.0)
        assert math.fsum(
            (a2[0] * b1[0], a2[1] * b1[1], a2[2] * b1[2])
        ) == pytest.approx(0.0, abs=1.0e-12, rel=0.0)
        assert math.fsum(
            (a2[0] * b2[0], a2[1] * b2[1], a2[2] * b2[2])
        ) == pytest.approx(2.0 * math.pi, abs=1.0e-12, rel=0.0)
        assert math.fsum(
            (a2[0] * b3[0], a2[1] * b3[1], a2[2] * b3[2])
        ) == pytest.approx(0.0, abs=1.0e-12, rel=0.0)
        assert math.fsum(
            (a3[0] * b1[0], a3[1] * b1[1], a3[2] * b1[2])
        ) == pytest.approx(0.0, abs=1.0e-12, rel=0.0)
        assert math.fsum(
    ...
    def test_method__execute__rejects_wrong_input_type(self) -> None:
