...
class TestPlaneWaveRecordRejection:
    ...
    def test_artifact__serialization__rejects_duplicate_and_unknown_fields(
        self,
    ) -> None:
        ...
        canonical = TestPlaneWaveRecordRejection.make_record_text()
    ...
    def test_artifact__serialization__round_trips_configured_duality_tolerance(
        self,
    ) -> None:
        ...
        """
        payload = json.loads(TestPlaneWaveRecordRejection.make_record_text())
    ...
    def test_artifact__serialization__rejects_invalid_types_units_and_scales(
        self,
    ) -> None:
        ...
        Acceptance: Every independently malformed payload raises ``TypeError`` or
        ``ValueError``.
        """
        serializer = KohnShamPlaneWaveCalculationRecordJsonSerializer()
        original = json.loads(TestPlaneWaveRecordRejection.make_record_text())
    ...
    def test_artifact__serialization__rejects_cross_object_count_disagreement(
        self,
    ) -> None:
        ...
        """
        serializer = KohnShamPlaneWaveCalculationRecordJsonSerializer()
        record = serializer.deserialize(TestPlaneWaveRecordRejection.make_record_text())
        assert record.spectrum.occupations is not None
        spectrum = replace(
            record.spectrum,
            eigenvalues=record.spectrum.eigenvalues
            + (record.spectrum.eigenvalues[-1],),
            occupations=record.spectrum.occupations
            + (record.spectrum.occupations[-1],),
        )
    ...
    def test_artifact__record_invariants__reject_inconsistent_reciprocal_scale(
        self,
    ) -> None:
