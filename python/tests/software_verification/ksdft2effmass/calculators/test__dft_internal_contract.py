...
class TestDftInternalContract:
    """Own software evidence for the private DFT probe artifact."""

    def make_context(self) -> TaskExecutionContext:
    ...
    def make_qe_scf_input(self) -> QuantumEspressoScfInput:
    ...
    def make_qe_scf_output(self) -> QuantumEspressoScfOutput:
    ...
    def test_artifact__variants__retain_exact_calculator_specific_state(self) -> None:
        ...
        """
        qe_scf_input = self.make_qe_scf_input()
        qe_scf_output = self.make_qe_scf_output()
        ...
        assert (
            qe_bands_input.native_state_identity is qe_scf_output.native_state_identity
        )
    ...
    def test_artifact__construction__rejects_invalid_intrinsic_state(self) -> None:
    ...
    def test_artifact__immutability__prevents_record_mutation(self) -> None:
    ...
    def test_artifact__protocol__admits_explicit_injected_implementation(self) -> None:
        ...
        expected_input = self.make_qe_scf_input()
        expected_context = self.make_context()
        ...
        class RetainedQeCalculator:
            ...
            def execute(
                ...
                assert simulation_input == expected_input
                assert context == expected_context
                return self.output

        output = self.make_qe_scf_output()
        calculator = RetainedQeCalculator(output)
        assert isinstance(calculator, DftCalculator)
        assert calculator.execute(expected_input, expected_context) is output
