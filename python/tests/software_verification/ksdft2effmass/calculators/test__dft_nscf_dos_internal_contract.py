...
class TestDftNscfDosInternalContract:
    """Own software evidence for private QE NSCF and DOS probe records."""

    def test_artifact__nscf_input__binds_exact_scf_result_and_state(self) -> None:
        """Evidence ID: SV-DFT-NSCF-DOS-001

        Requirement: A QE NSCF input retains its own exact native input and
        pseudopotentials plus the admitted SCF result and immutable native-state
        identity.
        ...
        Limitations: Identity binding does not inspect or validate native-state
        contents.
        """
    ...
    def test_artifact__dos_input__binds_exact_nscf_result_and_state(self) -> None:
        ...
        Interpretation: Failure identifies DOS predecessor or native-input contract
        drift.
    ...
    def test_artifact__operation_outputs__separate_state_and_dos_artifacts(
        self,
    ) -> None:
        """Evidence ID: SV-DFT-NSCF-DOS-003

        Requirement: NSCF produces a new native-state identity whereas DOS produces a
        DOS result-artifact identity, and each output retains its own process
        observation.
        ...
        Oracle: The private operation contracts distinguish continuation state from
        final DOS data and require one mechanical process-observation identity per
        output.
    ...
    def test_artifact__predecessor_fields__reject_wrong_nominal_identities(
        self,
    ) -> None:
        ...
        Method: Construct NSCF and DOS inputs with the two nominal identity types
        swapped.

        Oracle: Exact nominal validation assigns result correlation and artifact
        identity different semantic roles.
    ...
    def test_artifact__closed_unions__include_nscf_and_dos_variants(self) -> None:
        """Evidence ID: SV-DFT-NSCF-DOS-005

        Requirement: The private injected DFT calculator port accepts the new concrete
        QE NSCF and DOS input and output variants without erasing their concrete types.
        ...
        Acceptance: The four concrete NSCF and DOS types are members of the
        corresponding union arguments.
    ...
    def test_artifact__package_surface__keeps_operation_variants_private(self) -> None:
