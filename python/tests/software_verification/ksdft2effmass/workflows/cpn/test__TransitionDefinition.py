r"""Software verification of ``TransitionDefinition``.
...
def test_constructor__transition_requires__preserves_valid_state() -> None:
    """Evidence ID: SV-CPN-057

    Requirement: ``TransitionDefinition`` preserves the documented exact valid-state
    behavior for
    its ``transition_requires`` contract.
...
def test_constructor__transition_requires__rejects_wrong_types() -> None:
    """Evidence ID: SV-CPN-119

    Requirement: ``TransitionDefinition`` rejects wrong semantic types for its
    ``transition_requires`` contract.
    ...
    numerical verification, scientific validation, UQ, physics, and portability.
    """
    GuardExpression(GuardOperator.TRUE)
    with pytest.raises(TypeError):
        SUT("t", "transition", True)  # type: ignore[arg-type]


def test_constructor__transition_requires__rejects_invalid_values() -> None:
    """Evidence ID: SV-CPN-090

    Requirement: ``TransitionDefinition`` rejects malformed values of accepted semantic
    types for its
    ``transition_requires`` contract.
