r"""Software verification of ``TokenPattern``.
...
def test_constructor__pattern_requires__preserves_valid_state() -> None:
    """Evidence ID: SV-CPN-054

    Requirement: ``TokenPattern`` preserves the documented exact valid-state behavior
    for its
    ``pattern_requires`` contract.
...
def test_constructor__pattern_requires__rejects_wrong_types() -> None:
    """Evidence ID: SV-CPN-122

    Requirement: ``TokenPattern`` rejects wrong semantic types for its
    ``pattern_requires`` contract.
...
def test_constructor__pattern_requires__rejects_invalid_values() -> None:
    """Evidence ID: SV-CPN-094

    Requirement: ``TokenPattern`` rejects malformed values of accepted semantic
    types for its
    ``pattern_requires`` contract.
