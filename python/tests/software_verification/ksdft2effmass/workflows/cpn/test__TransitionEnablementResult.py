...
def test_constructor__fields__bindings_match_transition_and_are_unique() -> None:
    """Evidence ID: SV-CPN-038

    Requirement: enforce coherent deterministic enablement bindings.
    ...
    The prior scenario documentation requires every binding to name the result
    transition and occur once. The test constructs one mismatched binding and one
    duplicated matching binding; exact transition-string equality and tuple uniqueness
    determine the documented ``ValueError`` outcomes. Enumeration itself is owned by
    ``TransitionEnabler`` and is not re-tested here.

    Oracle: The documented public rule that the SUT must enforce coherent deterministic
...
def test_constructor__enablement_container__preserves_valid_state() -> None:
    """Evidence ID: SV-CPN-077

    Requirement: ``TransitionEnablementResult`` preserves the documented exact
    valid-state
...
def test_constructor__enablement_container__rejects_wrong_types() -> None:
    """Evidence ID: SV-CPN-131

    Requirement: ``TransitionEnablementResult`` rejects wrong semantic types for its
...
def test_constructor__enablement_container__rejects_invalid_values() -> None:
    """Evidence ID: SV-CPN-106

    Requirement: ``TransitionEnablementResult`` rejects malformed values of accepted
    semantic
    types for its
    ``enablement_container`` contract.
