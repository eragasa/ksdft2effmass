...
def test_constructor__fields__guard_result_requires_exact_boolean() -> None:
    """Evidence ID: SV-CPN-047

    Requirement: ``GuardEvaluationResult`` preserves the exact accepted state for its
    ``fields`` contract.
...
def test_constructor__fields__rejects_invalid_state() -> None:
    """Evidence ID: SV-CPN-141

    Requirement: ``GuardEvaluationResult`` rejects the documented invalid state for its
    ``fields`` contract.
