...
def test_constructor__fields__audit_fields_are_intrinsically_coherent(
    ...
    """Evidence ID: SV-CPN-039

    Requirement: reject contradictory firing audit state.
    ...
    The binding identity, successor revision, produced identities, and audit-ID tuples
    must agree internally. The test independently violates each relation through the
    public constructor; exact transition equality, revision ``previous + 1``,
    uniqueness, and nonempty IDs determine the expected ``ValueError`` outcomes.

    Oracle: The documented public rule that the SUT must reject contradictory firing
    audit state
...
def make_valid_firing_result(executable_net: CpnNetDefinition) -> FiringResult:
    """Evidence ID: Owns no identifier; supports SV-CPN-070, SV-CPN-072, SV-CPN-073,
    SV-CPN-089,
    SV-CPN-103.
...
def test_constructor__result_requires__preserves_valid_state(
    ...
    """Evidence ID: SV-CPN-070

    Requirement: ``FiringResult`` preserves the documented exact valid-state behavior
    for its
...
def test_constructor__result_requires__rejects_wrong_types(
    ...
    """Evidence ID: SV-CPN-128

    Requirement: ``FiringResult`` rejects wrong semantic types for its
    ``result_requires`` contract.
...
def test_constructor__result_requires__rejects_invalid_values(
    ...
    """Evidence ID: SV-CPN-101

    Requirement: ``FiringResult`` rejects malformed values of accepted semantic
    types for its
    ``result_requires`` contract.
...
def test_constructor__previous_revision__rejects_wrong_types(
    ...
    """Evidence ID: SV-CPN-071

    Requirement: ``FiringResult`` rejects wrong semantic types at the public
...
def test_constructor__fields__result_revision_boundary_is_coherent(
    ...
    """Evidence ID: SV-CPN-084

    Requirement: ``FiringResult`` preserves the exact accepted state for its
    ``fields`` contract.
...
def test_constructor__fields__rejects_invalid_state(
    ...
    """Evidence ID: SV-CPN-149

    Requirement: ``FiringResult`` rejects the documented invalid state for its
    ``fields`` contract.
...
def test_constructor__audit_id_sequence_types__rejects_wrong_types(
    ...
    """Evidence ID: SV-CPN-072

    Requirement: enforce immutable unique consumed/read identity sequences.
    ...
    Oracle: The documented public rule that the SUT must enforce immutable unique
    consumed/read
...
def test_constructor__audit_id_sequence_values__rejects_empty_or_duplicate_ids(
    ...
    """Evidence ID: SV-CPN-089

    Requirement: Consumed and read audit identifiers must be nonempty and unique per
    sequence.

    Method: Construct the public result with one named invalid value partition and no
    warnings.

    Oracle: The public nonempty-unique identifier invariant fixes each rejected tuple.

    Acceptance: Construction raises exactly ``ValueError`` for every explicit parameter
    case.

    Interpretation: Pass supports value validation; failure permits ambiguous firing
    audit identities.

    Limitations: Synthetic identifiers exclude engine execution, scientific validation,
    UQ, physical
