...
def test_constructor__binding_requires__preserves_valid_state() -> None:
    ...
    Requirement
    -----------
    ``TokenBinding`` preserves the documented exact valid-state behavior for its
...
def test_constructor__binding_requires__rejects_wrong_types() -> None:
    ...
    Requirement
    -----------
    ``TokenBinding`` rejects wrong semantic types for its ``binding_requires`` contract.
...
def test_constructor__binding_requires__rejects_invalid_values() -> None:
    ...
    Requirement
    -----------
    ``TokenBinding`` rejects malformed values of accepted semantic
    types for its
    ``binding_requires`` contract.
