r"""Software verification of ``TokenBinding``.
...
def test_constructor__binding_requires__preserves_valid_state() -> None:
    """Evidence ID: SV-CPN-052

    Requirement: ``TokenBinding`` preserves the documented exact valid-state behavior
    for its
    ``binding_requires`` contract.
...
def test_constructor__binding_requires__rejects_wrong_types() -> None:
    """Evidence ID: SV-CPN-134

    Requirement: ``TokenBinding`` rejects wrong semantic types for its
    ``binding_requires`` contract.
...
def test_constructor__binding_requires__rejects_invalid_values() -> None:
    """Evidence ID: SV-CPN-110

    Requirement: ``TokenBinding`` rejects malformed values of accepted semantic
    types for its
    ``binding_requires`` contract.
