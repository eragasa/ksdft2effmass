...
def test_constructor__place_marking__preserves_valid_state(
    ...
    Requirement
    -----------
    ``PlaceMarking`` preserves the documented exact valid-state behavior for its
...
def test_constructor__place_marking__rejects_wrong_types(
    ...
    Requirement
    -----------
    ``PlaceMarking`` rejects wrong semantic types for its ``place_marking`` contract.
...
def test_constructor__place_marking__rejects_invalid_values(
    ...
    Requirement
    -----------
    ``PlaceMarking`` rejects malformed values of accepted semantic
    types for its
    ``place_marking`` contract.
