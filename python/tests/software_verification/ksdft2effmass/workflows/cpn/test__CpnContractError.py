...
def test_constructor__fields__error_retains_authoritative_detail() -> None:
    """Evidence ID: SV-CPN-042

    Requirement: ``CpnContractError`` preserves the exact accepted state for its
    ``fields`` contract.
...
def test_constructor__fields__rejects_invalid_state() -> None:
    """Evidence ID: SV-CPN-150

    Requirement: ``CpnContractError`` rejects the documented invalid state for its
    ``fields`` contract.
