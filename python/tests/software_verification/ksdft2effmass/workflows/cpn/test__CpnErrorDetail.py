...
def test_constructor__detail_validates__preserves_valid_state() -> None:
    ...
    Requirement
    -----------
    ``CpnErrorDetail`` preserves the documented exact valid-state behavior for its
...
def test_constructor__detail_validates__rejects_wrong_types() -> None:
    ...
    Requirement
    -----------
    ``CpnErrorDetail`` rejects wrong semantic types for its ``detail_validates``
    contract.
...
def test_constructor__detail_validates__rejects_invalid_values() -> None:
    ...
    Requirement
    -----------
    ``CpnErrorDetail`` rejects malformed values of accepted semantic
    types for its
    ``detail_validates`` contract.
