...
def test_constructor__issue_owns__preserves_valid_state() -> None:
    ...
    Requirement
    -----------
    ``CpnValidationIssue`` preserves the documented exact valid-state behavior for
...
def test_constructor__issue_owns__rejects_wrong_types() -> None:
    ...
    Requirement
    -----------
    ``CpnValidationIssue`` rejects wrong semantic types for its ``issue_owns`` contract.
...
def test_constructor__issue_owns__rejects_invalid_values() -> None:
    ...
    Requirement
    -----------
    ``CpnValidationIssue`` rejects malformed values of accepted semantic
    types for its
    ``issue_owns`` contract.
