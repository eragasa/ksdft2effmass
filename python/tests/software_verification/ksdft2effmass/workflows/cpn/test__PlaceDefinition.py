...
def test_constructor__place_requires__preserves_valid_state() -> None:
    ...
    Requirement
    -----------
    ``PlaceDefinition`` preserves the documented exact valid-state behavior for its
...
def test_constructor__place_requires__rejects_wrong_types() -> None:
    ...
    Requirement
    -----------
    ``PlaceDefinition`` rejects wrong semantic types for its ``place_requires``
    contract.
...
def test_constructor__place_requires__rejects_invalid_values() -> None:
    ...
    Requirement
    -----------
    ``PlaceDefinition`` rejects malformed values of accepted semantic
    types for its
    ``place_requires`` contract.
