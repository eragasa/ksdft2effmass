r"""Software verification of ``ResultProductionRecordIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ResultProductionRecordIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ResultProductionRecordIdentity

pytestmark = pytest.mark.software_verification
SUT = ResultProductionRecordIdentity


class TestResultProductionRecordIdentity:
    """Own software evidence for ``ResultProductionRecordIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-RESULT-PRODUCTION-RECORD-IDENTITY-001

        Requirement: ``ResultProductionRecordIdentity`` wraps one nonempty exact
        built-in string.
