r"""Software verification of ``ExternalResultProducerIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ExternalResultProducerIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ExternalResultProducerIdentity

pytestmark = pytest.mark.software_verification
SUT = ExternalResultProducerIdentity


class TestExternalResultProducerIdentity:
    """Own software evidence for ``ExternalResultProducerIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-EXTERNAL-RESULT-PRODUCER-IDENTITY-001

        Requirement: ``ExternalResultProducerIdentity`` wraps one nonempty exact
        built-in string.
