r"""Software verification of ``ResultObjectTypeIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ResultObjectTypeIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``ResultObjectTypeIdentity``.
...
from ksdft2effmass.workflows.runs import ResultObjectTypeIdentity

pytestmark = pytest.mark.software_verification
SUT = ResultObjectTypeIdentity


class TestResultObjectTypeIdentity:
    """Own software evidence for ``ResultObjectTypeIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-RESULT-OBJECT-TYPE-IDENTITY-001

        Requirement: ``ResultObjectTypeIdentity`` wraps one nonempty exact
        built-in string.
