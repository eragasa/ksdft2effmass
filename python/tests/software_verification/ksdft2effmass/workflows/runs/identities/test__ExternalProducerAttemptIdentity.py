r"""Software verification of ``ExternalProducerAttemptIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ExternalProducerAttemptIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ExternalProducerAttemptIdentity

pytestmark = pytest.mark.software_verification
SUT = ExternalProducerAttemptIdentity


class TestExternalProducerAttemptIdentity:
    """Own software evidence for ``ExternalProducerAttemptIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-EXTERNAL-PRODUCER-ATTEMPT-IDENTITY-001

        Requirement: ``ExternalProducerAttemptIdentity`` wraps one nonempty
