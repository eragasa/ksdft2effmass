r"""Software verification of ``DispatchResourceScopeIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``DispatchResourceScopeIdentity`` contract.
...
from ksdft2effmass.workflows.runs import DispatchResourceScopeIdentity

pytestmark = pytest.mark.software_verification
SUT = DispatchResourceScopeIdentity


class TestDispatchResourceScopeIdentity:
    """Own software evidence for ``DispatchResourceScopeIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-DISPATCH-RESOURCE-SCOPE-IDENTITY-001

        Requirement: ``DispatchResourceScopeIdentity`` wraps one nonempty exact
        built-in string.
