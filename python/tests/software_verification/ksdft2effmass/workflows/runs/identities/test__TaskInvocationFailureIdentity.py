r"""Software verification of ``TaskInvocationFailureIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskInvocationFailureIdentity`` contract.
...
from ksdft2effmass.workflows.runs import TaskInvocationFailureIdentity

pytestmark = pytest.mark.software_verification
SUT = TaskInvocationFailureIdentity


class TestTaskInvocationFailureIdentity:
    """Own software evidence for ``TaskInvocationFailureIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-TASK-INVOCATION-FAILURE-IDENTITY-001

        Requirement: ``TaskInvocationFailureIdentity`` wraps one nonempty exact
        built-in string.
