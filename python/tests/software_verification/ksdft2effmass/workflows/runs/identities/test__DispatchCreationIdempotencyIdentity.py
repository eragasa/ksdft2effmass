r"""Software verification of ``DispatchCreationIdempotencyIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``DispatchCreationIdempotencyIdentity`` contract.
...
from ksdft2effmass.workflows.runs import DispatchCreationIdempotencyIdentity

pytestmark = pytest.mark.software_verification
SUT = DispatchCreationIdempotencyIdentity


class TestDispatchCreationIdempotencyIdentity:
    """Own software evidence for ``DispatchCreationIdempotencyIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-DISPATCH-CREATION-IDEMPOTENCY-IDENTITY-001

        Requirement: ``DispatchCreationIdempotencyIdentity`` wraps one nonempty
