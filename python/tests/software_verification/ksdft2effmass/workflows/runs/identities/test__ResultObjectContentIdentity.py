r"""Software verification of ``ResultObjectContentIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ResultObjectContentIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ResultObjectContentIdentity

pytestmark = pytest.mark.software_verification
SUT = ResultObjectContentIdentity


class TestResultObjectContentIdentity:
    """Own software evidence for ``ResultObjectContentIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-RESULT-OBJECT-CONTENT-IDENTITY-001

        Requirement: ``ResultObjectContentIdentity`` wraps one nonempty exact
        built-in string.
