r"""Software verification of ``TaskWorkflowMembershipIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskWorkflowMembershipIdentity`` contract.
...
from ksdft2effmass.workflows.runs import TaskWorkflowMembershipIdentity

pytestmark = pytest.mark.software_verification
SUT = TaskWorkflowMembershipIdentity


class TestTaskWorkflowMembershipIdentity:
    """Own software evidence for ``TaskWorkflowMembershipIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-TASK-WORKFLOW-MEMBERSHIP-IDENTITY-001

        Requirement: ``TaskWorkflowMembershipIdentity`` wraps one nonempty exact
        built-in string.
