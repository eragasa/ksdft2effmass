r"""Software verification of ``TaskInvocationOutcomeIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskInvocationOutcomeIdentity`` contract.
...
from ksdft2effmass.workflows.runs import TaskInvocationOutcomeIdentity

pytestmark = pytest.mark.software_verification
SUT = TaskInvocationOutcomeIdentity


class TestTaskInvocationOutcomeIdentity:
    """Own software evidence for ``TaskInvocationOutcomeIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-TASK-INVOCATION-OUTCOME-IDENTITY-001

        Requirement: ``TaskInvocationOutcomeIdentity`` wraps one nonempty exact
        built-in string.
