r"""Software verification of ``ScientificExecutorIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ScientificExecutorIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``ScientificExecutorIdentity``.
...
from ksdft2effmass.workflows.runs import ScientificExecutorIdentity

pytestmark = pytest.mark.software_verification
SUT = ScientificExecutorIdentity


class TestScientificExecutorIdentity:
    """Own software evidence for ``ScientificExecutorIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-SCIENTIFIC-EXECUTOR-IDENTITY-001

        Requirement: ``ScientificExecutorIdentity`` wraps one nonempty exact
        built-in string.
