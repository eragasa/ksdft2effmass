r"""Software verification of ``WorkflowRunRevisionIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``WorkflowRunRevisionIdentity`` contract.
...
from ksdft2effmass.workflows.runs import WorkflowRunRevisionIdentity

pytestmark = pytest.mark.software_verification
SUT = WorkflowRunRevisionIdentity


class TestWorkflowRunRevisionIdentity:
    """Own software evidence for ``WorkflowRunRevisionIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-WORKFLOW-RUN-REVISION-IDENTITY-001

        Requirement: ``WorkflowRunRevisionIdentity`` wraps one nonempty exact
        built-in string.
