r"""Software verification of ``HumanResultAuthorIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``HumanResultAuthorIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``HumanResultAuthorIdentity``.
...
from ksdft2effmass.workflows.runs import HumanResultAuthorIdentity

pytestmark = pytest.mark.software_verification
SUT = HumanResultAuthorIdentity


class TestHumanResultAuthorIdentity:
    """Own software evidence for ``HumanResultAuthorIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-HUMAN-RESULT-AUTHOR-IDENTITY-001

        Requirement: ``HumanResultAuthorIdentity`` wraps one nonempty exact
        built-in string.
