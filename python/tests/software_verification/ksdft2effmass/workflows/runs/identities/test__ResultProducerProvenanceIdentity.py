r"""Software verification of ``ResultProducerProvenanceIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ResultProducerProvenanceIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ResultProducerProvenanceIdentity

pytestmark = pytest.mark.software_verification
SUT = ResultProducerProvenanceIdentity


class TestResultProducerProvenanceIdentity:
    """Own software evidence for ``ResultProducerProvenanceIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-RESULT-PRODUCER-PROVENANCE-IDENTITY-001

        Requirement: ``ResultProducerProvenanceIdentity`` wraps one nonempty
