r"""Software verification of ``ScientificDecisionRequestIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ScientificDecisionRequestIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ScientificDecisionRequestIdentity

pytestmark = pytest.mark.software_verification
SUT = ScientificDecisionRequestIdentity


class TestScientificDecisionRequestIdentity:
    """Own software evidence for ``ScientificDecisionRequestIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-SCIENTIFIC-DECISION-REQUEST-IDENTITY-001

        Requirement: ``ScientificDecisionRequestIdentity`` wraps one nonempty
