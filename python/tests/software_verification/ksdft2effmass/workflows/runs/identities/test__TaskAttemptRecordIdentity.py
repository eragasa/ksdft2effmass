r"""Software verification of ``TaskAttemptRecordIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskAttemptRecordIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``TaskAttemptRecordIdentity``.
...
from ksdft2effmass.workflows.runs import TaskAttemptRecordIdentity

pytestmark = pytest.mark.software_verification
SUT = TaskAttemptRecordIdentity


class TestTaskAttemptRecordIdentity:
    """Own software evidence for ``TaskAttemptRecordIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-TASK-ATTEMPT-RECORD-IDENTITY-001

        Requirement: ``TaskAttemptRecordIdentity`` wraps one nonempty exact
        built-in string.
