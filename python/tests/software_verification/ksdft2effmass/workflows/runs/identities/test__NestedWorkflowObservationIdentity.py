r"""Software verification of ``NestedWorkflowObservationIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``NestedWorkflowObservationIdentity`` contract.
...
from ksdft2effmass.workflows.runs import NestedWorkflowObservationIdentity

pytestmark = pytest.mark.software_verification
SUT = NestedWorkflowObservationIdentity


class TestNestedWorkflowObservationIdentity:
    """Own software evidence for ``NestedWorkflowObservationIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-NESTED-WORKFLOW-OBSERVATION-IDENTITY-001

        Requirement: ``NestedWorkflowObservationIdentity`` wraps one nonempty
