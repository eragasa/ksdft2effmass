r"""Software verification of ``ResultObjectReferenceIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ResultObjectReferenceIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ResultObjectReferenceIdentity

pytestmark = pytest.mark.software_verification
SUT = ResultObjectReferenceIdentity


class TestResultObjectReferenceIdentity:
    """Own software evidence for ``ResultObjectReferenceIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-RESULT-OBJECT-REFERENCE-IDENTITY-001

        Requirement: ``ResultObjectReferenceIdentity`` wraps one nonempty exact
        built-in string.
