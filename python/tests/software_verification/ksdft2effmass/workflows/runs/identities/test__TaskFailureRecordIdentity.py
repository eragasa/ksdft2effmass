r"""Software verification of ``TaskFailureRecordIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskFailureRecordIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``TaskFailureRecordIdentity``.
...
from ksdft2effmass.workflows.runs import TaskFailureRecordIdentity

pytestmark = pytest.mark.software_verification
SUT = TaskFailureRecordIdentity


class TestTaskFailureRecordIdentity:
    """Own software evidence for ``TaskFailureRecordIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-TASK-FAILURE-RECORD-IDENTITY-001

        Requirement: ``TaskFailureRecordIdentity`` wraps one nonempty exact
        built-in string.
