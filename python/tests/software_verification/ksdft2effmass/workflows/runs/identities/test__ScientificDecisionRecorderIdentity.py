r"""Software verification of ``ScientificDecisionRecorderIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ScientificDecisionRecorderIdentity`` contract.
...
from ksdft2effmass.workflows.runs import ScientificDecisionRecorderIdentity

pytestmark = pytest.mark.software_verification
SUT = ScientificDecisionRecorderIdentity


class TestScientificDecisionRecorderIdentity:
    """Own software evidence for ``ScientificDecisionRecorderIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-SCIENTIFIC-DECISION-RECORDER-IDENTITY-001

        Requirement: ``ScientificDecisionRecorderIdentity`` wraps one nonempty
