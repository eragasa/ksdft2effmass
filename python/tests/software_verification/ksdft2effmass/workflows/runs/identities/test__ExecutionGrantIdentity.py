r"""Software verification of ``ExecutionGrantIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ExecutionGrantIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``ExecutionGrantIdentity``.
...
from ksdft2effmass.workflows.runs import ExecutionGrantIdentity

pytestmark = pytest.mark.software_verification
SUT = ExecutionGrantIdentity


class TestExecutionGrantIdentity:
    """Own software evidence for ``ExecutionGrantIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-EXECUTION-GRANT-IDENTITY-001

        Requirement: ``ExecutionGrantIdentity`` wraps one nonempty exact built-
        in string.
