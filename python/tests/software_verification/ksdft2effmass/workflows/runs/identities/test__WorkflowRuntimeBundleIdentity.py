r"""Software verification of ``WorkflowRuntimeBundleIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``WorkflowRuntimeBundleIdentity`` contract.
...
from ksdft2effmass.workflows.runs import WorkflowRuntimeBundleIdentity

pytestmark = pytest.mark.software_verification
SUT = WorkflowRuntimeBundleIdentity


class TestWorkflowRuntimeBundleIdentity:
    """Own software evidence for ``WorkflowRuntimeBundleIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-WORKFLOW-RUNTIME-BUNDLE-IDENTITY-001

        Requirement: ``WorkflowRuntimeBundleIdentity`` wraps one nonempty exact
        built-in string.
