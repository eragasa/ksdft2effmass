r"""Software verification of ``ResultDependencyIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``ResultDependencyIdentity`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``ResultDependencyIdentity``.
...
from ksdft2effmass.workflows.runs import ResultDependencyIdentity

pytestmark = pytest.mark.software_verification
SUT = ResultDependencyIdentity


class TestResultDependencyIdentity:
    """Own software evidence for ``ResultDependencyIdentity``."""
    ...
    def test_constructor__value__requires_nonempty_exact_text(self) -> None:
        ...
        Evidence ID: SV-WFR-RESULT-DEPENDENCY-IDENTITY-001

        Requirement: ``ResultDependencyIdentity`` wraps one nonempty exact
        built-in string.
