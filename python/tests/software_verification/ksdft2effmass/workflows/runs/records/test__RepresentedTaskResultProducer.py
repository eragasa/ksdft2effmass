r"""Software verification of ``RepresentedTaskResultProducer``.

Evidence profile: routine

Bounded artifact scope: the public ``RepresentedTaskResultProducer`` contract.
...
from ksdft2effmass.workflows.runs import RepresentedTaskResultProducer

pytestmark = pytest.mark.software_verification
SUT = RepresentedTaskResultProducer


class TestRepresentedTaskResultProducer:
    """Own software evidence for ``RepresentedTaskResultProducer``."""

    def test_fields__public_contract__matches_exact_inventory(self) -> None:
        """Expose the exact documented immutable field inventory.

        Evidence ID: SV-WFR-REPRESENTED-TASK-RESULT-PRODUCER-001

        Requirement: ``RepresentedTaskResultProducer`` declares exactly its
