r"""Software verification of ``TaskInvocationFailure``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskInvocationFailure`` contract.

Facet and represented meaning

This module verifies the exact public structure owned by ``TaskInvocationFailure``.
...
from ksdft2effmass.workflows.runs import TaskInvocationFailure

pytestmark = pytest.mark.software_verification
SUT = TaskInvocationFailure


class TestTaskInvocationFailure:
    """Own software evidence for ``TaskInvocationFailure``."""

    def test_fields__public_contract__matches_exact_inventory(self) -> None:
        """Expose the exact documented immutable field inventory.

        Evidence ID: SV-WFR-TASK-INVOCATION-FAILURE-001

        Requirement: ``TaskInvocationFailure`` declares exactly its documented
        public DataObject
