r"""Software verification of ``UnknownLegacyResultProducer``.

Evidence profile: routine

Bounded artifact scope: the public ``UnknownLegacyResultProducer`` contract.
...
from ksdft2effmass.workflows.runs import UnknownLegacyResultProducer

pytestmark = pytest.mark.software_verification
SUT = UnknownLegacyResultProducer


class TestUnknownLegacyResultProducer:
    """Own software evidence for ``UnknownLegacyResultProducer``."""

    def test_fields__public_contract__matches_exact_inventory(self) -> None:
        """Expose the exact documented immutable field inventory.

        Evidence ID: SV-WFR-UNKNOWN-LEGACY-RESULT-PRODUCER-001

        Requirement: ``UnknownLegacyResultProducer`` declares exactly its
