r"""Software verification of ``TaskStartGateIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskStartGateIdentity`` identity contract.

Facet and represented meaning

The class nominally identifies one Task start gate without selecting a wire grammar.
...
from ksdft2effmass.workflows import TaskStartGateIdentity

pytestmark = pytest.mark.software_verification
SUT = TaskStartGateIdentity


def test_constructor__value__requires_nonempty_exact_string() -> None:
    """Test the exact lexical boundary of ``TaskStartGateIdentity.value``.

    Evidence ID: SV-WFM-IDENTITY-011
...
def test_method__setattr__rejects_identity_mutation() -> None:
    """Test that ``TaskStartGateIdentity`` is operationally immutable.

    Evidence ID: SV-WFM-IDENTITY-012
