r"""Software verification of ``TaskInstanceIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``TaskInstanceIdentity`` identity contract.

Facet and represented meaning

The class nominally identifies one run-scoped Task instance without selecting a
wire grammar.
...
SUT = TaskInstanceIdentity


def test_constructor__value__requires_nonempty_exact_string() -> None:
    """Test the exact lexical boundary of ``TaskInstanceIdentity.value``.

    Evidence ID: SV-WFM-IDENTITY-005
...
def test_method__setattr__rejects_identity_mutation() -> None:
    """Test that ``TaskInstanceIdentity`` is operationally immutable.

    Evidence ID: SV-WFM-IDENTITY-006
