r"""Software verification of ``AttemptIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``AttemptIdentity`` identity contract.

Facet and represented meaning

The class nominally identifies one bounded operation attempt without selecting a
wire grammar.
...
from ksdft2effmass.workflows import AttemptIdentity
...
def test_constructor__value__requires_nonempty_exact_string() -> None:
    """Test the exact lexical boundary of ``AttemptIdentity.value``.

    Evidence ID: SV-WFM-IDENTITY-019
...
def test_method__setattr__rejects_identity_mutation() -> None:
    """Test that ``AttemptIdentity`` is operationally immutable.

    Evidence ID: SV-WFM-IDENTITY-020
