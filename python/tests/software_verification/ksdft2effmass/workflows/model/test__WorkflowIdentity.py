r"""Software verification of ``WorkflowIdentity``.

Evidence profile: routine

Bounded artifact scope: the public ``WorkflowIdentity`` identity contract.

Facet and represented meaning

The class nominally identifies one Workflow definition without selecting a wire grammar.
...
from ksdft2effmass.workflows import WorkflowIdentity

pytestmark = pytest.mark.software_verification
SUT = WorkflowIdentity


def test_constructor__value__requires_nonempty_exact_string() -> None:
    """Test the exact lexical boundary of ``WorkflowIdentity.value``.

    Evidence ID: SV-WFM-IDENTITY-007
...
def test_method__setattr__rejects_identity_mutation() -> None:
    """Test that ``WorkflowIdentity`` is operationally immutable.

    Evidence ID: SV-WFM-IDENTITY-008
