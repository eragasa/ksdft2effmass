...
def test_public_api__package_import__resolves_module() -> None:
    ...
    SV-PACKAGE-001
    Requirement
    The public root package is importable as ``ksdft2effmass``.
    Method
    Import the package through Python's public import statement in the configured test
    environment and retain the resulting module binding.
    Oracle
    Python import semantics require a successful import to bind a module object; the
    accepted public package name is the fixed literal ``ksdft2effmass``.
    Acceptance
    Import completes without an exception and the resulting binding is not ``None``.
    Interpretation
    Failure indicates package discovery, environment configuration, or public root-name
    drift; passing establishes only availability in this test environment.
    Limitations
    Subpackage exports, installed-wheel behavior, numerical verification, scientific
