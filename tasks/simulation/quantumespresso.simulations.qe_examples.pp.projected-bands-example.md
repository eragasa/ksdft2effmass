# QE 7.2 bundled PP projected_bands_example example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.pp.projected-bands-example`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `PP/examples/projected_bands_example`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `PP/examples/projected_bands_example` containing `PP/examples/projected_bands_example/run_example`.
...
- Inventory source group `PP/examples/projected_bands_example` and candidate script(s): `PP/examples/projected_bands_example/run_example`.
