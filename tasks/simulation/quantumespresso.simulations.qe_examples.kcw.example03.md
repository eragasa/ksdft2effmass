# QE 7.2 bundled KCW example03 example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.kcw.example03`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `KCW/examples/example03`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `KCW/examples/example03` containing `KCW/examples/example03/run_example`.
...
- Inventory source group `KCW/examples/example03` and candidate script(s): `KCW/examples/example03/run_example`.
