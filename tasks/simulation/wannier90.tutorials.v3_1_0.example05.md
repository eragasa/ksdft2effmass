# Wannier90 3.1.0 example05: Diamond valence states
...
- **Schema version:** `3`
- **Task ID:** `wannier90.tutorials.v3_1_0.example05`
- **Status:** `blocked`
- **Status detail:** Blocked candidate first tutorial. Exact source and executable identities are known, but no workspace or execution is authorized; explicit pseudopotential-use terms or operator acceptance and the IEEE-warning handling rule remain unresolved. The private Project Koios Tutorial 5 note is an AUTOMATED_UNREVIEWED procedure aid only; retained source inputs and tutorial PDFs control.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the bundled Wannier90 3.1.0 `examples/example05` tutorial on diamond valence states.
...
- Inventory the exact `examples/example05` source directory: 4 files with canonical path-and-content manifest SHA-256 `3994b438ccd2002eed09175c9a505d9babe08492cf5097711441321fdc382b2e`.
- Use the private Project Koios Tutorial 5 note only as an AUTOMATED_UNREVIEWED procedure aid; read it in place without copying its content, and resolve any conflict in favor of the retained source inputs and the source-distribution tutorial and solution PDFs.
...
- Preflight the exact five-stage sequence: Quantum ESPRESSO SCF, 64-point uniform-grid NSCF, `wannier90.x -pp diamond`, `pw2wannier90.x`, and `wannier90.x diamond`; do not omit or reorder a stage based on a documentation-derived aid.
- Treat singleton local execution with `OMP_NUM_THREADS=1`, a proposed 10-minute and 2-GiB-RSS limit per stage, a 1-GiB total workspace limit, no retries, and no setting edits as a proposed envelope requiring exact operator authorization rather than an active grant.
- Under a future exact grant, stop without retry on an identity mismatch, pre-existing output, missing input or pseudopotential, nonzero exit, signal, timeout or resource breach, missing required normal-completion marker, fatal diagnostic, or missing predecessor/artifact.
