# QE 7.2 bundled CPV Restart_example example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.cpv.restart-example`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `CPV/examples/Restart_example`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `CPV/examples/Restart_example` containing `CPV/examples/Restart_example/run_example`.
...
- Inventory source group `CPV/examples/Restart_example` and candidate script(s): `CPV/examples/Restart_example/run_example`.
