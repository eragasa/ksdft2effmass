# QE 7.2 bundled FFTXlib root example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.fftxlib.root`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `FFTXlib/examples`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `FFTXlib/examples` containing `FFTXlib/examples/run_example`.
...
- Inventory source group `FFTXlib/examples` and candidate script(s): `FFTXlib/examples/run_example`.
