# QE 7.2 bundled PP WAN90_example example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.pp.wan90-example`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `PP/examples/WAN90_example`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `PP/examples/WAN90_example` containing `PP/examples/WAN90_example/run_example`.
...
- Inventory source group `PP/examples/WAN90_example` and candidate script(s): `PP/examples/WAN90_example/run_example`.
