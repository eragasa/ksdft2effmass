# QE 7.2 bundled XSpectra NiO example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.xspectra.ni-o`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `XSpectra/examples/NiO`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `XSpectra/examples/NiO` containing `XSpectra/examples/run_example_NiO`.
...
- Inventory source group `XSpectra/examples/NiO` and candidate script(s): `XSpectra/examples/run_example_NiO`.
