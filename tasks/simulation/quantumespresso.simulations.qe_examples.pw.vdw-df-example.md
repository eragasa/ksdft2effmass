# QE 7.2 bundled PW vdwDF_example example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.pw.vdw-df-example`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `PW/examples/vdwDF_example`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `PW/examples/vdwDF_example` containing `PW/examples/vdwDF_example/run_example`, `PW/examples/vdwDF_example/run_example_delta_scf`.
...
- Inventory source group `PW/examples/vdwDF_example` and candidate script(s): `PW/examples/vdwDF_example/run_example`, `PW/examples/vdwDF_example/run_example_delta_scf`.
