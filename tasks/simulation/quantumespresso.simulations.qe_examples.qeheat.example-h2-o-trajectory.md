# QE 7.2 bundled QEHeat example_H2O_trajectory example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.qeheat.example-h2-o-trajectory`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `QEHeat/examples/example_H2O_trajectory`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `QEHeat/examples/example_H2O_trajectory` containing `QEHeat/examples/example_H2O_trajectory/run_example.sh`.
...
- Inventory source group `QEHeat/examples/example_H2O_trajectory` and candidate script(s): `QEHeat/examples/example_H2O_trajectory/run_example.sh`.
