# QE 7.2 bundled QEHeat example_small_H20_trajectory example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.qeheat.example-small-h20-trajectory`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `QEHeat/examples/example_small_H20_trajectory`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `QEHeat/examples/example_small_H20_trajectory` containing `QEHeat/examples/example_small_H20_trajectory/run_example_water`.
...
- Inventory source group `QEHeat/examples/example_small_H20_trajectory` and candidate script(s): `QEHeat/examples/example_small_H20_trajectory/run_example_water`.
