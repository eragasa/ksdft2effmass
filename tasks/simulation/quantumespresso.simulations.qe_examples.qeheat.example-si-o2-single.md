# QE 7.2 bundled QEHeat example_SiO2_single example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.qeheat.example-si-o2-single`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `QEHeat/examples/example_SiO2_single`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `QEHeat/examples/example_SiO2_single` containing `QEHeat/examples/example_SiO2_single/run_example.sh`.
...
- Inventory source group `QEHeat/examples/example_SiO2_single` and candidate script(s): `QEHeat/examples/example_SiO2_single/run_example.sh`.
