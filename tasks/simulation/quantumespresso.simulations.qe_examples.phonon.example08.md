# QE 7.2 bundled PHonon example08 example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.phonon.example08`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `PHonon/examples/example08`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `PHonon/examples/example08` containing `PHonon/examples/example08/run_example`.
...
- Inventory source group `PHonon/examples/example08` and candidate script(s): `PHonon/examples/example08/run_example`.
