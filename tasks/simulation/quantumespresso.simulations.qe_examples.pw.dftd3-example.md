# QE 7.2 bundled PW dftd3_example example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.pw.dftd3-example`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `PW/examples/dftd3_example`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `PW/examples/dftd3_example` containing `PW/examples/dftd3_example/run_example`.
...
- Inventory source group `PW/examples/dftd3_example` and candidate script(s): `PW/examples/dftd3_example/run_example`.
