# QE 7.2 bundled TDDFPT example02 example
...
- **Schema version:** `3`
- **Task ID:** `quantumespresso.simulations.qe_examples.tddfpt.example02`
- **Status:** `blocked`
- **Status detail:** Inventory-only Task for the bundled Quantum ESPRESSO 7.2 source group `TDDFPT/examples/example02`. No script has been invoked by this Task. Exact source, executable, input, pseudopotential, dependency, resource, retention, and protected-execution preflights remain required.
...
Preflight and, only after exact protected-execution authorization, execute or deliberately defer the Quantum ESPRESSO 7.2 bundled example group `TDDFPT/examples/example02` containing `TDDFPT/examples/example02/run_example`.
...
- Inventory source group `TDDFPT/examples/example02` and candidate script(s): `TDDFPT/examples/example02/run_example`.
