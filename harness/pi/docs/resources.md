...
The skill does not reproduce mechanics. `ResourceManifestValidator` owns manifest and overlay structure, `ResourceManifestRefresher` owns explicit selected byte-identity proposals, `ResourceResolver` owns root-confined selection and hash agreement, `SkillResourceValidator` owns descriptor closure, the JSON actions own canonical wire representation, `ChecksumManifestValidator` owns checksum agreement, `ProjectProfileLoader` owns explicit profile loading, and the project-local context ActionObject owns local composition. Historical resource agents remain historical and disabled.

## Validation, resolution, and identity refresh

Intrinsic field invariants and canonical resource ordering belong to `ArtifactIdentity`, `ResourceReference`, and `ResourceManifest`. Cross-resource duplicate, dependency, overlay, and profile compatibility checks belong to `ResourceManifestValidator`. Exact selection with identity agreement belongs to `ResourceResolver`.
