...
A parameter family reused by multiple tests may use one module-local named inventory. The name must be assigned exactly once, before every consumer, directly to a literal tuple (preferred) or list of direct `pytest.param(...)` calls. Every case retains one explicit static semantic `id=`. The validator resolves only a simple decorator name in the same module; it never imports or executes the module and never evaluates parameter arguments. Named inventories prohibit imported or unresolved names, generated or callable values, comprehensions, concatenation, unpacking or starred expansion, reassignment, augmented assignment, duplicate IDs, non-`pytest.param` elements, and list mutation through append, extend, insert, clear, sort, reverse, or subscript assignment. A named list is accepted only when the complete module AST proves absence of these mutations. Reuse contributes the same static case count independently to each consuming test.
